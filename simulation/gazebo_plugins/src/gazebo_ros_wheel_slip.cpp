...
public:
  ...
  double validate_wheel_slip_value(const double& value, std::string slip_type, std::string wheel_name) const {
    if (value < 0.0) {
      RCLCPP_INFO(
          ros_node_->get_logger(), "Negative slip %s compliance value found in sdf for %s will be set to 0.0",
          slip_type.c_str(), wheel_name.c_str());
      return 0.0;
    }
    return value;
  }
...
GazeboRosWheelSlip::GazeboRosWheelSlip() : impl_(std::make_unique<GazeboRosWheelSlipPrivate>()) {}
...
void GazeboRosWheelSlip::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("wheel")) {
    for (auto wheel_element = _sdf->GetElement("wheel"); wheel_element;
      ...
      const double slip_lateral = this->impl_->validate_wheel_slip_value(
          wheel_element->Get<double>("slip_compliance_lateral"), "lateral", wheel_name);
      ...
      const double slip_longitudinal = this->impl_->validate_wheel_slip_value(
          wheel_element->Get<double>("slip_compliance_longitudinal"), "longitudinal", wheel_name);
  ...
  impl_->ros_node_->declare_parameter("slip_compliance_unitless_lateral", rclcpp::PARAMETER_DOUBLE);
  impl_->ros_node_->declare_parameter("slip_compliance_unitless_longitudinal", rclcpp::PARAMETER_DOUBLE);
  ...
  for (auto& wheel_parameter : impl_->map_slip_lateral_default_) {
    double value = impl_->ros_node_->declare_parameter(
        "slip_compliance_unitless_lateral/" + wheel_parameter.first, wheel_parameter.second);
    this->SetSlipComplianceLateral(wheel_parameter.first, value);
  }

  for (auto& wheel_parameter : impl_->map_slip_longitudinal_default_) {
    double value = impl_->ros_node_->declare_parameter(
        "slip_compliance_unitless_longitudinal/" + wheel_parameter.first, wheel_parameter.second);
    this->SetSlipComplianceLongitudinal(wheel_parameter.first, value);
  }

  auto param_validation_callback = [this](std::vector<rclcpp::Parameter> parameters) {
  ...
  impl_->on_set_parameters_callback_handle_ =
      impl_->ros_node_->add_on_set_parameters_callback(param_validation_callback);
    ...
    if (event.node == complete_node_name) {
      ...
      for (auto& parameter : param_list) {
        ...
        if (parameter.name == "slip_compliance_unitless_lateral") {
          ...
          RCLCPP_INFO(
              this->impl_->ros_node_->get_logger(), "Setting lateral slip compliance for all wheels: %.3e", slip);
        ...
        if (parameter.name == "slip_compliance_unitless_longitudinal") {
          double slip = temp_param.as_double();
          RCLCPP_INFO(
              this->impl_->ros_node_->get_logger(), "Setting longitudinal slip compliance for all wheels: %.3e", slip);
          // Iterate over all wheels
        ...
        if (parameter.name.find("slip_compliance_unitless_lateral/") != std::string::npos) {
          ...
          if (this->impl_->map_slip_lateral_default_.count(wheel_name) == 1) {
            ...
            double slip = temp_param.as_double();
            RCLCPP_INFO(
                this->impl_->ros_node_->get_logger(), "New lateral slip compliance for %s: %.3e", wheel_name.c_str(),
                slip);
        ...
        if (parameter.name.find("slip_compliance_unitless_longitudinal/") != std::string::npos) {
          ...
          if (this->impl_->map_slip_longitudinal_default_.count(wheel_name) == 1) {
            ...
            double slip = temp_param.as_double();
            RCLCPP_INFO(
                this->impl_->ros_node_->get_logger(), "New longitudinal slip compliance for %s: %.3e",
                wheel_name.c_str(), slip);
        ...
        if (parameter.name.find("friction_coefficient_primary/") != std::string::npos) {
          ...
          if (this->impl_->map_friction_primary_default_.count(wheel_name) == 1) {
            ...
            if (this->SetMuPrimary(wheel_name, friction)) {
              RCLCPP_INFO(
                  this->impl_->ros_node_->get_logger(), "New friction coefficient in primary direction for %s: %.3e",
                  wheel_name.c_str(), friction);
            } else {
              RCLCPP_ERROR(
                  this->impl_->ros_node_->get_logger(),
                  "Unable to set friction coefficient in primary direction for %s", wheel_name.c_str());
            }
          }
        }
        ...
        if (parameter.name.find("friction_coefficient_secondary/") != std::string::npos) {
          ...
          if (this->impl_->map_friction_secondary_default_.count(wheel_name) == 1) {
            ...
            if (this->SetMuSecondary(wheel_name, friction)) {
              RCLCPP_INFO(
                  this->impl_->ros_node_->get_logger(), "New friction coefficient in secondary direction for %s: %.3e",
                  wheel_name.c_str(), friction);
            } else {
              RCLCPP_ERROR(
                  this->impl_->ros_node_->get_logger(),
                  "Unable to set friction coefficient in secondary direction for %s", wheel_name.c_str());
            }
          }
        }
#endif
  ...
  impl_->parameter_set_event_callback_ =
      impl_->parameter_event_handler_->add_parameter_event_callback(parameter_set_callback_function);

  double wheel_spin_tolerance = 1.0e-3;
  if (!_sdf->HasElement("wheel_spin_tolerance")) {
    RCLCPP_INFO(impl_->ros_node_->get_logger(), "Missing <wheel_spin_tolerance>, defaults to %f", wheel_spin_tolerance);
  } else {
  ...
  if (!_sdf->HasElement("publisher_update_rate")) {
    RCLCPP_INFO(
        impl_->ros_node_->get_logger(), "Missing <publisher_update_rate>, defaults to %f", publisher_update_rate);
  } else {
  ...
  impl_->slip_publisher_ = impl_->ros_node_->create_publisher<gazebo_msgs::msg::WheelSlip>("wheel_slip", 10);

  auto on_update_callback = [this, wheel_spin_tolerance](const gazebo::common::UpdateInfo& info) {
    ...
    // Check period
    double seconds_since_last_update = (current_time - impl_->publisher_last_update_time_).Double();
  ...
  impl_->publisher_update_connection_ = gazebo::event::Events::ConnectWorldUpdateBegin(on_update_callback);
}
