...
int main(int argc, char **argv) {
  ...
  gazebo::transport::PublisherPtr pub_camera = node->Advertise<gap::msgs::CameraUtilsRequest>(CAMERA_UTILS_TOPIC);
  // Subscribe to the CameraUtils reply topic and link callback function
  gazebo::transport::SubscriberPtr sub_camera = node->Subscribe(CAMERA_UTILS_RESPONSE_TOPIC, onCameraUtilsResponse);

  // Publish to the VisualUtils topic
  gazebo::transport::PublisherPtr pub_visual = node->Advertise<gap::msgs::VisualUtilsRequest>(VISUAL_UTILS_TOPIC);
  // Subscribe to the VisualUtils reply topic and link callback function
  gazebo::transport::SubscriberPtr sub_visual = node->Subscribe(VISUAL_UTILS_RESPONSE_TOPIC, onVisualUtilsResponse);

  // Publish to the WorldUtils request topic
  gazebo::transport::PublisherPtr pub_world = node->Advertise<gap::msgs::WorldUtilsRequest>(WORLD_UTILS_TOPIC);
  // Subscribe to the WorldUtils reply topic and link callback function
  gazebo::transport::SubscriberPtr sub_world = node->Subscribe(WORLD_UTILS_RESPONSE_TOPIC, onWorldUtilsResponse);
  ...
  for (int iteration = start; iteration < scenes + start; iteration++) {
    ...
    debugPrintTrace("Scene (" << iteration << "/" << scenes + start - 1 << "): " << num_objects << " objects");
...
void addDynamicModels(gap::msgs::WorldUtilsRequest &msg) {
  ...
  for (int i = 0; i < types.size(); i++) {
    for (int j = 1; j <= g_obj_max; j++) {
      ...
      std::string sdf{std::istreambuf_iterator<char>(infile), std::istreambuf_iterator<char>()};
...
ignition::math::Pose3d getRandomCameraPose() {
  ...
  if (g_viewpoint == MOVING_VIEW_POS) {
    offset.Set(getRandomDouble(-0.1, 0.1), getRandomDouble(-0.1, 0.1), getRandomDouble(-0.1, 0.1));
  }
  // Random orientation (Big variation)
  if (g_viewpoint == MOVING_VIEW_POS_ROT) {
    ignition::math::Quaternion<double> aux(
        getRandomDouble(0, M_PI / 4.0), getRandomDouble(0, M_PI / 4.0), getRandomDouble(0, M_PI / 4.0));
    original_orientation.Euler(aux.Euler());
  }

  new_pose.Set(position + offset, (correct_orientation * original_orientation).Inverse());
...
ignition::math::Pose3d getRandomLightPose() {
  ignition::math::Quaternion<double> light_orientation(
      getRandomDouble(-M_PI / 5.0, M_PI / 5.0), getRandomDouble(-M_PI / 5.0, M_PI / 5.0),
...
void storeAnnotations(const std::string &path, const int iteration) {
  ...
  std::string image_name = std::to_string(iteration / 100) + "00/" + std::to_string(iteration) + ext_img;
