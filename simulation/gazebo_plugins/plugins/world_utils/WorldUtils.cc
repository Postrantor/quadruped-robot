...
WorldUtils::WorldUtils() : WorldPlugin() { gzmsg << "[WorldUtils] Loaded world tools." << std::endl; }
...
void WorldUtils::Load(physics::WorldPtr _world, sdf::ElementPtr _sdf) {
  ...
  this->pub = this->node->Advertise<gap::msgs::WorldUtilsResponse>(RESPONSE_TOPIC);
  ...
  this->updateConnection = event::Events::ConnectPreRender(std::bind(&WorldUtils::onUpdate, this));
}
  ...
  if (type == SPAWN) {
    for (int i = 0; i < _msg->object_size(); i++) {
      ...
      if (model_type == SPHERE) {
        name = _msg->object(i).has_name() ? _msg->object(i).name()
                                          : "plugin_sphere_" + std::to_string(this->sphere_counter++);
        radius = _msg->object(i).has_radius() ? _msg->object(i).radius() : 1.0;

        sdf_string = genSphere(name, mass, radius, pos, ori);

      } else if (model_type == CYLINDER) {
        name = _msg->object(i).has_name() ? _msg->object(i).name()
                                          : "plugin_cylinder_" + std::to_string(this->cylinder_counter++);
        radius = _msg->object(i).has_radius() ? _msg->object(i).radius() : 1.0;
        length = _msg->object(i).has_length() ? _msg->object(i).length() : 1.0;

        sdf_string = genCylinder(name, mass, radius, length, pos, ori);

      } else if (model_type == BOX) {
        name =
            _msg->object(i).has_name() ? _msg->object(i).name() : "plugin_box_" + std::to_string(this->box_counter++);
        if (_msg->object(i).has_box_size()) box_size = msgs::ConvertIgn(_msg->object(i).box_size());

        sdf_string = genBox(name, mass, box_size, pos, ori);

      } else if (model_type == CUSTOM || model_type == CUSTOM_LIGHT) {
        sdf_string = _msg->object(i).has_sdf() ? _msg->object(i).sdf() : "";
      ...
      if (!sdf_string.empty()) {
        ...
        if (model_type != CUSTOM && model_type != CUSTOM_LIGHT) {
          ...
          model_str << "<sdf version='" << SDF_VERSION << "'>" << sdf_string << "</sdf>";
          ...
          if (_msg->object(i).has_pose()) {
            ...
            pose_xml << "<pose>" << pos.X() << " " << pos.Y() << " " << pos.Z() << " " << rpy.X() << " " << rpy.Y()
                     << " " << rpy.Z() << "</pose>";

            std::string new_model_str = std::regex_replace(sdf_string, this->pose_reg, pose_xml.str());
        ...
        if (_msg->object(i).has_texture_uri() && _msg->object(i).has_texture_name()) {
          ...
          std::string texture_str =
              "<script><uri>" + texture_uri + "</uri>" + "<name>" + texture_name + "</name></script>";

          new_model_str = std::regex_replace(model_str.str(), this->script_reg, texture_str);
    ...
    bool state = (_msg->has_state()) ? _msg->state() : !this->world->PhysicsEnabled();
    this->world->SetPhysicsEnabled(state);

    gzdbg << "[WorldUtils] Physics state " << this->world->PhysicsEnabled() << std::endl;

  } else if (type == PAUSE) {
    // Pause or unpause the world
    bool state = (_msg->has_state()) ? _msg->state() : !this->world->IsPaused();
    ;
...
/////////////////////////////////////////////////
void WorldUtils::clearWorld() { this->world->Clear(); }
...
const std::string WorldUtils::genSphere(
  ...
  msgs::Set(model.mutable_pose(), ignition::math::Pose3d(position, orientation));
