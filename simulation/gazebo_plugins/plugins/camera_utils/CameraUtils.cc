...
/////////////////////////////////////////////////
CameraUtils::CameraUtils() : SensorPlugin(), dataPtr(new CameraUtilsPrivate) {
...
void CameraUtils::Load(sensors::SensorPtr _sensor, sdf::ElementPtr _sdf) {
  ...
  if (!_sensor) gzerr << "[CameraUtils] Invalid sensor pointer." << std::endl;
  ...
  this->dataPtr->sub = this->dataPtr->node->Subscribe(REQUEST_TOPIC, &CameraUtils::onRequest, this);
  // Setup publisher for the reply topic
  this->dataPtr->pub = this->dataPtr->node->Advertise<gap::msgs::CameraUtilsResponse>(RESPONSE_TOPIC);

  // Create output directory
  boost::filesystem::path dir(output_dir);
  boost::filesystem::create_directories(dir);

  this->newFrameConnection = this->camera->ConnectNewImageFrame(std::bind(
      &CameraUtils::OnNewFrame, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
  ...
  if (_msg->type() == CAPTURE_REQUEST) {
    ...
    */
    ...
    for (int i = 0; i < _msg->projections_size(); i++) {
      ...
      if (_msg->projections(i).has_name()) proj->set_name(_msg->projections(i).name());

      // For each 3D point in the set
      for (int j = 0; j < _msg->projections(i).point3_size(); j++) {
        ignition::math::Vector3d point3 = gazebo::msgs::ConvertIgn(_msg->projections(i).point3(j));
