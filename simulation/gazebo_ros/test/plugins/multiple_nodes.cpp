...
void MultipleNodes::Load(int argc, char** argv) {
  ...
  // Create publishers
  auto pubA =
      nodeA->create_publisher<std_msgs::msg::String>("testA", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local());
  auto pubB =
      nodeB->create_publisher<std_msgs::msg::String>("testB", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local());

  // Run lambdas every 1 second
  using namespace std::chrono_literals;
  timer_ = nodeA->create_wall_timer(1s, [nodeA, nodeB, pubA, pubB]() {
