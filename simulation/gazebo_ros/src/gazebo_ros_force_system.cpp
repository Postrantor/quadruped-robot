...
public:
  ...
  void ClearLinkWrenches(
      gazebo_msgs::srv::LinkRequest::Request::SharedPtr _req, gazebo_msgs::srv::LinkRequest::Response::SharedPtr _res);
...
GazeboRosForceSystem::GazeboRosForceSystem() : impl_(std::make_unique<GazeboRosForceSystemPrivate>()) {}
...
void GazeboRosForceSystemPrivate::OnWorldCreated(const std::string& _world_name) {
  ...
  apply_link_wrench_service_ = ros_node_->create_service<gazebo_msgs::srv::ApplyLinkWrench>(
      "apply_link_wrench",
      std::bind(&GazeboRosForceSystemPrivate::ApplyLinkWrench, this, std::placeholders::_1, std::placeholders::_2));

  clear_link_wrenches_service_ = ros_node_->create_service<gazebo_msgs::srv::LinkRequest>(
      "clear_link_wrenches",
      std::bind(&GazeboRosForceSystemPrivate::ClearLinkWrenches, this, std::placeholders::_1, std::placeholders::_2));

  apply_joint_effort_service_ = ros_node_->create_service<gazebo_msgs::srv::ApplyJointEffort>(
      "apply_joint_effort",
      std::bind(&GazeboRosForceSystemPrivate::ApplyJointEffort, this, std::placeholders::_1, std::placeholders::_2));

  clear_joint_efforts_service_ = ros_node_->create_service<gazebo_msgs::srv::JointRequest>(
      "clear_joint_efforts",
      std::bind(&GazeboRosForceSystemPrivate::ClearJointEfforts, this, std::placeholders::_1, std::placeholders::_2));
}
...
void GazeboRosForceSystemPrivate::TaskExecutor(const gazebo::common::UpdateInfo& _info) {
  ...
  for (auto link_wrench_task = link_wrench_tasks_.begin(); link_wrench_task != link_wrench_tasks_.end();
       ++link_wrench_task) {
    ...
    if (!(*link_wrench_task)->link) {
      ...
      RCLCPP_ERROR(
          ros_node_->get_logger(), "Link [%s] does not exist. Deleting task",
          (*link_wrench_task)->link->GetName().c_str());
    }
  ...
  for (auto joint_effort_task = joint_effort_tasks_.begin(); joint_effort_task != joint_effort_tasks_.end();
       ++joint_effort_task) {
    ...
    if (!(*joint_effort_task)->joint) {
      ...
      RCLCPP_ERROR(
          ros_node_->get_logger(), "Joint [%s] does not exist. Deleting task",
          (*joint_effort_task)->joint->GetName().c_str());
    }
...
void GazeboRosForceSystemPrivate::ApplyLinkWrench(
    gazebo_msgs::srv::ApplyLinkWrench::Request::SharedPtr _req,
    gazebo_msgs::srv::ApplyLinkWrench::Response::SharedPtr _res) {
  auto link = boost::dynamic_pointer_cast<gazebo::physics::Link>(world_->EntityByName(_req->link_name));
  ...
  ignition::math::Vector3d ref_force(_req->wrench.force.x, _req->wrench.force.y, _req->wrench.force.z);
  ignition::math::Vector3d ref_torque(_req->wrench.torque.x, _req->wrench.torque.y, _req->wrench.torque.z);
  ignition::math::Vector3d ref_point(_req->reference_point.x, _req->reference_point.y, _req->reference_point.z);
  ...
  if (frame) {
    ...
    RCLCPP_DEBUG(
        ...
        link_pose.X(), link_pose.Y(), link_pose.Z(), link_rot.X(), link_rot.Y(), link_rot.Z(), frame_pose.X(),
        frame_pose.Y(), frame_pose.Z(), frame_rot.X(), frame_rot.Y(), frame_rot.Z(), target_pose.X(), target_pose.Y(),
        target_pose.Z(), target_rot.X(), target_rot.Y(), target_rot.Z());

    TransformWrench(target_force, target_torque, ref_force, ref_torque, target_to_ref);

    RCLCPP_INFO(
        ros_node_->get_logger(), "Wrench defined as [%s]:[%f %f %f, %f %f %f] -> Applied as [%s]:[%f %f %f, %f %f %f]",
        frame->GetName().c_str(), ref_force.X(), ref_force.Y(), ref_force.Z(), ref_torque.X(), ref_torque.Y(),
        ref_torque.Z(), link->GetName().c_str(), target_force.X(), target_force.Y(), target_force.Z(),
    ...
    RCLCPP_INFO(
        ros_node_->get_logger(),
        "Reference_frame is empty/world/map,"
