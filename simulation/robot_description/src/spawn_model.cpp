...
/**
 * `ros2 launch qr_gazebo spwan.launch.py`
 */
...
bool setup_and_spawn_model(GazeboSpawner& cm, const std::string& robot_type) {
  ...
  //   RCLCPP_ERROR_STREAM(cm.get_node()->get_logger(), "fail to spawn model in gazebo: " << robot_type.c_str());
  //   return false;
  // }
  return true;
}
...
int main(int argc, char** argv) {
  ...
  std::string robot_type{"a1"};
  if (argc >= 2) {
    robot_type = argv[1];
  }

  GazeboSpawner cm(robot_type, node);
  RCLCPP_INFO_STREAM(node->get_logger(), "Robot Type: " << cm.get_robot_type().c_str());
  RCLCPP_INFO_STREAM(node->get_logger(), "Node Name: " << cm.get_node()->get_name());

  if (!setup_and_spawn_model(cm, robot_type)) {
    RCLCPP_ERROR_STREAM(node->get_logger(), "fail to spawn model in gazebo: " << robot_type.c_str());
    return 1;
  }

  wait_for_user_input("press any key to start controllers.");
  cm.load_controllers();
  cm.start_controllers();

  wait_for_user_input("press any key to delete controllers and model.");
  if (!cm.stop_controllers()) {
    RCLCPP_ERROR_STREAM(node->get_logger(), "failed to stop controllers in gazebo: " << robot_type.c_str());
    return 1;
  }
