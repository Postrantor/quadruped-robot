...
class JointStatePublisher(rclpy.node.Node):
    ...
    def declare_ros_parameter(self, name, default, descriptor):
        ...
        # 当 rclpy.create_node() 的automatic_declare_parameters_from_overrides参数为True时，会自动声明在命令行上传入的任何参数。在这种情况下，调用 node.declare_parameter() 将引发异常。然而，在参数*不*被覆盖的情况下，我们仍然想要声明它（例如，它显示在“ros2 param list”中）。因此，我们总是进行声明并忽略 ParameterAlreadyDeclaredException。
