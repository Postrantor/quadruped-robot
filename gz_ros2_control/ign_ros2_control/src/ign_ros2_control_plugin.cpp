  ...
  for (const auto &jointEntity : jointEntities) {
    ...
    switch (jointType->Data()) {
      ...
      case sdf::JointType::FIXED: {
        RCLCPP_INFO(
            node_->get_logger(), "[ign_ros2_control] Fixed joint [%s] (Entity=%lu)] is skipped", jointName.c_str(),
            jointEntity);
        continue;
      }
      ...
      default: {
        RCLCPP_WARN(
            node_->get_logger(), "[ign_ros2_control] Joint [%s] (Entity=%lu)] is of unknown type", jointName.c_str(),
            jointEntity);
        continue;
      }
    }
...
std::string IgnitionROS2ControlPluginPrivate::getURDF() const {
  ...
  while (!parameters_client->wait_for_service(0.5s)) {
    if (!rclcpp::ok()) {
      RCLCPP_ERROR(
          node_->get_logger(), "Interrupted while waiting for %s service. Exiting.", robot_description_node_.c_str());
      return 0;
    }
  ...
  RCLCPP_INFO(
      node_->get_logger(), "connected to service!! %s asking for %s", robot_description_node_.c_str(),
...
void IgnitionROS2ControlPlugin::Configure(
  ...
  if (paramFileName.empty()) {
    RCLCPP_ERROR(logger, "Ignition ros2 control found an empty parameters file. Failed to initialize.");
    return;
  }
  ...
  RCLCPP_DEBUG_STREAM(
      ...
      "[Ignition ROS 2 Control] Setting up controller for [" << model.Name(_ecm) << "] (Entity=" << _entity << ")].");
  ...
  try {
    ...
    RCLCPP_ERROR(this->dataPtr->node_->get_logger(), "Error initializing URDF to resource manager!");
  }
  try {
    this->dataPtr->robot_hw_sim_loader_.reset(new pluginlib::ClassLoader<ign_ros2_control::IgnitionSystemInterface>(
        "ign_ros2_control", "ign_ros2_control::IgnitionSystemInterface"));
  } catch (pluginlib::LibraryLoadException &ex) {
    RCLCPP_ERROR(
        this->dataPtr->node_->get_logger(), "Failed to create robot simulation interface loader: %s ", ex.what());
    return;
  }
  ...
  for (unsigned int i = 0; i < control_hardware_info.size(); ++i) {
    ...
    RCLCPP_DEBUG(this->dataPtr->node_->get_logger(), "Load hardware interface %s ...", robot_hw_sim_type_str_.c_str());
    ...
    try {
      ...
      RCLCPP_ERROR(
          this->dataPtr->node_->get_logger(), "The plugin failed to load for some reason. Error: %s\n", ex.what());
      continue;
    }
    if (!ignitionSystem->initSim(
            this->dataPtr->node_, enabledJoints, control_hardware_info[i], _ecm, this->dataPtr->update_rate)) {
...
void IgnitionROS2ControlPlugin::PreUpdate(
  ...
  if (!warned) {
    ...
    if (this->dataPtr->control_period_ < _info.dt) {
      RCLCPP_ERROR_STREAM(
          this->dataPtr->node_->get_logger(), "Desired controller update period ("
...
void IgnitionROS2ControlPlugin::PostUpdate(
  ...
  if (sim_period >= this->dataPtr->control_period_) {
    ...
    auto ign_controller_manager =
        std::dynamic_pointer_cast<ign_ros2_control::IgnitionSystemInterface>(this->dataPtr->controller_manager_);
