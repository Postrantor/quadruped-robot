...
/// \tparam T Output type
template <class T>
inline T Convert(const gazebo::msgs::Contacts &) {
...
template <>
inline gazebo_msgs::msg::ContactsState Convert(const gazebo::msgs::Contacts &in) {
  ...
  for (int i = 0; i < contacts_packet_size; ++i) {
    ...
    stream << "Debug:  i: (" << i + 1 << "/" << contacts_packet_size << ")     my_geom: [" << state.collision1_name
           << "]  other_geom: [" << state.collision2_name << "]        time: [" << contact.time().sec() << "."
           << contact.time().nsec() << "]\n";
    ...
    for (int j = 0; j < contact_group_size; ++j) {
      ...
      ignition::math::Vector3d force = frame_rot.RotateVectorReverse(ignition::math::Vector3d(
          contact.wrench(j).body_1_wrench().force().x(), contact.wrench(j).body_1_wrench().force().y(),
          contact.wrench(j).body_1_wrench().force().z()));
      ignition::math::Vector3d torque = frame_rot.RotateVectorReverse(ignition::math::Vector3d(
          contact.wrench(j).body_1_wrench().torque().x(), contact.wrench(j).body_1_wrench().torque().y(),
