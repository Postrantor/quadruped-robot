...
public:
  ...
  void Load(sensors::SensorPtr _sensor, sdf::ElementPtr _sdf) {
    this->parentSensor = std::dynamic_pointer_cast<sensors::ContactSensor>(
        _sensor); // Make sure the parent sensor is valid.
    ...
    this->force_pub = this->rosnode->advertise<geometry_msgs::WrenchStamped>(
    ...
    this->parentSensor->SetActive(
        true); // Make sure the parent sensor is active.
    count = 0;
    Fx = 0;
    Fy = 0;
    Fz = 0;
  ...
  void OnUpdate() {
    ...
    for (unsigned int i = 0; i < count; ++i) {
      ...
      for (unsigned int j = 0; j < contacts.contact(i).position_size(); ++j) {
        std::cout << i << " " << contacts.contact(i).position_size()
                  << "Force : "
                  ...
                  << "\n";
        Fx += contacts.contact(i)
                  .wrench(0)
                  .body_1_wrench()
                  .force()
                  .x(); // Notice: the force is in local coordinate, not in
                        // world or base coordnate.
