...
public:
  ...
  void Load(rendering::VisualPtr _parent, sdf::ElementPtr _sdf) {
    ...
    if (!_sdf->HasElement("topicName")) {
      ROS_INFO("Force draw plugin missing <topicName>, defaults to "
               "/default_force_draw");
    ...
    this->line =
        this->visual->CreateDynamicLine(rendering::RENDERING_LINE_STRIP);
    ...
    this->line->AddPoint(ignition::math::Vector3d(0, 0, 0),
                         ignition::math::Color(0, 1, 0, 1.0));
    this->line->AddPoint(ignition::math::Vector3d(1, 1, 1),
                         ignition::math::Color(0, 1, 0, 1.0));
#else
    this->line->AddPoint(ignition::math::Vector3d(0, 0, 0),
                         common::Color(0, 1, 0, 1.0));
    this->line->AddPoint(ignition::math::Vector3d(1, 1, 1),
                         common::Color(0, 1, 0, 1.0));
#endif
    ...
    this->force_sub = this->rosnode->subscribe(
        ...
        &UnitreeDrawForcePlugin::GetForceCallback, this);
    this->update_connection = event::Events::ConnectPreRender(
        boost::bind(&UnitreeDrawForcePlugin::OnUpdate, this));
