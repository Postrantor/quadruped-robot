...
public:
  low_state_suber() : Node("low_state_suber") {
    ...
    suber = this->create_subscription<unitree_go::msg::LowState>(
        topic_name, 10,  //
        std::bind(&low_state_suber::topic_callback, this, _1));
  }
...
private:
  void topic_callback(unitree_go::msg::LowState::SharedPtr data) {
    if (INFO_IMU) {
      ...
      RCLCPP_INFO(
          this->get_logger(), "Euler angle -- roll: %f; pitch: %f; yaw: %f",  //
          imu.rpy[0], imu.rpy[1], imu.rpy[2]);
      RCLCPP_INFO(
          this->get_logger(), "Quaternion -- qw: %f; qx: %f; qy: %f; qz: %f",  //
          imu.quaternion[0], imu.quaternion[1], imu.quaternion[2], imu.quaternion[3]);
      RCLCPP_INFO(
          this->get_logger(), "Gyroscope -- wx: %f; wy: %f; wz: %f",  //
          imu.gyroscope[0], imu.gyroscope[1], imu.gyroscope[2]);
      RCLCPP_INFO(
          this->get_logger(), "Accelerometer -- ax: %f; ay: %f; az: %f",  //
    ...
    if (INFO_MOTOR) {
      ...
      for (int i = 0; i < 12; i++) {
        ...
        RCLCPP_INFO(
            this->get_logger(), "Motor state -- num: %d; q: %f; dq: %f; ddq: %f; tau: %f",  //
    ...
    if (INFO_FOOT_FORCE) {
      ...
      RCLCPP_INFO(
          this->get_logger(), "Foot force -- foot0: %d; foot1: %d; foot2: %d; foot3: %d",  //
    ...
    if (INFO_BATTERY) {
      ...
      RCLCPP_INFO(
          this->get_logger(), "Battery state -- current: %f; voltage: %f",  //
...
int main(int argc, char *argv[]) {
  ...
  rclcpp::spin(std::make_shared<low_state_suber>());
  rclcpp::shutdown();
