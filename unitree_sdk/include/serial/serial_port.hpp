...
public:
  SerialPort(const std::string &portName, size_t recvLength = 16,
             uint32_t baudrate = 4000000, size_t timeOutUs = 20000,
  ...
  void resetSerial(size_t recvLength = 16, uint32_t baudrate = 4000000,
                   size_t timeOutUs = 20000, BlockYN blockYN = BlockYN::NO,
                   bytesize_t bytesize = bytesize_t::eightbits,
                   parity_t parity = parity_t::parity_none,
                   stopbits_t stopbits = stopbits_t::stopbits_one,
                   flowcontrol_t flowcontrol = flowcontrol_t::flowcontrol_none);
  ...
  bool sendRecv(std::vector<MotorCmd> &sendVec,
  ...
  bool send_recv(MotorCmd *sendMsg, MotorData *recvMsg) {
    return sendRecv(sendMsg, recvMsg);
  }
  bool send_recv(std::vector<MotorCmd> &sendVec,
                 std::vector<MotorData> &recvVec) {
