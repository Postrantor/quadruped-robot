...
extern const int
    LOW_STATE_LENGTH;  // shorter than sizeof(LowState), bytes compressed LowState length

#pragma pack(1)

typedef struct {
  uint8_t
      off;  // set 0xA5 to turn off the battery, please try it under the premise of ensuring safety
...
typedef struct {
  uint8_t version_h;
  uint8_t version_l;
  uint8_t bms_status;  // 0x00 : wakeup, 0X01 :  discharge, 0x02 : charge, 0x03 : charger, 0x04 :
                       // precharge, 0x05 : charge_err, 0x06 : waterfall_light, 0x07 :
                       // self_discharge, 0x08 : junk.
...
typedef struct {
  ...
  float footRaiseHeight;  // (unit: m, range: -0.06~0.03m, default: 0.09m), foot up height while
                          // walking, delta value
