...
public:
  Custom()
      : low_udp(LOWLEVEL), high_udp(8090, "192.168.123.161", 8082,
...
rclcpp::Subscription<ros2_unitree_legged_msgs::msg::HighCmd>::SharedPtr
    sub_high;
...
void lowCmdCallback(
    const ros2_unitree_legged_msgs::msg::LowCmd::SharedPtr msg) {
...
int main(int argc, char **argv) {
  ...
  if (strcasecmp(argv[1], "LOWLEVEL") == 0) {
    ...
    pub_low = node->create_publisher<ros2_unitree_legged_msgs::msg::LowState>(
        "low_state", 1);
    sub_low = node->create_subscription<ros2_unitree_legged_msgs::msg::LowCmd>(
    ...
    pub_high = node->create_publisher<ros2_unitree_legged_msgs::msg::HighState>(
        "high_state", 1);
    sub_high =
        node->create_subscription<ros2_unitree_legged_msgs::msg::HighCmd>(
            "high_cmd", 1, highCmdCallback);

    rclcpp::spin(node);
  } else {
    std::cout << "Control level name error! Can only be highlevel or "
                 "lowlevel(not case sensitive)"
              << std::endl;
    exit(-1);
  }
