...
public:
  motion_state_suber() : Node("motion_state_suber") {
    // the cmd_puber is set to subscribe "sportmodestate" or  "lf/sportmodestate" (low frequencies)
    // topic
    ...
    writer_->create_topic(
        {"a", "unitree_go/msg/sport_mode_state", rmw_get_serialization_format(), ""});
...
private:
  void topic_callback(unitree_go::msg::SportModeState::SharedPtr data) {
    // Info motion states
    RCLCPP_INFO(
        this->get_logger(), "Gait state -- gait type: %d; raise height: %f",  //
    ...
    RCLCPP_INFO(
        this->get_logger(), "Velocity -- vx: %f; vy: %f; vz: %f; yaw: %f",  //
    ...
    bag_message->serialized_data = std::shared_ptr<rcutils_uint8_array_t>(
        new rcutils_uint8_array_t, [this](rcutils_uint8_array_t *msg) {
          ...
          if (fini_return != RCUTILS_RET_OK) {
            RCLCPP_ERROR(
                get_logger(), "Failed to destroy serialized message %s",
...
int main(int argc, char *argv[]) {
  ...
  rclcpp::spin(std::make_shared<motion_state_suber>());
  rclcpp::shutdown();
