...
int main(int argc, char **argv) {
  ...
  while (rclcpp::ok()) {
    ...
    high_cmd.level_flag = UNITREE_LEGGED_SDK::HIGHLEVEL;
    high_cmd.mode = 0;
    ...
    if (motion_time > 6000 && motion_time < 7000) {
      high_cmd.mode = 1;
      high_cmd.body_height = -0.2;
    }
    if (motion_time > 7000 && motion_time < 8000) {
      high_cmd.mode = 1;
      high_cmd.body_height = 0.1;
    }
    ...
    if (motion_time > 14000 && motion_time < 18000) {
      high_cmd.mode = 2;
      high_cmd.gait_type = 2;
      high_cmd.velocity[0] = 0.4f;  // -1  ~ +1
      high_cmd.yaw_speed = 2;
      high_cmd.foot_raise_height = 0.1;
