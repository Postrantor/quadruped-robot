...
private:
  ...
  float _targetPos_3[12] = {0.0, 1.36, -2.65, 0.0, 1.36, -2.65, 0.0, 1.36, -2.65, 0.0, 1.36, -2.65};
...
int Custom::queryMotionStatus() {
  ...
  if (motionName.empty()) {
    ...
    std::cout << (motionName == "normal" ? "sport_mode is activate" : "ai_sport is activate")
              << std::endl;
    motionStatus = 1;
  }
  return motionStatus;
...
void Custom::ReleaseMode() { msc.ReleaseMode(); }
...
void Custom::LowCmdWrite() {
  if (_percent_5 < 1) {
    ...
    std::cout << "Imu accelerometer : "
              << "x: " << low_state.imu_state().accelerometer()[0]
  ...
  if (motiontime >= 500) {
    ...
    if ((_percent_1 == 1) && (_percent_2 == 1) && (_percent_3 == 1) && (_percent_4 == 1) &&
      ...
      for (int j = 0; j < 12; j++) {
        low_cmd.motor_cmd()[j].q() =
            (1 - _percent_5) * _targetPos_3[j] + _percent_5 * _targetPos_4[j];
        low_cmd.motor_cmd()[j].dq() = 0;
        low_cmd.motor_cmd()[j].kp() = Kp;
        low_cmd.motor_cmd()[j].kd() = Kd;
        low_cmd.motor_cmd()[j].tau() = 0;
