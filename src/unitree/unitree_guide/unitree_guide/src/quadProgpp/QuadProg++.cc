...
bool add_constraint(
    Matrix<double>& R, Matrix<double>& J, Vector<double>& d, unsigned int& iq, double& rnorm);
...
l2a: /* Step 2a: determine step direction */
  ...
  /* Compute t2: full step length (minimum step in primal space such that the constraint ip becomes
   * feasible */
  if (fabs(scalar_product(z, z)) > std::numeric_limits<double>::epsilon())  // i.e. z != 0
  {
...
bool add_constraint(
    Matrix<double>& R, Matrix<double>& J, Vector<double>& d, unsigned int& iq, double& R_norm) {
...
inline void forward_elimination(
    const Matrix<double>& L, Vector<double>& y, const Vector<double>& b) {
