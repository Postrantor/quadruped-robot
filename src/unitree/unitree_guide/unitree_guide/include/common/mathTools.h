...
inline T1 invNormalize(
    const T0 value, const T1 min, const T2 max, const double minLim = -1, const double maxLim = 1) {
...
inline void updateCovariance(T1& cov, T2 expPast, T3 newValue, double n) {
  ...
  if (fabs(n - 1) < 0.1) {
    ...
    cov = cov * (n - 1) / n +
          (newValue - expPast) * (newValue - expPast).transpose() * (n - 1) / (n * n);
  }
}
