...
struct Plot {
  ...
  double getX(long long startT) { return (double)(getSystemTime() - startT) * 1e-6; }
...
inline void PyPlot::printXY(
    std::string plotName, std::string curveName, double xRough, int pointNum) {
  _plots[_plotName2ID[plotName]]->printXY(curveName, xRough, pointNum);
}

inline void PyPlot::addPlot(
    std::string plotName, int curveCount, std::vector<std::string> labelVec) {
...
inline void PyPlot::showPlot(std::vector<std::string> plotNameVec) {
  for (std::vector<std::string>::iterator itName = plotNameVec.begin(); itName != plotNameVec.end();
       ++itName) {
    plt::figure();
