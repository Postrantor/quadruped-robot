...
/////////////////////////////////////////////////
DRPlugin::DRPlugin() : WorldPlugin(), data_ptr(new DRPluginPrivate) {
  gzmsg << "[DRPlugin] Launched plugin." << std::endl;
}

/////////////////////////////////////////////////
DRPlugin::~DRPlugin() { gzmsg << "[DRPlugin] Unloaded plugin." << std::endl; }
...
void DRPlugin::Load(physics::WorldPtr _world, sdf::ElementPtr _sdf) {
  ...
  this->update_connection = event::Events::ConnectWorldUpdateBegin(std::bind(&DRPlugin::onUpdate, this));
  ...
  this->data_ptr->sub = this->data_ptr->node->Subscribe(req_topic, &DRPlugin::onRequest, this);
...
void DRPlugin::onUpdate() {
  ...
  if (feedback_pending) {
    ...
    gzdbg << common::Time::GetWallTimeAsISOString() << " - Provided feedback to client." << std::endl;
  }
...
/////////////////////////////////////////////////
void DRPlugin::processInertial(physics::LinkPtr link, const msgs::Inertial& msg) {
  ...
  if (msg.has_ixx() && msg.has_ixy() && msg.has_ixz() && msg.has_iyy() && msg.has_iyz() && msg.has_izz()) {
    ixx = msg.ixx();
    ixy = msg.ixy();
    ixz = msg.ixz();
    iyy = msg.iyy();
...
/////////////////////////////////////////////////
void DRPlugin::processSurface(physics::CollisionPtr collision, const msgs::Surface& msg) {
...
void DRPlugin::processPID(
    int type, physics::JointControllerPtr controller, const std::string& joint, const msgs::PID& msg) {
  bool requires_search{false};
  common::PID pid;
  std::map<std::string, common::PID> pids;

  requires_search = msg.has_p_gain() || msg.has_i_gain() || msg.has_d_gain() || msg.has_i_max() || msg.has_i_min();
  ...
  if (type == POSITION) {
    controller->SetPositionPID(joint, pid);
  } else if (type == VELOCITY) {
    controller->SetVelocityPID(joint, pid);
  }
