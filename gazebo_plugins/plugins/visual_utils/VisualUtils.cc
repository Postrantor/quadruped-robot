...
void VisualUtils::Load(rendering::VisualPtr _visual, sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("patterns")) {
    ...
    boost::split(dataPtr->patterns, patterns_arg, boost::is_any_of(" "), boost::token_compress_on);
  } else {
  ...
  dataPtr->updateConnection = event::Events::ConnectPreRender(std::bind(&VisualUtils::Update, this));
  // Setup transport node
  dataPtr->node = transport::NodePtr(new transport::Node());
  dataPtr->node->Init();
  // Subcribe to the monitored requests topic
  dataPtr->sub = dataPtr->node->Subscribe(REQUEST_TOPIC, &VisualUtils::onRequest, this);
  // Setup publisher for the response topic
  dataPtr->pub = dataPtr->node->Advertise<gap::msgs::VisualUtilsResponse>(RESPONSE_TOPIC);
...
void VisualUtils::onRequest(VisualUtilsRequestPtr &_msg) {
  ...
  if (_msg->type() == UPDATE) {
    ...
    if (index != -1) {
      if (index < _msg->poses_size()) {
        dataPtr->default_pose = gazebo::msgs::ConvertIgn(_msg->poses(index));
      }
    }
  }
}
...
void VisualUtils::loadResources() {
  ...
  for (auto &material : resources) {
    ...
    for (auto &pattern : dataPtr->patterns) {
      ...
      auto res = std::mismatch(pattern.begin(), pattern.end(), name.begin());
  ...
  std::shuffle(std::begin(dataPtr->materials), std::end(dataPtr->materials), rng);
