...
# already been loaded.  See FindJava.cmake for information on how to load Java
# into your CMake project.
#
# ::
#
# add_jar(target_name [SOURCES] source1 [source2 ...] [resource1 ...]
# [INCLUDE_JARS jar1 [jar2 ...]] [ENTRY_POINT entry] [VERSION version]
# [OUTPUT_NAME name] [OUTPUT_DIR dir] )
#
# This command creates a <target_name>.jar.  It compiles the given source files
# (source) and adds the given resource files (resource) to the jar file.  Source
# files can be java files or listing files (prefixed by '@').  If only resource
# files are given then just a jar file is created.  The list of include jars are
# added to the classpath when compiling the java sources and also to the
# dependencies of the target. INCLUDE_JARS also accepts other target names
# created by add_jar.  For backwards compatibility, jar files listed as sources
...
# ::
#
# To use a different output directory than CMAKE_CURRENT_BINARY_DIR you can set
# it with:
#
# ::
#
...
# ::
#
# To define an entry point in your jar you can set it with the ENTRY_POINT named
# argument:
#
# ::
#
...
# named argument to add_jar. This will create a jar file with the name
# shibboleet-1.0.0.jar and will create a symlink shibboleet.jar pointing to the
# jar with the version information.
#
# ::
#
...
# ::
#
# If the target is a JNI library, utilize the following commands to create a JNI
# symbolic link:
#
# ::
#
# set(CMAKE_JNI_TARGET TRUE) add_jar(shibboleet shibbotleet.java VERSION 1.2.0)
...
# code, to prevent the accumulation of duplicate class files in subsequent jars,
# set/reset CMAKE_JAR_CLASSES_PREFIX prior to calling the add_jar() function:
#
# ::
#
...
# properties with the get_property(TARGET <target_name> PROPERTY <propery_name>)
# command.
#
# ::
#
# INSTALL_FILES      The files which should be installed. This is used by
# install_jar(). JNI_SYMLINK        The JNI symlink which should be installed.
# This is used by install_jni_symlink(). JAR_FILE           The location of the
# jar file so that you can include it. CLASSDIR           The directory where
# the class files can be found. For example to use them with javah.
#
# ::
#
# find_jar(<VAR> name | NAMES name1 [name2 ...] [PATHS path1 [path2 ... ENV
# var]] [VERSIONS version1 [version2]] [DOC "cache documentation string"] )
#
# This command is used to find a full path to the named jar.  A cache entry
# named by <VAR> is created to stor the result of this command. If the full path
# to a jar is found the result is stored in the variable and the search will not
# repeated unless the variable is cleared.  If nothing is found, the result will
# be <VAR>-NOTFOUND, and the search will be attempted again next time find_jar
# is invoked with the same variable.  The name of the full path to a file that
# is searched for is specified by the names listed after NAMES argument.
# Additional search locations can be specified after the PATHS argument. If you
# require special a version of a jar file you can specify it with the VERSIONS
# argument.  The argument after DOC will be used for the documentation string in
# the cache.
#
# ::
#
...
# ::
#
# The install_jar() function sets the INSTALL_DESTINATION target property on
# jars so installed. This property holds the DESTINATION as described above, and
# is used by install_jar_exports(). You can get this property with the
# get_property(TARGET <target_name> PROPERTY INSTALL_DESTINATION) command.
#
# ::
#
# install_jni_symlink(target_name destination) install_jni_symlink(target_name
...
# ::
#
# install_jar_exports(TARGETS jar1 [jar2 ...] FILE export_filename DESTINATION
# destination [COMPONENT component])
#
...
# ::
#
# export_jars(TARGETS jar1 [jar2 ...] FILE export_filename)
#
...
# ::
#
# create_javadoc(<VAR> PACKAGES pkg1 [pkg2 ...] [SOURCEPATH <sourcepath>]
# [CLASSPATH <classpath>] [INSTALLPATH <install path>] [DOCTITLE "the
# documentation title"] [WINDOWTITLE "the title of the document"] [AUTHOR
# TRUE|FALSE] [USE TRUE|FALSE] [VERSION TRUE|FALSE] )
#
# Create java documentation based on files or packages.  For more details please
# read the javadoc manpage.
#
...
# ::
#
# Example: create_javadoc(my_example_doc PACKAGES com.exmaple.foo
# com.example.bar SOURCEPATH "${CMAKE_CURRENT_SOURCE_DIR}" CLASSPATH
# ${CMAKE_JAVA_INCLUDE_PATH} WINDOWTITLE "My example" DOCTITLE "<h1>My
# example</h1>" AUTHOR TRUE USE TRUE VERSION TRUE )
#
# The second signature for create_javadoc works on a given list of files.
#
# ::
#
# create_javadoc(<VAR> FILES file1 [file2 ...] [CLASSPATH <classpath>]
# [INSTALLPATH <install path>] [DOCTITLE "the documentation title"] [WINDOWTITLE
# "the title of the document"] [AUTHOR TRUE|FALSE] [USE TRUE|FALSE] [VERSION
# TRUE|FALSE] )
#
# Example:
#
# ::
#
# create_javadoc(my_example_doc FILES ${example_SRCS} CLASSPATH
# ${CMAKE_JAVA_INCLUDE_PATH} WINDOWTITLE "My example" DOCTITLE "<h1>My
# example</h1>" AUTHOR TRUE USE TRUE VERSION TRUE )
#
# Both signatures share most of the options.  These options are the same as what
# you can find in the javadoc manpage.  Please look at the manpage for
...
# [CLASSPATH <classpath>...] [DEPENDS <depend>...] [OUTPUT_NAME
...
# ::
#
# Example: Create_javah(GENERATED_FILES files_headers CLASSES
# org.cmake.HelloWorld CLASSPATH hello.jar )
#
...
# ::
#
# Example: Create_javah(TARGET target_headers CLASSES org.cmake.HelloWorld
# CLASSPATH hello.jar )
#
...
#
# ``CLASSPATH <classpath>...`` Specifies various paths to look up classes. Here
# .class files, jar files or targets created by command add_jar can be used.
#
# ``DEPENDS <depend>...`` Targets on which the javah target depends
#
# ``OUTPUT_NAME <path>`` Concatenates the resulting header files for all the
# classes listed by option CLASSES into <path>. Same behavior as option '-o' of
# javah tool.
#
# ``OUTPUT_DIR <path>`` Sets the directory where the header files will be
# generated. Same behavior as option '-d' of javah tool. If not specified,
# ${CMAKE_CURRENT_BINARY_DIR} is used as output directory.

# =============================================================================
# Copyright 2013 OpenGamma Ltd. <graham@opengamma.com> Copyright 2010-2011
# Andreas schneider <asn@redhat.com> Copyright 2010-2013 Kitware, Inc.
#
...
function(__java_export_jar VAR TARGET PATH)
  ...
  __java_lcat(
    ${VAR} "# Create imported target ${TARGET}" "add_custom_target(${TARGET})"
...
function(add_jar _TARGET_NAME)
  ...
  cmake_parse_arguments(
    _add_jar "" "VERSION;OUTPUT_DIR;OUTPUT_NAME;ENTRY_POINT;MANIFEST"
    "SOURCES;INCLUDE_JARS" ${ARGN})
  ...
  set(CMAKE_JAVA_CLASS_OUTPUT_PATH
      "${_add_jar_OUTPUT_DIR}${CMAKE_FILES_DIRECTORY}/${_TARGET_NAME}.dir")
  ...
  foreach(_JAVA_SOURCE_FILE ${_JAVA_SOURCE_FILES})
    ...
    elseif(_JAVA_EXT MATCHES ".java")
      file(RELATIVE_PATH _JAVA_REL_BINARY_PATH ${_add_jar_OUTPUT_DIR}
           ${_JAVA_FULL})
  ...
  # create the jar file
  set(_JAVA_JAR_OUTPUT_PATH ${_add_jar_OUTPUT_DIR}/${_JAVA_TARGET_OUTPUT_NAME})
  if(CMAKE_JNI_TARGET)
    add_custom_command(
...
function(export_jars)
  ...
  cmake_parse_arguments(_export_jars "" "FILE" "TARGETS" ${ARGN})
...
function(install_jar_exports)
  # Parse and validate arguments
  cmake_parse_arguments(_install_jar_exports "" "FILE;DESTINATION;COMPONENT"
                        "TARGETS" ${ARGN})
