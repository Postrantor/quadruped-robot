...
static bool impl_format_to_ops(
    impl_pack_op_t *ops,
    size_t max_num_ops,
    size_t *num_ops,
    bool *is_little_endian,
  ...
  *num_ops = 0;
  ...
  while (c != '\0') {
    ...
    if (c == 'b' || c == 'h' || c == 'i' || c == 'l' || c == 'I' || c == 'L' || c == 'q' || c == 'f' || c == 'd' ||
        c == 's' || c == '?' || c == 'B' || c == 'X') {
      switch (c) {
        case 'b':
...
static bool impl_unpack_buffer(
    lua_State *L,
    impl_pack_op_t *ops,
    size_t num_ops,
    bool is_little_endian,
...
static void impl_unpack_byte(lua_State *L, const uint8_t *buf, size_t *offset, size_t bytes_size, bool swap) {
...
static void impl_pack_byte(lua_State *L, uint8_t *buf, size_t *offset, int *stack_pos, size_t bytes_size, bool swap) {
