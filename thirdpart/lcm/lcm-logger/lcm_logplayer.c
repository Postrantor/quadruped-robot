...
void handler(const lcm_recv_buf_t *rbuf, const char *channel, void *u) {
  ...
  if (l->verbose) printf("%.3f Channel %-20s size %d\n", rbuf->recv_utime / 1000000.0, channel, rbuf->data_size);
...
int main(int argc, char **argv) {
  ...
  struct option long_opts[] = {
      {"help", no_argument, 0, 'h'},    {"speed", required_argument, 0, 's'},  {"lcm-url", required_argument, 0, 'l'},
      {"verbose", no_argument, 0, 'v'}, {"regexp", required_argument, 0, 'e'}, {0, 0, 0, 0},
  };
  ...
  if (!expression) expression = strdup(".*");
#ifndef WIN32
