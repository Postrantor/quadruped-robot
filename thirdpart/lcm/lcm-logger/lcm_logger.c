...
static inline int64_t timestamp_seconds(int64_t v) { return v / 1000000; }
...
static int open_logfile(logger_t *logger) {
  ...
  if (logger->use_strftime) {
    ...
    if (logger->auto_increment && strcmp(new_prefix, logger->fname_prefix)) logger->next_increment_num = 0;
  ...
  if (logger->auto_increment) {
    ...
    do {
      snprintf(logger->fname, sizeof(logger->fname), "%s.%02d", logger->fname_prefix, logger->next_increment_num);
...
static void *write_thread(void *user_data) {
  ...
  while (1) {
    ...
    if (split_log) {
      ...
      if (logger->rotate > 0) rotate_logfiles(logger);
      if (0 != open_logfile(logger)) exit(1);
      num_splits++;
      logger->logsize = 0;
    ...
    if (!logger->quiet && (offset_utime - logger->last_report_time > 1000000)) {
      ...
      printf(
          info_format_str, logger->fname, timestamp_seconds(offset_utime), logger->nevents, logger->logsize / 1048576,
          tps, kbps);
...
static void message_handler(const lcm_recv_buf_t *rbuf, const char *channel, void *u) {
  ...
  if (logger->invert_channels) {
    if (g_regex_match(logger->regex, channel, (GRegexMatchFlags)0, NULL)) return;
  }
  ...
  if (mem_required > logger->max_write_queue_size) {
    ...
    if (now - logger->last_drop_report_utime > 1000000 && rc > 0) {
      if (!logger->quiet) printf("Can't write to log fast enough.  Dropped %d packet%s\n", rc, rc == 1 ? "" : "s");
...
#ifdef USE_SIGHUP
static void sighup_handler(int signum) { _reset_logfile = 1; }
#endif
...
int main(int argc, char *argv[]) {
  ...
  if (0 != open_logfile(&logger)) return 1;
  ...
  for (void *msg = g_async_queue_try_pop(logger.write_queue); msg; msg = g_async_queue_try_pop(logger.write_queue)) {
    if (msg == &logger.write_thread_exit_flag) continue;
    free(msg);
  }
