...
static void on_example_msg(const lcm_recv_buf_t *rbuf, const char *channel, const exlcm_example_t *msg, void *user) {
  int i;
...
static gboolean on_lcm(GIOChannel *source, GIOCondition cond, void *user_data) {
  ...
  if (0 != lcm_handle(state->lcm)) g_main_loop_quit(state->mainloop);
  return TRUE;
}
