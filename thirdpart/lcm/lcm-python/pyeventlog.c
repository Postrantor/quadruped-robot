...
static PyObject *pylog_read_next_event(PyLogObject *self) {
  ...
  PyObject *result = Py_BuildValue(
      "LLs#y#",  // build from bytes
      next_event->eventnum, next_event->timestamp, next_event->channel, next_event->channellen, next_event->data,
      next_event->datalen);
#else
  PyObject *result = Py_BuildValue(
      "LLs#s#", next_event->eventnum, next_event->timestamp, next_event->channel, next_event->channellen,
...
static PyObject *pylog_ftell(PyLogObject *self) { return PyLong_FromLongLong(ftello(self->eventlog->f)); }
...
static int pylog_initobj(PyObject *s, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|s", keywords, &filename, &mode)) return -1;
