...
public:
  ...
  bool Run(void) {
    ...
    for (int iter = 0; iter < num_trials_; iter++) {
      ...
      for (int i = 0; i < echo_msg_len_; i++) echo_data_[i] = rand() % 256;
...
private:
  void Handler(const lcm::ReceiveBuffer *rbuf, const std::string &channel) {
    if (rbuf->data_size != echo_msg_len_) return;
    if (memcmp(rbuf->data, echo_data_, rbuf->data_size)) return;
    response_count_++;
  }
...
public:
  TypedTest(const std::string test_name, int num_trials)
      : lcm_(), response_count_(0), num_trials_(num_trials), subscription_(), test_channel_(test_name) {}

  bool Run(void) {
    LcmType msg;
    lcm::Subscription *subscription = lcm_.subscribe(test_channel_ + "_reply", &TypedTest<LcmType>::Handler, this);
    bool result = true;
...
TEST(LCM_CPP, primitives_t) { EXPECT_TRUE(TypedTest<lcmtest::primitives_t>("test_lcmtest_primitives_t", 1000).Run()); }

TEST(LCM_CPP, primitives_list_t) {
  EXPECT_TRUE(TypedTest<lcmtest::primitives_list_t>("test_lcmtest_primitives_list_t", 100).Run());
}

TEST(LCM_CPP, node_t) { EXPECT_TRUE(TypedTest<lcmtest::node_t>("test_lcmtest_node_t", 7).Run()); }
...
class LambdaTest {
public:
  LambdaTest(const std::string test_name, int num_trials) : lcm_(), num_trials_(num_trials), test_channel_(test_name) {}

  bool Run(void) {
    LcmType msg;
    int response_count = 0;
    lcm::LCM::HandlerFunction<LcmType> handler =
        [&response_count](const lcm::ReceiveBuffer *rbuf, const std::string &channel, const LcmType *msg) {
...
TEST(LCM_CPP, Lambda_A) { EXPECT_TRUE(LambdaTest<lcmtest::primitives_t>("test_lcmtest_primitives_t", 1000).Run()); }

TEST(LCM_CPP, Lambda_B) {
  EXPECT_TRUE(LambdaTest<lcmtest::primitives_list_t>("test_lcmtest_primitives_list_t", 100).Run());
}

TEST(LCM_CPP, Lambda_C) { EXPECT_TRUE(LambdaTest<lcmtest::node_t>("test_lcmtest_node_t", 7).Run()); }
#endif
