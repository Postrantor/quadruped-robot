  ...
  for (i = 0; i < 2; i++) {
    for (k = 0; k < msg->size_c; k++) {
      ...
      if (strcmp(expected_buf, msg->strarray[i][k].c_str())) {
        info(
            "Expected msg->strarray[%d][%d] to be %s, got %s instead", i, k, expected_buf, msg->strarray[i][k].c_str());
        return 0;
      }
      n++;
    }
...
int CheckLcmType(const lcmtest::node_t *msg, int expected) {
  ...
  if (!msg->num_children) return 1;

  int i;
  for (i = 0; i < msg->num_children; i++) {
    const lcmtest::node_t *child = &msg->children[i];
    if (!CheckLcmType(child, msg->num_children - 1)) return 0;
  }
  return 1;
}
...
int CheckLcmType(const lcmtest::primitives_list_t *msg, int expected) {
  ...
  for (int n = 0; n < expected; n++) {
    ...
    int i;
    for (i = 0; i < n; i++) CHECK_FIELD(ex->ranges[i], -i, "%d");
...
void ClearLcmType(lcmtest::primitives_list_t *msg) { msg->items.clear(); }
...
int CheckLcmType(const lcmtest::primitives_t *msg, int expected) {
  ...
  int i;
  for (i = 0; i < n; i++) CHECK_FIELD(msg->ranges[i], i, "%d");
  char expected_name[100];
  sprintf(expected_name, "%d", n);
  if (strcmp(expected_name, msg->name.c_str())) {
    info("Expected msg->expected_name to be %s, got %s instead", expected_name, msg->name.c_str());
    return 0;
  }
...
void FillLcmType(int n, lcmtest2::another_type_t *msg) { msg->val = n; }

void ClearLcmType(lcmtest2::another_type_t *msg) { return; }
