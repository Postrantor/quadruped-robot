...
#define MAKE_CLIENT_TEST(type, num_iters)                                                                    \
  ...
  static int g_##type##_response_count = 0;                                                                  \
                                                                                                             \
  static void type##_handler(const lcm_recv_buf_t *rbuf, const char *channel, const type *msg, void *user) { \
  ...
  static int do_##type##_test(void) {                                                                        \
    type msg;                                                                                                \
    type##_subscription_t *subs = type##_subscribe(g_lcm, "test_" #type "_reply", type##_handler, NULL);     \
...
static void echo_handler(const lcm_recv_buf_t *rbuf, const char *channel, void *user) {
  if (rbuf->data_size != g_echo_msg_len) return;
  if (memcmp(rbuf->data, g_echo_data, rbuf->data_size)) return;
...
TEST(LCM_C, EchoTest) {
  ...
  for (iter = 0; iter < 100; iter++) {
    ...
    int i;
    for (i = 0; i < g_echo_msg_len; i++) g_echo_data[i] = rand() % 256;
