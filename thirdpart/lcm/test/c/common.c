...
int check_lcmtest_node_t(const lcmtest_node_t *msg, int expected) {
  ...
  for (i = 0; i < msg->num_children; i++) {
    ...
    if (!check_lcmtest_node_t(child, msg->num_children - 1)) return 0;
  }
  return 1;
}
...
void fill_lcmtest2_another_type_t(int n, lcmtest2_another_type_t *msg) { msg->val = n; }

void clear_lcmtest2_another_type_t(lcmtest2_another_type_t *msg) { return; }
