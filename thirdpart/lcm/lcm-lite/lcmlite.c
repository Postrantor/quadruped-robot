...
int lcmlite_receive_packet(lcmlite_t *lcm, const void *_buf, int buf_len, uint64_t from_addr) {
  ...
  if (magic == MAGIC_LCM2) {
    ...
    for (int idx = 0; idx < LCM3_NUM_BUFFERS; idx++) {
      if (lcm->fragment_buffers[idx].from_addr == from_addr && lcm->fragment_buffers[idx].msg_seq == msg_seq) {
    ...
    if (buf_pos < buf_len) memcpy(&fbuf->buf[fragment_offset], &buf[buf_pos], buf_len - buf_pos);
