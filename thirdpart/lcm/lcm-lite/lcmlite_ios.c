...
void lcmlite_impl_transmit_packet(const void *_buf, int buf_len, void *user) {
  ...
  ssize_t res =
      sendto(tinfo->send_fd, _buf, buf_len, 0, (struct sockaddr *)&tinfo->send_addr, sizeof(tinfo->send_addr));
  if (res < 0) perror("transmit_packet: sendto");
}
...
int lcmlite_impl_init(struct lcmlite_impl *impl) {
  ...
  if (inet_aton("239.255.76.67", (struct in_addr *)&mc_addr) < 0) return 1;
...
void lcmlite_impl_readloop(lcmlite_t *lcm, struct lcmlite_impl *impl) {
  ...
  while (1) {
    ...
    int res =
        lcmlite_receive_packet(lcm, buf, buf_len, from_addr.sin_addr.s_addr | ((uint64_t)from_addr.sin_port << 32));
