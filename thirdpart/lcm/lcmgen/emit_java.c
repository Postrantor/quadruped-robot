...
const char *make_fqn(lcmgen_t *lcm, const char *type_name) {
  if (strchr(type_name, '.') != NULL) return type_name;

  if (!jdefaultpkg_warned && !getopt_was_specified(lcm->gopt, "jdefaultpkg")) {
    printf(
        "Notice: enclosing LCM types without package into java namespace '%s'.\n",
...
static void make_accessor(lcm_member_t *lm, const char *obj, char *s) {
  ...
  for (int d = 0; d < ndim; d++) pos += sprintf(&s[pos], "[%c]", 'a' + d);
}
...
static void make_accessor_array(lcm_member_t *lm, const char *obj, char *s) {
  ...
  pos += sprintf(s, "%s%s%s", obj, obj[0] == 0 ? "" : ".", lm->membername);
  for (int d = 0; d < ndim - 1; d++) pos += sprintf(&s[pos], "[%c]", 'a' + d);
}
...
static int struct_has_string_member(lcm_struct_t *lr) {
  for (unsigned int member = 0; member < g_ptr_array_size(lr->members); member++) {
    ...
    if (!strcmp("string", lm->type->lctypename)) return 1;
  }
  return 0;
}
...
void encode_recursive(lcmgen_t *lcm, lcm_member_t *lm, FILE *f, primitive_info_t *pinfo, char *accessor, int depth) {
  ...
  if (depth + 1 == g_ptr_array_size(lm->dimensions) && pinfo != NULL) {
    ...
    if (0 && !strcmp(pinfo->storage, "float")) {
      ...
      emit(2 + depth, "{ ByteBuffer bbuf = ByteBuffer.allocate(%s%s*4);", dim_size_prefix(dim->size), dim->size);
  ...
  emit(
      2 + depth, "for (int %c = 0; %c < %s%s; %c++) {", 'a' + depth, 'a' + depth, dim_size_prefix(dim->size), dim->size,
      'a' + depth);
...
void decode_recursive(lcmgen_t *lcm, lcm_member_t *lm, FILE *f, primitive_info_t *pinfo, char *accessor, int depth) {
  ...
  if (depth + 1 == g_ptr_array_size(lm->dimensions) && pinfo != NULL) {
    ...
    if (0 && !strcmp(pinfo->storage, "float")) {
      ...
      emit(2 + depth, "{ byte bb[] = new byte[%s%s*4]; ins.readFully(bb);", dim_size_prefix(dim->size), dim->size);
...
void copy_recursive(lcmgen_t *lcm, lcm_member_t *lm, FILE *f, primitive_info_t *pinfo, char *accessor, int depth) {
  ...
  if (depth + 1 == g_ptr_array_size(lm->dimensions) && pinfo != NULL) {
    ...
    if (dim->mode == LCM_VAR) {
      emit(2 + depth, "if (this.%s > 0)", dim->size);
      emit_start(
          3 + depth, "System.arraycopy(this.%s, 0, outobj.%s, 0, %s%s);", accessor_array, accessor_array,
          dim_size_prefix(dim->size), dim->size);
    } else {
      emit_start(
          2 + depth, "System.arraycopy(this.%s, 0, outobj.%s, 0, %s%s);", accessor_array, accessor_array,
          dim_size_prefix(dim->size), dim->size);
    }
...
int emit_java(lcmgen_t *lcm) {
  ...
  g_hash_table_insert(type_table, "byte", prim("byte", "# = ins.readByte();", "outs.writeByte(#);"));
  g_hash_table_insert(type_table, "int8_t", prim("byte", "# = ins.readByte();", "outs.writeByte(#);"));
  g_hash_table_insert(type_table, "int16_t", prim("short", "# = ins.readShort();", "outs.writeShort(#);"));
  g_hash_table_insert(type_table, "int32_t", prim("int", "# = ins.readInt();", "outs.writeInt(#);"));
  g_hash_table_insert(type_table, "int64_t", prim("long", "# = ins.readLong();", "outs.writeLong(#);"));
  ...
  g_hash_table_insert(type_table, "boolean", prim("boolean", "# = ins.readByte()!=0;", "outs.writeByte( # ? 1 : 0);"));
  g_hash_table_insert(type_table, "float", prim("float", "# = ins.readFloat();", "outs.writeFloat(#);"));
  g_hash_table_insert(type_table, "double", prim("double", "# = ins.readDouble();", "outs.writeDouble(#);"));
  ...
  for (unsigned int en = 0; en < g_ptr_array_size(lcm->enums); en++) {
    ...
    char *path = g_strdup_printf(
        ...
        strlen(getopt_get_string(lcm->gopt, "jpath")) > 0 ? G_DIR_SEPARATOR_S : "", dots_to_slashes(classname));

    if (!lcm_needs_generation(lcm, le->lcmfile, path)) continue;

    if (getopt_get_bool(lcm->gopt, "jmkdir")) make_dirs_for_file(path);
  ...
  for (unsigned int st = 0; st < g_ptr_array_size(lcm->structs); st++) {
    ...
    if (!lcm_needs_generation(lcm, lr->lcmfile, path)) continue;
    ...
    if (0) {
      ...
      if (usenio) emit(0, "import java.nio.*;");
    }
    ...
    for (unsigned int member = 0; member < g_ptr_array_size(lr->members); member++) {
      ...
      for (unsigned int i = 0; i < g_ptr_array_size(lm->dimensions); i++) emit_continue("[]");
      emit_end(";");
    }
    emit(0, " ");
    ...
    for (unsigned int member = 0; member < g_ptr_array_size(lr->members); member++) {
      ...
      if (g_ptr_array_size(lm->dimensions) == 0 || !lcm_is_constant_size_array(lm)) continue;
    ...
    if (g_ptr_array_size(lr->constants) > 0) emit(0, "");
    ...
    if (struct_has_string_member(lr)) emit(2, "char[] __strbuf = null;");
    char accessor[1024];
