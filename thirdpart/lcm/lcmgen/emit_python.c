...
static void _emit_decode_list(
  ...
  if (!strcmp("byte", tn)) {
    ...
    if (fixed_len) {
      emit(
          indent, "%smap(bool, struct.unpack('>%s%c', buf.read(%d)))%s", accessor, len, _struct_format(lm),
          atoi(len) * _primitive_type_size(tn), suffix);
    } else {
      emit(
          indent, "%smap(bool, struct.unpack('>%%d%c' %% self.%s, buf.read(self.%s)))%s", accessor, _struct_format(lm),
          len, len, suffix);
    }
  } else if (
      !strcmp("int8_t", tn) || !strcmp("int16_t", tn) || !strcmp("int32_t", tn) || !strcmp("int64_t", tn) ||
      !strcmp("float", tn) || !strcmp("double", tn)) {
    if (fixed_len) {
      emit(
          indent, "%sstruct.unpack('>%s%c', buf.read(%d))%s", accessor, len, _struct_format(lm),
          atoi(len) * _primitive_type_size(tn), suffix);
    } else {
      if (_primitive_type_size(tn) > 1) {
        emit(
            indent, "%sstruct.unpack('>%%d%c' %% self.%s, buf.read(self.%s * %d))%s", accessor, _struct_format(lm), len,
            len, _primitive_type_size(tn), suffix);
      } else {
        emit(
            indent, "%sstruct.unpack('>%%d%c' %% self.%s, buf.read(self.%s))%s", accessor, _struct_format(lm), len, len,
            suffix);
      }
    }
  } else {
  ...
  for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
    ...
    if (!lm->dimensions->len) {
      ...
      if (lcm_is_primitive_type(lm->type->lctypename) && 0 != strcmp(lm->type->lctypename, "string")) {
        ...
        _emit_decode_list(lcm, f, ls, lm, accessor->str, 2 + n, n == 0, last_dim->size, last_dim_fixed_len);
      } else {
...
static void _emit_encode_list(
  ...
  if (!strcmp("byte", tn)) {
    ...
    return;
      ...
      !strcmp("boolean", tn) || !strcmp("int8_t", tn) || !strcmp("int16_t", tn) || !strcmp("int32_t", tn) ||
    ...
    if (fixed_len) {
      ...
      emit(
          indent, "buf.write(struct.pack('>%%d%c' %% self.%s, *%s[:self.%s]))", _struct_format(lm), len, accessor, len);
    }
  } else {
    assert(0);
  }
...
static void emit_python_encode_one(const lcmgen_t *lcm, FILE *f, lcm_struct_t *ls) {
  ...
  for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
    ...
    if (!lm->dimensions->len) {
      ...
      if (lcm_is_primitive_type(lm->type->lctypename) && 0 != strcmp(lm->type->lctypename, "string")) {
        _emit_encode_list(lcm, f, ls, lm, accessor->str, 2 + n, last_dim->size, last_dim_fixed_len);
      } else {
        if (last_dim_fixed_len) {
          emit(2 + n, "for i%d in range(%s):", n, last_dim->size);
        } else {
