...
static void emit_header_start(lcmgen_t *lcmgen, FILE *f, lcm_struct_t *ls) {
  ...
  for (unsigned int mind = 0; mind < g_ptr_array_size(ls->members); mind++) {
    ...
    if (g_ptr_array_size(lm->dimensions) != 0 && !lcm_is_constant_size_array(lm) && !emit_include_vector) {
  ...
  for (unsigned int mind = 0; mind < g_ptr_array_size(ls->members); mind++) {
    ...
    if (!lcm_is_primitive_type(lm->type->lctypename) && strcmp(lm->type->lctypename, ls->structname->lctypename)) {
      ...
      emit(
          ...
          strlen(getopt_get_string(lcmgen->gopt, "cpp-include")) > 0 ? G_DIR_SEPARATOR_S : "", other_tn);
      free(other_tn);
    }
  }
    ...
    for (unsigned int mind = 0; mind < g_ptr_array_size(ls->members); mind++) {
      ...
      if (ndim == 0) {
        ...
        if (lcm_is_constant_size_array(lm)) {
          ...
          emit_start(2, "");
          for (unsigned int d = 0; d < ndim; d++) emit_continue("std::vector< ");
...
static void _encode_recursive(lcmgen_t *lcm, FILE *f, lcm_member_t *lm, int depth, int extra_indent) {
  ...
  // primitive array
  if (depth + 1 == g_ptr_array_size(lm->dimensions) && lcm_is_primitive_type(lm->type->lctypename) &&
      strcmp(lm->type->lctypename, "string")) {
    lcm_dimension_t *dim = (lcm_dimension_t *)g_ptr_array_index(lm->dimensions, depth);
    emit_start(
        indent, "tlen = __%s_encode_array(buf, offset + pos, maxlen - pos, &this->%s", lm->type->lctypename,
        lm->membername);
    ...
    if (0 == num_dims) {
      if (lcm_is_primitive_type(lm->type->lctypename)) {
        if (!strcmp(lm->type->lctypename, "string")) {
          ...
          emit(
              1, "tlen = __%s_encode_array(buf, offset + pos, maxlen - pos, &this->%s, 1);", lm->type->lctypename,
              lm->membername);
        }
        emit(1, "if(tlen < 0) return tlen; else pos += tlen;");
      } else {
...
static void _decode_recursive(lcmgen_t *lcm, FILE *f, lcm_member_t *lm, int depth) {
  ...
  if (depth + 1 == g_ptr_array_size(lm->dimensions) && lcm_is_primitive_type(lm->type->lctypename) &&
    ...
    emit_start(
        decode_indent, "tlen = __%s_decode_array(buf, offset + pos, maxlen - pos, &this->%s", lm->type->lctypename,
        lm->membername);
    for (int i = 0; i < depth; i++) emit_continue("[a%d]", i);
    emit_end("[0], %s%s);", dim_size_prefix(dim->size), dim->size);
    ...
    emit(
        1 + depth, "for (int a%d = 0; a%d < %s%s; a%d++) {", depth, depth, dim_size_prefix(dim->size), dim->size,
        depth);
  ...
  for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
    ...
    if (0 == g_ptr_array_size(lm->dimensions) && lcm_is_primitive_type(lm->type->lctypename)) {
      if (!strcmp(lm->type->lctypename, "string")) {
        ...
        emit(
            1, "tlen = __%s_decode_array(buf, offset + pos, maxlen - pos, &this->%s, 1);", lm->type->lctypename,
            lm->membername);
        emit(1, "if(tlen < 0) return tlen; else pos += tlen;");
      }
    } else {
