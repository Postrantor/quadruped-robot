...
static void _emit_decode_one(
    const lcmgen_t *lcm, FILE *f, lcm_struct_t *ls, lcm_member_t *lm, const char *accessor, int indent) {
...
static void _emit_decode_list(
    ...
    int fixed_len) {
  const char *tn = lm->type->lctypename;
  if (!strcmp("byte", tn) || !strcmp("int8_t", tn) || !strcmp("boolean", tn) || !strcmp("int16_t", tn) ||
      !strcmp("int32_t", tn) || !strcmp("int64_t", tn) || !strcmp("float", tn) || !strcmp("double", tn)) {
    if (fixed_len) {
      emit(
          indent, "%s = {lcm._pack.unpack('>%s%c', data:read(%d))}", accessor, len, _struct_format(lm),
      ...
      if (_primitive_type_size(tn) > 1) {
        ...
        emit(
            indent, "%s = {lcm._pack.unpack(string.format('>%%d%c', obj.%s), data:read(obj.%s))}", accessor,
            _struct_format(lm), len, len);
      }
    }
  } else {
    assert(0);
...
static void emit_lua_decode_one(const lcmgen_t *lcm, FILE *f, lcm_struct_t *ls) {
  ...
  for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
    ...
    if (!lm->dimensions->len) {
      ...
      // last dimension.
      lcm_dimension_t *last_dim = (lcm_dimension_t *)g_ptr_array_index(lm->dimensions, lm->dimensions->len - 1);
      int last_dim_fixed_len = last_dim->mode == LCM_CONST;

      if (lcm_is_primitive_type(lm->type->lctypename) && 0 != strcmp(lm->type->lctypename, "string")) {
        // member is a primitive non-string type.  Emit code to
        // decode a full array in one call to struct.unpack

        _emit_decode_list(lcm, f, ls, lm, accessor->str, 1 + n, last_dim->size, last_dim_fixed_len);
      } else {
...
static void _emit_encode_list(
    ...
    int fixed_len) {
  const char *tn = lm->type->lctypename;
  if (!strcmp("byte", tn) || !strcmp("boolean", tn) || !strcmp("int8_t", tn) || !strcmp("int16_t", tn) ||
      !strcmp("int32_t", tn) || !strcmp("int64_t", tn) || !strcmp("float", tn) || !strcmp("double", tn)) {
    if (fixed_len) {
      emit(indent, "table.insert(buf_table, lcm._pack.pack('>%s%c', unpack(%s)))", len, _struct_format(lm), accessor);
    } else {
      emit(
          indent,
...
static void emit_lua_encode_one(const lcmgen_t *lcm, FILE *f, lcm_struct_t *ls) {
  ...
  for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
    ...
    if (!lm->dimensions->len) {
      ...
      if (lcm_is_primitive_type(lm->type->lctypename) && 0 != strcmp(lm->type->lctypename, "string")) {
        _emit_encode_list(lcm, f, ls, lm, accessor->str, 1 + n, last_dim->size, last_dim_fixed_len);
      } else {
        if (last_dim_fixed_len) {
          emit(1 + n, "for i%d = 1, %s do", n, last_dim->size);
        } else {
...
static int emit_package(lcmgen_t *lcm, _package_contents_t *pc) {
  ...
  sprintf(
      package_dir_prefix, "%s%s", getopt_get_string(lcm->gopt, "lpath"),
      strlen(getopt_get_string(lcm->gopt, "lpath")) > 0 ? G_DIR_SEPARATOR_S : "");
  sprintf(package_dir, "%s%s%s", package_dir_prefix, pdname, have_package ? G_DIR_SEPARATOR_S : "");
  free(pdname);
      ...
      if (g_file_test(initlua_fname, G_FILE_TEST_EXISTS)) {
        ...
        while (!feof(init_lua_fp)) {
          ...
          GRegex *regex = g_regex_new(
              "require\\('([\\w+\\.]*\\.)(\\w+)'\\)( -- subpackage)?", (GRegexCompileFlags)0, (GRegexMatchFlags)0,
              NULL);
  ...
  for (int i = 0; i < pc->structs->len; i++) {
    ...
    if (!lcm_needs_generation(lcm, ls->lcmfile, path)) continue;
