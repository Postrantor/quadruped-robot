...
int lcm_is_primitive_type(const char *t) { return string_in_array(t, primitive_types); }

int lcm_is_array_dimension_type(const char *t) { return string_in_array(t, array_dimension_types); }

int lcm_is_legal_member_name(const char *t) { return isalpha(t[0]) || t[0] == '_'; }

int lcm_is_legal_const_type(const char *t) { return string_in_array(t, const_types); }
...
lcm_typename_t *lcm_typename_create(lcmgen_t *lcmgen, const char *lctypename) {
  ...
  if (rtmp == NULL) {
    ...
    if (lcm_is_primitive_type(lt->shortname)) {
      ...
      lt->lctypename = g_strdup_printf("%s%s%s", lt->package, strlen(lcmgen->package) > 0 ? "." : "", lt->shortname);
    }
  } else {
    lt->package = tmp;
    *rtmp = 0;
...
int64_t lcm_struct_hash(lcm_struct_t *lr) {
  ...
  for (unsigned int i = 0; i < g_ptr_array_size(lr->members); i++) {
    ...
    if (lcm_is_primitive_type(lm->type->lctypename)) v = hash_string_update(v, lm->type->lctypename);
...
int parse_const(lcmgen_t *lcmgen, lcm_struct_t *lr, tokenize_t *t) {
  ...
  if (!lcm_is_legal_const_type(t->token)) parse_error(t, "invalid type for const");
...
another_constant:
  ...
  if (!lcm_is_legal_member_name(t->token)) parse_error(t, "Invalid member name: must start with [a-zA-Z_].");
  char *membername = strdup(t->token);

  // make sure this name isn't already taken.
  if (lcm_find_member(lr, t->token) != NULL) semantic_error(t, "Duplicate member name '%s'.", t->token);
  if (lcm_find_const(lr, t->token) != NULL) semantic_error(t, "Duplicate member name '%s'.", t->token);
...
int parse_entity(lcmgen_t *lcmgen, const char *lcmfile, tokenize_t *t) {
  ...
  if (!strcmp(t->token, "struct")) {
    ...
    const lcm_struct_t *prior = find_struct(lcmgen, lr->structname->package, lr->structname->shortname);
    if (prior) {
...
int lcm_needs_generation(lcmgen_t *lcmgen, const char *declaringfile, const char *outfile) {
  ...
  if (!getopt_get_bool(lcmgen->gopt, "lazy")) return 1;
