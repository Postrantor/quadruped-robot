...
const char *make_fqn_csharp(lcmgen_t *lcm, const char *type_name) {
  ...
  if (!ndefaultpkg_warned && !getopt_was_specified(lcm->gopt, "csharp-default-nsp")) {
    printf(
        "Notice: enclosing LCM types without package into C#.NET namespace '%s'.\n",
  ...
  if (strlen(def_nsp) > 0)
    return g_strdup_printf(
        "%s%s%s.%s", root_nsp, (root_nsp[0] == 0 ? "" : "."), getopt_get_string(lcm->gopt, "csharp-default-nsp"),
        type_name);
  else
...
int emit_csharp(lcmgen_t *lcm) {
  ...
  g_hash_table_insert(type_table, "int8_t", prim("byte", "# = ins.ReadByte();", "outs.Write(#);"));
  g_hash_table_insert(type_table, "int16_t", prim("short", "# = ins.ReadInt16();", "outs.Write(#);"));
  g_hash_table_insert(type_table, "int32_t", prim("int", "# = ins.ReadInt32();", "outs.Write(#);"));
  g_hash_table_insert(type_table, "int64_t", prim("long", "# = ins.ReadInt64();", "outs.Write(#);"));
  g_hash_table_insert(
      type_table, "string",
      prim(
          "String",
          ...
          "outs.Write((byte) 0);"));
  g_hash_table_insert(type_table, "boolean", prim("bool", "# = ins.ReadBoolean();", "outs.Write(#);"));
  g_hash_table_insert(type_table, "float", prim("float", "# = ins.ReadSingle();", "outs.Write(#);"));
  g_hash_table_insert(type_table, "double", prim("double", "# = ins.ReadDouble();", "outs.Write(#);"));
  ...
  for (unsigned int en = 0; en < g_ptr_array_size(lcm->enums); en++) {
    ...
    if (getopt_get_bool(lcm->gopt, "csharp-mkdir")) make_dirs_for_file(path);
  ...
  for (unsigned int st = 0; st < g_ptr_array_size(lcm->structs); st++) {
    ...
    for (unsigned int member = 0; member < g_ptr_array_size(lr->members); member++) {
      ...
      if (g_ptr_array_size(lm->dimensions) > 0) {
        emit_continue("[");
        for (unsigned int i = 0; i < (g_ptr_array_size(lm->dimensions) - 1); i++) emit_continue(",");
        emit_continue("]");
      }
    ...
    for (unsigned int member = 0; member < g_ptr_array_size(lr->members); member++) {
      ...
      for (unsigned int i = 0; i < g_ptr_array_size(lm->dimensions); i++) {
        ...
        if (i < (g_ptr_array_size(lm->dimensions) - 1)) emit_continue(",");
      }
      emit_end("];");
    }
    emit(2, "}");
    ...
    if (struct_has_string_member(lr)) emit(3, "byte[] __strbuf = null;");
    char accessor[1024];
    ...
    for (unsigned int member = 0; member < g_ptr_array_size(lr->members); member++) {
      ...
      for (unsigned int i = 0; i < g_ptr_array_size(lm->dimensions); i++) {
        lcm_dimension_t *dim = (lcm_dimension_t *)g_ptr_array_index(lm->dimensions, i);
        emit(
            3 + i, "for (int %c = 0; %c < %s%s; %c++) {", 'a' + i, 'a' + i, dim_size_prefix(dim->size), dim->size,
            'a' + i);
      }
