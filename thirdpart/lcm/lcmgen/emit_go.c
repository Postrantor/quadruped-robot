 ...
 */
unsigned int lcm_find_member_with_named_dimension(lcm_struct_t *ls, const char *const name, unsigned int start) {
...
uint64_t __lcm_recursive_fingerprint(lcmgen_t *lcm, lcm_struct_t *ls, const struct __fingerprints *fs) {
  ...
  for (unsigned int m = 0; m < ls->members->len; m++) {
    ...
    if (!lcm_is_primitive_type(lm->type->lctypename)) {
      ...
      if (ls_ != NULL) {
        ...
        fprintf(
            stderr, "Unable to locate fingerprint for member '%s' of '%s'\n", lm->membername,
...
uint64_t lcm_get_fingerprint(lcmgen_t *lcm, lcm_struct_t *ls) { return __lcm_recursive_fingerprint(lcm, ls, NULL); }
 ...
 * Starts at given index.
 */
unsigned int lcm_find_named_dimension(
    FILE *f, lcm_struct_t *ls, lcm_member_t *lm, const char *const name, unsigned int start) {
 ...
 * CAUTION: memory has to be freed manually.
 */
static char *go_typename(
    const char *const package, const char *const typepackage, const char *const type, const int64_t fingerprint) {
 ...
 */
char *go_filename(
    lcmgen_t *lcm, const char *const dir, const char *const name, uint64_t fingerprint, const char *const suffix) {
  if (fingerprint != 0)
 ...
 */
static void emit_go_slice_make(
    FILE *f,
    int indent,
 ...
 */
static unsigned int emit_go_array_loops(
    FILE *f,
    lcmgen_t *lcm,
    lcm_struct_t *ls,
    lcm_member_t *lm,
  ...
  for (n = 0; n < end; n++) {
    ...
    if (slice_emit) g_string_assign(slicestr, arraystr->str);

    g_string_append_printf(arraystr, "[i%d]", n);

    if (dim->mode == LCM_VAR) {
      char *size = go_membername(ls, dim->size, FALSE);
      const char *type = map_builtintype_name(lcm_find_member(ls, dim->size)->type->lctypename);

      if (slice_emit) emit_go_slice_make(f, n + 1, ls->structname->package, lm, n, slicestr->str, size, fingerprint);
...
static void emit_go_lcm_imports(FILE *f, lcmgen_t *lcm, lcm_struct_t *ls) {
  ...
  for (int i = 0; i < ls->members->len; i++) {
    ...
    if (lcm_is_primitive_type(lm->type->lctypename)) continue;

    int imported = FALSE;
    for (int j = i - 1; j > 0; j--) {
      lcm_member_t *lm_ = (lcm_member_t *)g_ptr_array_index(ls->members, j);

      if (strcmp(lm->type->package, lm_->type->package) == 0) imported = TRUE;
    }
 ...
 */
static void emit_go_lcm_const_definitions(
    FILE *f, lcmgen_t *lcm, lcm_struct_t *ls, const char *const gotype, const uint64_t fingerprint) {
  if (!ls->constants->len) {
    return;
  }
  ...
  for (unsigned int i = 0; i < ls->constants->len; i++) {
    ...
    if (strlen(constname) > mlen) mlen = strlen(constname);

    if (strlen(consttype) > tlen) tlen = strlen(consttype);
  ...
  for (unsigned int i = 0; i < ls->constants->len; i++) {
    ...
    emit(1, "%s_%s%*s%s%*s= %s", structtype, constname, mspaces, "", consttype, tspaces, "", lc->val_str);
 ...
 */
static int emit_go_lcm_struct_definition(
    FILE *f, lcmgen_t *lcm, lcm_struct_t *ls, const char *const gotype, const uint64_t fingerprint) {
  ...
  for (unsigned int i = 0; i < ls->members->len; i++) {
    ...
    if (strlen(membername) > max_member_len) max_member_len = strlen(membername);
    free(membername);
    ...
    if (lcm_is_primitive_type(lm->type->lctypename)) {
      ...
      membertype = go_typename(ls->structname->package, lm->type->package, lm->type->lctypename, lm_fingerprint);
    }
    ...
    if (membertype_len > max_type_len) max_type_len = membertype_len;
    free(membertype);
  ...
  for (unsigned int i = 0; i < ls->members->len; i++) {
    ...
    if (lcm_is_primitive_type(lm->type->lctypename)) {
      ...
      membertype = go_typename(ls->structname->package, lm->type->package, lm->type->lctypename, member_fp);
    }
    ...
    emit(1, "%s%*s%s%s%*s`%s`", membername, type_spaces, "", arraystr->str, membertype, tag_spaces, "", gotag->str);
 ...
 */
static void emit_go_lcm_deep_copy(
    FILE *f, lcmgen_t *lcm, lcm_struct_t *ls, const char *const gotype, const uint64_t fingerprint) {
  ...
  for (unsigned int m = 0; m < ls->members->len; m++) {
    ...
    if (lcm_is_primitive_type(lm->type->lctypename)) {
      if (!lm->dimensions->len) {
        ...
        unsigned int n = emit_go_array_loops(f, lcm, ls, lm, arraystr, TRUE, lm->dimensions->len, fingerprint);
 ...
 */
static void emit_go_lcm_read_only_getters(
    FILE *f, lcmgen_t *lcm, lcm_struct_t *ls, const char *const gotype, const uint64_t fingerprint) {
  for (unsigned int k = 0; k < ls->members->len; k++) {
    ...
    char *type = go_typename(ls->structname->package, lm->type->package, lm->type->lctypename, fingerprint);
    ...
    for (; i < ls->members->len; i = lcm_find_member_with_named_dimension(ls, lm->membername, i + 1)) {
      ...
      for (; j < lm_->dimensions->len; j = lcm_find_named_dimension(f, ls, lm_, lm->membername, j + 1)) {
        GString *arraystr = g_string_new(NULL);
        unsigned int n = emit_go_array_loops(f, lcm, ls, lm_, arraystr, FALSE, j, fingerprint);
        ...
        if (iteration == 0) {
          ...
          if (n == 0) {
            emit(
                j + 1, "p.%s = %s(len(p.%s%s))", structname, map_builtintype_name(lm->type->lctypename), membername,
                arraystr->str);
          } else {
            emit(
                j + 2, "p.%s = %s(len(p.%s%s))", structname, map_builtintype_name(lm->type->lctypename), membername,
                arraystr->str);
          }
        }
        if (iteration == 1 || n != 0) {
        ...
        if (iteration != 0 || n != 0) {
          ...
          emit(j + 3, "\"matching actual array size (got %%d expected %%d for %s%s)\",", membername, arraystr->str);
 ...
 * that can be send.
 */
static void emit_go_lcm_marshal_binary(
    FILE *f, lcmgen_t *lcm, lcm_struct_t *ls, const char *const gotype, const uint64_t fingerprint) {
  ...
  for (unsigned int i = 0; i < ls->members->len; i++) {
    ...
    if (lcm_is_primitive_type(lm->type->lctypename)) {
      if (!lm->dimensions->len) {
        ...
        GString *arraystr = g_string_new(membername);
        unsigned int n = emit_go_array_loops(f, lcm, ls, lm, arraystr, FALSE, lm->dimensions->len, fingerprint);
      ...
      if (!lm->dimensions->len) {
        ...
        unsigned int n = emit_go_array_loops(f, lcm, ls, lm, arraystr, FALSE, lm->dimensions->len, fingerprint);
        emit(2, "var tmp []byte");
        emit(2, "if tmp, err = p.%s%s.MarshalBinary(); err != nil {", membername, arraystr->str);
        emit(3, "return");
        emit(2, "}");
        emit(2, "offset += copy(data[offset:], tmp)");
        emit_go_array_loops_end(f, n);
 ...
 * members.
 */
static void emit_go_lcm_unmarshal_binary(
    FILE *f, lcmgen_t *lcm, lcm_struct_t *ls, const char *const gotype, const uint64_t fingerprint) {
  int readVar = 0;
  ...
  for (unsigned int m = 0; m < ls->members->len; m++) {
    ...
    if (lcm_is_primitive_type(lm->type->lctypename)) {
      ...
      if (!lm->dimensions->len) {
        ...
        GString *arraystr = g_string_new(membername);
        unsigned int n = emit_go_array_loops(f, lcm, ls, lm, arraystr, TRUE, lm->dimensions->len, fingerprint);
        emit(n + 1, "if err = p.%s.UnmarshalBinary(data[offset:]); err != nil {", arraystr->str);
        emit(n + 2, "return");
        emit(n + 1, "}");
 ...
 *     https://lcm-proj.github.io/type_specification.html
 */
static void emit_go_lcm_fingerprint(
    FILE *f,
    lcmgen_t *lcm,
    lcm_struct_t *ls,
  ...
  for (unsigned int m = 0; m < ls->members->len; m++) {
    ...
    if (!lcm_is_primitive_type(lm->type->lctypename)) {
      emit_end("+");
      char *lm_gotype = go_typename(ls->structname->package, lm->type->package, lm->type->lctypename, fingerprint);
 ...
 */
static void emit_go_lcm_string_size(
    FILE *f,
    int indent,
    const char *const str_prefix,
    const char *const str_name,
    const char *const str_postfix,
    ...
    if (lcm_is_primitive_type(lm->type->lctypename)) {
      if (!lm->dimensions->len) {
        if (strcmp(lm->type->lctypename, "string") == 0) {
          ...
          emit(1, "size += %d // p.%s", primitive_type_size(lm->type->lctypename), membername);
        }
      } else {
        GString *arraystr = g_string_new(NULL);
        unsigned int n = emit_go_array_loops(f, lcm, ls, lm, arraystr, FALSE, lm->dimensions->len, fingerprint);
        ...
        if (strcmp(lm->type->lctypename, "string") == 0) {
          ...
          emit(n + 1, "size += %d // p.%s", primitive_type_size(lm->type->lctypename), membername);
        }
...
int emit_go_lcm(lcmgen_t *lcm, lcm_struct_t *ls, const char *const dir, int64_t fingerprint) {
  ...
  if (!lcm_needs_generation(lcm, ls->lcmfile, path)) return 0;
...
int emit_go_gopacket(lcmgen_t *lcm, lcm_struct_t *ls, const char *const dir, int64_t fingerprint) {
  ...
  emit(0, "func (lcm *%s) DecodeFromBytes(data []byte, df gopacket.DecodeFeedback) error {", gotype);
...
int emit_go(lcmgen_t *lcm) {
  ...
  for (unsigned int i = 0; i < lcm->structs->len; i++) {
    ...
    if (getopt_get_bool(lcm->gopt, "go-fingerprint")) {
      ...
      if (fingerprint == 0) {
        fprintf(
            stderr, "Unable to calculate fingerprint as requested (--go-fingerprint) for '%s'.\n",
            ls->structname->shortname);
        res = -1;
        goto ret;
      }
    ...
    if (getopt_get_bool(lcm->gopt, "go-emit-gopacket") && emit_go_gopacket(lcm, ls, dir->str, fingerprint)) {
