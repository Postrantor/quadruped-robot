...
lcm_eventlog_event_t *lcm_eventlog_read_next_event(lcm_eventlog_t *l) {
  ...
  if (0 != fread64(l->f, &le->eventnum) || 0 != fread64(l->f, &le->timestamp) || 0 != fread32(l->f, &le->channellen) ||
...
int lcm_eventlog_write_event(lcm_eventlog_t *l, lcm_eventlog_event_t *le) {
  ...
  if (le->channellen != fwrite(le->channel, 1, le->channellen, l->f)) return -1;
  if (le->datalen != fwrite(le->data, 1, le->datalen, l->f)) return -1;
...
int lcm_eventlog_seek_to_timestamp(lcm_eventlog_t *l, int64_t timestamp) {
  ...
  while (1) {
    ...
    if ((frac > frac2) || (frac < frac1) || (frac1 >= frac2)) break;
