...
int Subscription::getQueueSize() const { return lcm_subscription_get_queue_size(c_subs); }
...
private:
  ...
  void (*handler)(const ReceiveBuffer *rbuf, const std::string &channel, const MessageType *msg, ContextClass context);
...
private:
  ...
  void (MessageHandlerClass::*handlerMethod)(
      const ReceiveBuffer *rbuf, const std::string &channel, const MessageType *msg);
  static void cb_func(const lcm_recv_buf_t *rbuf, const char *channel, void *user_data) {
...
private:
  ...
  static void cb_func(const lcm_recv_buf_t *rbuf, const char *channel, void *user_data) {
    LCMLambdaSubscription<MessageType> *subs = static_cast<LCMLambdaSubscription<MessageType> *>(user_data);
    MessageType msg;
    int status = msg.decode(rbuf->data, 0, rbuf->data_size);
    if (status < 0) {
      fprintf(stderr, "error %d decoding %s!!!\n", status, MessageType::getTypeName());
...
inline LCM::LCM(std::string lcm_url) : owns_lcm(true) { this->lcm = lcm_create(lcm_url.c_str()); }

inline LCM::LCM(lcm_t *lcm_in) : owns_lcm(false) { this->lcm = lcm_in; }

inline bool LCM::good() const { return this->lcm != NULL; }
...
Subscription *LCM::subscribe(
  ...
  subs->c_subs =
      lcm_subscribe(this->lcm, channel.c_str(), LCMMHSubscription<MessageType, MessageHandlerClass>::cb_func, subs);
...
Subscription *LCM::subscribe(
  ...
  subs->handlerMethod = handlerMethod;
  subs->c_subs =
      lcm_subscribe(this->lcm, channel.c_str(), LCMMHUntypedSubscription<MessageHandlerClass>::cb_func, subs);
  subscriptions.push_back(subs);
  return subs;
}

template <class MessageType, class ContextClass>
Subscription *LCM::subscribeFunction(
    const std::string &channel,
    void (*handler)(
        const ReceiveBuffer *rbuf, const std::string &channel, const MessageType *msg, ContextClass context),
    ContextClass context) {
  if (!this->lcm) {
...
Subscription *LCM::subscribe(const std::string &channel, LCM::HandlerFunction<MessageType> handler) {
  ...
  subs->handler = handler;
  subs->c_subs = lcm_subscribe(this->lcm, channel.c_str(), LCMLambdaSubscription<MessageType>::cb_func, subs);
  subscriptions.push_back(subs);
  return subs;
}
#endif

lcm_t *LCM::getUnderlyingLCM() { return this->lcm; }
...
LogFile::~LogFile() {
  if (eventlog) lcm_eventlog_destroy(eventlog);
  eventlog = NULL;
  if (last_event) lcm_eventlog_free_event(last_event);
  last_event = NULL;
}

bool LogFile::good() const { return eventlog != NULL; }
...
int LogFile::seekToTimestamp(int64_t timestamp) { return lcm_eventlog_seek_to_timestamp(eventlog, timestamp); }
...
FILE *LogFile::getFilePtr() { return eventlog->f; }
