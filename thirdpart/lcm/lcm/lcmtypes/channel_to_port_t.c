...
int64_t __channel_to_port_t_hash_recursive(const __lcm_hash_ptr *p) {
  ...
  for (fp = p; fp != NULL; fp = fp->parent)
    if (fp->v == __channel_to_port_t_get_hash) return 0;
  ...
  int64_t hash = 0x11dde9fa42a43913LL + __string_hash_recursive(&cp) + __int16_t_hash_recursive(&cp);
...
int channel_to_port_t_decode_cleanup(channel_to_port_t *p) { return __channel_to_port_t_decode_array_cleanup(p, 1); }
