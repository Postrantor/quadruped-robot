...
static int _recv_uint32(int fd, uint32_t *result) {
  uint32_t v;
  if (_recv_fully(fd, &v, 4) != 4) return -1;
  *result = ntohl(v);
  return 0;
}
...
static void lcm_tcpq_destroy(lcm_tcpq_t *self) {
  ...
  if (self->socket >= 0) _close_socket(self->socket);
  if (self->server_addr_str) g_free(self->server_addr_str);
...
static int _connect_to_server(lcm_tcpq_t *self) {
  ...
  if (self->socket) _close_socket(self->socket);
...
static int lcm_tcpq_get_fileno(lcm_tcpq_t *self) { return self->socket; }
...
static int lcm_tcpq_handle(lcm_tcpq_t *self) {
  ...
  uint32_t msg_type;
  if (_recv_uint32(self->socket, &msg_type)) goto disconnected;

  // read channel length, channel
  uint32_t channel_len;
  if (_recv_uint32(self->socket, &channel_len)) goto disconnected;
  if (_ensure_buf_capacity((void **)&self->recv_channel_buf, &self->recv_channel_buf_len, channel_len + 1)) {
    fprintf(stderr, "Memory allocation error\n");
    return -1;
  }
  if (channel_len != _recv_fully(self->socket, self->recv_channel_buf, channel_len)) goto disconnected;
  self->recv_channel_buf[channel_len] = 0;

  // read payload size, payload
  uint32_t data_len;
  if (_recv_uint32(self->socket, &data_len)) goto disconnected;
  if (_ensure_buf_capacity(&self->data_buf, &self->data_buf_len, data_len)) {
    fprintf(stderr, "Memory allocation error\n");
    return -1;
  }
  if (data_len != _recv_fully(self->socket, self->data_buf, data_len)) goto disconnected;
...
static int lcm_tcpq_publish(lcm_tcpq_t *self, const char *channel, const void *data, unsigned int datalen) {
  if (self->socket < 0 && 0 != _connect_to_server(self)) {
    return -1;
  }
  ...
  if (_send_uint32(self->socket, MESSAGE_TYPE_PUBLISH) || _send_uint32(self->socket, channel_len) ||
      (channel_len != _send_fully(self->socket, channel, channel_len)) || _send_uint32(self->socket, datalen) ||
