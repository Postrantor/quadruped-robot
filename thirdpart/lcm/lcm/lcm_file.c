...
static void lcm_logprov_destroy(lcm_logprov_t *lr) {
  ...
  if (lr->event) lcm_eventlog_free_event(lr->event);
  if (lr->log) lcm_eventlog_destroy(lr->log);
...
static void new_argument(gpointer key, gpointer value, gpointer user) {
  ...
  if (!strcmp((char *)key, "speed")) {
    ...
    if (endptr == value) fprintf(stderr, "Warning: Invalid value for speed\n");
  } else if (!strcmp((char *)key, "start_timestamp")) {
    char *endptr = NULL;
    lr->start_timestamp = strtoll((char *)value, &endptr, 10);
    if (endptr == value) fprintf(stderr, "Warning: Invalid value for start_timestamp\n");
...
static int lcm_logprov_get_fileno(lcm_logprov_t *lr) { return lr->notify_pipe[0]; }
...
static int lcm_logprov_handle(lcm_logprov_t *lr) {
  ...
  if (lr->next_clock_time < 0) lr->next_clock_time = now;
  ...
  if (lcm_try_enqueue_message(lr->lcm, lr->event->channel)) lcm_dispatch_handlers(lr->lcm, &rbuf, lr->event->channel);
...
static int lcm_logprov_publish(lcm_logprov_t *lcm, const char *channel, const void *data, unsigned int datalen) {
