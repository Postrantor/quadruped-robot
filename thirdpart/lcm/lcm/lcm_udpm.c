...
void lcm_udpm_destroy(lcm_udpm_t *lcm) {
  ...
  if (lcm->sendfd >= 0) lcm_close_socket(lcm->sendfd);
...
static int _recv_message_fragment(lcm_udpm_t *lcm, lcm_buf_t *lcmb, uint32_t sz) {
  ...
  if (!fbuf && hdr->fragment_no == 0) {
    ...
    if (!lcm_has_handlers(lcm->lcm, channel)) return 0;
...
static int _recv_short_message(lcm_udpm_t *lcm, lcm_buf_t *lcmb, int sz) {
  ...
  // if the packet has no subscribers, drop the message now.
  if (!lcm_try_enqueue_message(lcm->lcm, pkt_channel_str)) return 0;
...
static int lcm_udpm_subscribe(lcm_udpm_t *lcm, const char *channel) { return _setup_recv_parts(lcm); }

static int lcm_udpm_publish(lcm_udpm_t *lcm, const char *channel, const void *data, unsigned int datalen) {
...
static int lcm_udpm_handle(lcm_udpm_t *lcm) {
  int status;
  char ch;
  if (0 != _setup_recv_parts(lcm)) return -1;
...
static int _setup_recv_parts(lcm_udpm_t *lcm) {
  ...
  getsockopt(lcm->recvfd, SOL_SOCKET, SO_RCVBUF, (char *)&lcm->kernel_rbuf_sz, (socklen_t *)&retsize);
  dbg(DBG_LCM, "LCM: receive buffer is %d bytes\n", lcm->kernel_rbuf_sz);
  if (lcm->params.recv_buf_size) {
    if (setsockopt(
            lcm->recvfd, SOL_SOCKET, SO_RCVBUF, (char *)&lcm->params.recv_buf_size, sizeof(lcm->params.recv_buf_size)) <
        0) {
...
lcm_provider_t *lcm_udpm_create(lcm_t *parent, const char *network, const GHashTable *args) {
  ...
  if (setsockopt(lcm->sendfd, IPPROTO_IP, IP_MULTICAST_TTL, (char *)&params.mc_ttl, sizeof(params.mc_ttl)) < 0) {
...
#endif
  if (setsockopt(lcm->sendfd, IPPROTO_IP, IP_MULTICAST_LOOP, (char *)&send_lo_opt, sizeof(send_lo_opt)) < 0) {
