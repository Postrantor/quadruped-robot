...
static gboolean _sockaddr_in_equal(const void *a, const void *b) {
  ...
  return a_addr->sin_addr.s_addr == b_addr->sin_addr.s_addr && a_addr->sin_port == b_addr->sin_port &&
...
lcm_frag_buf_store *lcm_frag_buf_store_new(uint32_t max_total_size, uint32_t max_n_frag_bufs) {
  ...
  store->frag_bufs =
      g_hash_table_new_full(_sockaddr_in_hash, _sockaddr_in_equal, NULL, (GDestroyNotify)lcm_frag_buf_destroy);
  return store;
}
...
void lcm_frag_buf_store_add(lcm_frag_buf_store *store, lcm_frag_buf_t *fbuf) {
  while (store->total_size > store->max_total_size || g_hash_table_size(store->frag_bufs) > store->max_n_frag_bufs) {
...
int lcm_buf_queue_is_empty(lcm_buf_queue_t *q) { return q->head == NULL ? 1 : 0; }

#ifdef __linux__
static inline int _parse_inaddr(const char *addr_str, struct in_addr *addr) {
  char buf[] = {
      '0',         'x', addr_str[6], addr_str[7], addr_str[4], addr_str[5], addr_str[2], addr_str[3], addr_str[0],
      addr_str[1], 0};
