...
int inet_aton(const char *cp, struct in_addr *inp) {
  ...
  if (a.S_un.S_addr == INADDR_NONE) return 0;
  *inp = a;
...
int fcntl(int fd, int flag1, ...)  // Fake out F_GETFL, set socket to block or not on F_SETFL
{
  ...
  if (flag1 == F_SETFL) {
    ...
    if (flag2 == -1) return 0;      // No 3rd arg? Just exit
...
size_t recvmsg(SOCKET s, struct msghdr *msg, int flags) {
  ...
  if (WSARecvMsg == NULL) {
    status = WSAIoctl(
        (SOCKET)s, SIO_GET_EXTENSION_FUNCTION_POINTER, &WSARecvMsg_GUID, sizeof WSARecvMsg_GUID, &WSARecvMsg,
        sizeof WSARecvMsg, &nRead, NULL, NULL);
...
size_t sendmsg(SOCKET s, const struct msghdr *msg, int flags) {
  ...
  status = WSASendTo(
      (SOCKET)s, (WSABUF *)msg->msg_iov, msg->msg_iovlen, &nWritten, 0, msg->msg_name, msg->msg_namelen, NULL, NULL);
  if (status != 0) {
    return -1;
  }
  return nWritten;
