...
static int publish_message_internal(lcm_mpudpm_t *lcm, const char *channel, const void *data, unsigned int datalen);
static void publish_channel_mapping_update(lcm_mpudpm_t *lcm);
static void channel_port_mapping_update_handler(
    lcm_mpudpm_t *lcm, const channel_port_map_update_t *msg, int64_t recv_time);
static void update_subscription_ports(lcm_mpudpm_t *lcm);
static void add_channel_to_subscriber(lcm_mpudpm_t *lcm, mpudpm_subscriber_t *sub, const char *channel, uint16_t port);
...
static void mpudpm_subscriber_t_destroy(mpudpm_subscriber_t *sub) {
  if (sub->channel_string != NULL) free(sub->channel_string);
  if (sub->regex != NULL) g_regex_unref(sub->regex);
  ...
  if (sub->channel_set != NULL) g_hash_table_destroy(sub->channel_set);
  free(sub);
}
...
void lcm_mpudpm_destroy(lcm_mpudpm_t *lcm) {
  ...
  if (lcm->send_fd >= 0) lcm_close_socket(lcm->send_fd);
...
static void new_argument(gpointer key, gpointer value, gpointer user) {
  ...
  if (!strcmp((char *)key, "recv_buf_size")) {
    ...
    if (endptr == value) fprintf(stderr, "Warning: Invalid value for recv_buf_size\n");
  } else if (!strcmp((char *)key, "ttl")) {
    char *endptr = NULL;
    params->mc_ttl = strtol((char *)value, &endptr, 0);
    if (endptr == value) fprintf(stderr, "Warning: Invalid value for ttl\n");
...
static int recv_message_fragment(lcm_mpudpm_t *lcm, lcm_buf_t *lcmb, uint32_t sz) {
  ...
  if (!fbuf && hdr->fragment_no == 0) {
    ...
    if (!lcm_has_handlers(lcm->lcm, channel) && !is_reserved_channel(channel)) return 0;

    fbuf = lcm_frag_buf_new(
        *((struct sockaddr_in *)&lcmb->from), channel, msg_seqno, data_size, fragments_in_msg, lcmb->recv_utime);
...
#ifdef __linux__
  if (lcm->kernel_rbuf_sz < 262145 && data_size > lcm->kernel_rbuf_sz && !lcm->warned_about_small_kernel_buf) {
    fprintf(
        stderr,
        "==== LCM Warning ===\n"
  ...
  if (0 == fbuf->fragments_remaining) {
    ...
    if (!is_reserved_channel(fbuf->channel) && !lcm_try_enqueue_message(lcm->lcm, fbuf->channel)) {
...
static void dispatch_complete_message(lcm_mpudpm_t *lcm, lcm_buf_t *lcmb, int actual_size) {
  ...
  if (strcmp(lcmb->channel_name, CHANNEL_TO_PORT_MAP_REQUEST_CHANNEL) == 0) {
    ...
    int status = channel_port_map_update_t_decode(lcmb->buf, lcmb->data_offset, lcmb->data_size, &upd_msg);
    if (status < 0) {
...
#endif
        if (!got_utime) lcmb->recv_utime = lcm_timestamp_now();
...
int lcm_mpudpm_subscribe(lcm_mpudpm_t *lcm, const char *channel) {
  ...
  if (g_regex_match(lcm->regex_finder_re, channel, (GRegexMatchFlags)0, NULL)) {
    ...
    char *msg = "r";
    g_static_mutex_lock(&lcm->transmit_lock);
    publish_message_internal(lcm, CHANNEL_TO_PORT_MAP_REQUEST_CHANNEL, (uint8_t *)msg, strlen(msg));
    g_static_mutex_unlock(&lcm->transmit_lock);
  } else {
...
static void publish_channel_mapping_update(lcm_mpudpm_t *lcm) {
  ...
  channel_port_map_update_t *msg = (channel_port_map_update_t *)calloc(1, sizeof(channel_port_map_update_t));
...
static void channel_port_mapping_update_handler(
    lcm_mpudpm_t *lcm, const channel_port_map_update_t *msg, int64_t recv_utime) {
  ...
  for (int i = 0; i < msg->num_channels; i++) {
    ...
    if (lookup_value == NULL) {
      ...
      // insert the new destination into the hash table
      g_hash_table_insert(lcm->channel_to_port_map, strdup(msg->mapping[i].channel), GUINT_TO_POINTER(port));
      updated_channel_to_port_map = TRUE;
    }
  }
  int channel_to_port_map_size = g_hash_table_size(lcm->channel_to_port_map);
  if (!updated_channel_to_port_map && channel_to_port_map_size - NUM_INTERNAL_CHANNELS == msg->num_channels) {
  ...
  for (GSList *it = lcm->subscribers; it != NULL; it = it->next) {
    ...
    if (sub->regex == NULL) {
      ...
      while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        uint16_t port = GPOINTER_TO_UINT(value);
        if (g_regex_match(sub->regex, channel, (GRegexMatchFlags)0, NULL) && !is_reserved_channel(channel)) {
...
static int publish_message_internal(lcm_mpudpm_t *lcm, const char *channel, const void *data, unsigned int datalen) {
  ...
  if (lcm_timestamp_now() - lcm->last_mapping_update_utime > lcm->channel_to_port_map_update_period) {
...
int lcm_mpudpm_publish(lcm_mpudpm_t *lcm, const char *channel, const void *data, unsigned int datalen) {
  if (is_reserved_channel(channel)) {
    fprintf(
        stderr,
...
int lcm_mpudpm_handle(lcm_mpudpm_t *lcm) {
  ...
  if (!lcm_buf_queue_is_empty(lcm->inbufs_filled))
    if (lcm_internal_pipe_write(lcm->notify_pipe[1], "+", 1) < 0) perror("write to notify");
  g_static_mutex_unlock(&lcm->receive_lock);
  ...
  if (lcm->creating_read_thread) {
    ...
    if (!strcmp(lcmb->channel_name, SELF_TEST_CHANNEL)) lcm_dispatch_handlers(lcm->lcm, &rbuf, lcmb->channel_name);
  } else {
...
static mpudpm_socket_t *add_recv_socket(lcm_mpudpm_t *lcm, uint16_t port) {
  ...
  getsockopt(recv_fd, SOL_SOCKET, SO_RCVBUF, (char *)&lcm->kernel_rbuf_sz, (socklen_t *)&retsize);
  dbg(DBG_LCM, "LCM: receive buffer is %d bytes\n", lcm->kernel_rbuf_sz);
  if (lcm->params.recv_buf_size) {
    if (setsockopt(
            recv_fd, SOL_SOCKET, SO_RCVBUF, (char *)&lcm->params.recv_buf_size, sizeof(lcm->params.recv_buf_size)) <
        0) {
...
static int setup_recv_parts(lcm_mpudpm_t *lcm) {
  ...
  lcm->channel_to_port_map_update_period =
      CHANNEL_TO_PORT_MAP_UPDATE_NOMINAL_PERIOD + g_random_int_range(0, CHANNEL_TO_PORT_MAP_UPDATE_NOMINAL_PERIOD / 4);
...
lcm_provider_t *lcm_mpudpm_create(lcm_t *parent, const char *network, const GHashTable *args) {
  ...
  dbg(DBG_LCM, "Multicast to %s on ports %d:%d\n", inet_ntoa(params.mc_addr), params.mc_port_range_start,
  ...
  GError *rerr = NULL;
  lcm->regex_finder_re = g_regex_new(REGEX_FINDER_RE, (GRegexCompileFlags)0, (GRegexMatchFlags)0, &rerr);
  if (rerr) {
    fprintf(stderr, "%s: %s\n", __FUNCTION__, rerr->message);
    g_error_free(rerr);
    lcm_mpudpm_destroy(lcm);
  ...
  g_hash_table_insert(
      lcm->channel_to_port_map, strdup(CHANNEL_TO_PORT_MAP_UPDATE_CHANNEL),
      GUINT_TO_POINTER(lcm->params.mc_port_range_start));
  g_hash_table_insert(
      lcm->channel_to_port_map, strdup(CHANNEL_TO_PORT_MAP_REQUEST_CHANNEL),
      GUINT_TO_POINTER(lcm->params.mc_port_range_start));
  g_hash_table_insert(
      lcm->channel_to_port_map, strdup(SELF_TEST_CHANNEL),
  ...
  if (setsockopt(lcm->send_fd, IPPROTO_IP, IP_MULTICAST_TTL, (char *)&params.mc_ttl, sizeof(params.mc_ttl)) < 0) {
...
#endif
  if (setsockopt(lcm->send_fd, IPPROTO_IP, IP_MULTICAST_LOOP, (char *)&send_lo_opt, sizeof(send_lo_opt)) < 0) {
