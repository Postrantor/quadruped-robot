...
lcm_t *lcm_create(const char *url) {
  ...
  if (!url || !strlen(url)) url = getenv("LCM_DEFAULT_URL");
  if (!url || !strlen(url)) url = LCM_DEFAULT_URL;
...
fail:
  free(provider_str);
  free(network);
  if (args) g_hash_table_destroy(args);
  if (providers) g_ptr_array_free(providers, TRUE);
  //    if (lcm)
  //        lcm_destroy (lcm);
  return NULL;
}
...
static void map_add_handler_callback(gpointer _key, gpointer _value, gpointer _data) {
  ...
  if (!is_handler_subscriber(h, channel_name)) return;
...
GPtrArray *lcm_get_handlers(lcm_t *lcm, const char *channel) {
  ...
  for (unsigned int i = 0; i < lcm->handlers_all->len; i++) {
    lcm_subscription_t *h = (lcm_subscription_t *)g_ptr_array_index(lcm->handlers_all, i);
    if (is_handler_subscriber(h, channel)) g_ptr_array_add(handlers, h);
  }
...
int lcm_try_enqueue_message(lcm_t *lcm, const char *channel) {
  ...
  for (unsigned int i = 0; i < handlers->len; i++) {
    ...
    if (h->num_queued_messages < h->max_num_queued_messages || h->max_num_queued_messages <= 0) {
...
int lcm_has_handlers(lcm_t *lcm, const char *channel) {
  ...
  g_static_rec_mutex_lock(&lcm->mutex);
  GPtrArray *handlers = lcm_get_handlers(lcm, channel);
  if (!handlers || !handlers->len) has_handlers = 0;
  g_static_rec_mutex_unlock(&lcm->mutex);
  return has_handlers;
}
...
int lcm_dispatch_handlers(lcm_t *lcm, lcm_recv_buf_t *buf, const char *channel) {
  ...
  for (int i = 0; i < nhandlers; i++) {
    lcm_subscription_t *h = (lcm_subscription_t *)g_ptr_array_index(handlers, i);
    h->callback_scheduled = 0;
    if (h->marked_for_deletion) to_remove = g_list_prepend(to_remove, h);
  }
    ...
    for (char **option_arg = network_args + 1; *option_arg != NULL; option_arg++) {
      ...
      for (int i = 0; split_args[i]; i++) {
        ...
        if (key_value[0] && strlen(key_value[0])) {
          g_hash_table_replace(args, strdup(key_value[0]), key_value[1] ? strdup(key_value[1]) : strdup(""));
        }
