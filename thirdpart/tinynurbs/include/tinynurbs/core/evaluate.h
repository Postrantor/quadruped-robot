...
glm::vec<dim, T> curvePoint(
    unsigned int degree, const std::vector<T> &knots, const std::vector<glm::vec<dim, T>> &control_points, T u) {
 ...
 */
template <int dim, typename T>
std::vector<glm::vec<dim, T>> curveDerivatives(
    unsigned int degree,
 ...
 */
template <int dim, typename T>
array2<glm::vec<dim, T>> surfaceDerivatives(
    unsigned int degree_u,
    unsigned int degree_v,
    const std::vector<T> &knots_u,
    const std::vector<T> &knots_v,
    const array2<glm::vec<dim, T>> &control_points,
    unsigned int num_ders,
    T u,
    T v) {
...
glm::vec<3, T> surfacePoint(const RationalSurface<T> &srf, T u, T v) {
  ...
  for (int i = 0; i < srf.control_points.rows(); i++) {
    for (int j = 0; j < srf.control_points.cols(); j++) {
      Cw(i, j) = tvecnp1(util::cartesianToHomogenous(srf.control_points(i, j), srf.weights(i, j)));
    }
  }
...
array2<glm::vec<3, T>> surfaceDerivatives(const Surface<T> &srf, int num_ders, T u, T v) {
  return internal::surfaceDerivatives(
      srf.degree_u, srf.degree_v, srf.knots_u, srf.knots_v, srf.control_points, num_ders, u, v);
}
...
array2<glm::vec<3, T>> surfaceDerivatives(const RationalSurface<T> &srf, int num_ders, T u, T v) {
  ...
  array2<tvecnp1> homo_ders =
      internal::surfaceDerivatives(srf.degree_u, srf.degree_v, srf.knots_u, srf.knots_v, homo_cp, num_ders, u, v);
