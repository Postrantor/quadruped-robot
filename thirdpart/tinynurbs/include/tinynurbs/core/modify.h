...
void curveKnotInsert(
    unsigned int deg,
    const std::vector<T> &knots,
    const std::vector<glm::vec<dim, T>> &cp,
    T u,
    unsigned int r,
 ...
 */
template <int dim, typename T>
void surfaceKnotInsert(
    unsigned int degree,
    const std::vector<T> &knots,
    const array2<glm::vec<dim, T>> &cp,
    T knot,
    unsigned int r,
    bool along_u,
    std::vector<T> &new_knots,
...
Curve<T> curveKnotInsert(const Curve<T> &crv, T u, unsigned int repeat = 1) {
  ...
  internal::curveKnotInsert(
      crv.degree, crv.knots, crv.control_points, u, repeat, new_crv.knots, new_crv.control_points);
  return new_crv;
}
...
Surface<T> surfaceKnotInsertU(const Surface<T> &srf, T u, unsigned int repeat = 1) {
  ...
  internal::surfaceKnotInsert(
      new_srf.degree_u, srf.knots_u, srf.control_points, u, repeat, true, new_srf.knots_u, new_srf.control_points);
  return new_srf;
}
...
Surface<T> surfaceKnotInsertV(const Surface<T> &srf, T v, unsigned int repeat = 1) {
  ...
  internal::surfaceKnotInsert(
      srf.degree_u, srf.knots_u, srf.control_points, v, repeat, false, new_srf.knots_v, new_srf.control_points);
  return new_srf;
}
...
std::tuple<Curve<T>, Curve<T>> curveSplit(const Curve<T> &crv, T u) {
  ...
  internal::curveSplit(
      crv.degree, crv.knots, crv.control_points, u, left.knots, left.control_points, right.knots, right.control_points);
 ...
 */
template <typename T>
std::tuple<Surface<T>, Surface<T>> surfaceSplitU(const Surface<T> &srf, T u) {
  ...
  internal::surfaceSplit(
      srf.degree_u, srf.knots_u, srf.control_points, u, true, left.knots_u, left.control_points, right.knots_u,
 ...
 */
template <typename T>
std::tuple<Surface<T>, Surface<T>> surfaceSplitV(const Surface<T> &srf, T v) {
  Surface<T> left, right;
  left.degree_u = srf.degree_u;
  left.degree_v = srf.degree_v;
  ...
  internal::surfaceSplit(
      srf.degree_v, srf.knots_v, srf.control_points, v, false, left.knots_v, left.control_points, right.knots_v,
      right.control_points);
  return std::make_tuple(std::move(left), std::move(right));
}
