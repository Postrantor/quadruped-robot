...
void curveReadOBJ(
    ...
    unsigned int &deg,
    std::vector<T> &knots,
    std::vector<glm::vec<3, T>> &ctrlPts,
...
void curveSaveOBJ(
    ...
    const std::vector<glm::vec<3, T>> &ctrlPts,
    const std::vector<T> &weights,
    bool rational) {
  using std::endl;

  for (int i = 0; i < ctrlPts.size(); ++i) {
    os << "v " << ctrlPts[i].x << " " << ctrlPts[i].y << " " << ctrlPts[i].z << " " << weights[i] << endl;
  }
...
void surfaceSaveOBJ(
    ...
    const array2<glm::vec<3, T>> &ctrlPts,
  ...
  for (int j = 0; j < ctrlPts.cols(); j++) {
    for (int i = 0; i < ctrlPts.rows(); i++) {
      os << "v " << ctrlPts(i, j).x << " " << ctrlPts(i, j).y << " " << ctrlPts(i, j).z << " " << weights(i, j) << endl;
    }
  }
 ...
 */
template <typename T>
RationalCurve<T> curveReadOBJ(std::istream &is) {
  RationalCurve<T> crv;
 ...
 */
template <typename T>
RationalSurface<T> surfaceReadOBJ(std::istream &is) {
  RationalSurface<T> srf;
  bool rat;
  internal::surfaceReadOBJ(
      is, srf.degree_u, srf.degree_v, srf.knots_u, srf.knots_v, srf.control_points, srf.weights, rat);
  return srf;
}
 ...
 */
template <typename T>
void curveSaveOBJ(std::ostream &os, const Curve<T> &crv) {
 ...
 */
template <typename T>
void curveSaveOBJ(std::ostream &os, const RationalCurve<T> &crv) {
 ...
 */
template <typename T>
void surfaceSaveOBJ(std::ostream &os, const Surface<T> &srf) {
  array2<T> w(srf.control_points.rows(), srf.control_points.cols(), T(1));
  internal::surfaceSaveOBJ(os, srf.degree_u, srf.degree_v, srf.knots_u, srf.knots_v, srf.control_points, w, false);
}
...
void surfaceSaveOBJ(std::ostream &os, const RationalSurface<T> &srf) {
  internal::surfaceSaveOBJ(
      os, srf.degree_u, srf.degree_v, srf.knots_u, srf.knots_v, srf.control_points, srf.weights, true);
}
