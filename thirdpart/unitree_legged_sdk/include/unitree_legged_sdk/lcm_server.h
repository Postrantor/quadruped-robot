...
public:
  Lcm_Server_Low(LeggedType rname) : udp(LOWLEVEL), mylcm(LOWLEVEL) { udp.InitCmdData(cmd); }
...
public:
  Lcm_Server_High(LeggedType rname) : udp(HIGHLEVEL), mylcm(HIGHLEVEL) { udp.InitCmdData(cmd); }
  void UDPRecv() { udp.Recv(); }
  void UDPSend() { udp.Send(); }
  void LCMRecv();
  void RobotControl();
