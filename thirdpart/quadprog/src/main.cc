...
int main(int argc, char *const argv[]) {
  ...
  CE.resize(n, m);
  {
    ...
    for (int i = 0; i < n; i++)
      for (int j = 0; j < m; j++) is >> CE[i][j] >> ch;
  }
  ...
  CI.resize(n, p);
  {
    ...
    for (int i = 0; i < n; i++)
      for (int j = 0; j < p; j++) is >> CI[i][j] >> ch;
  }
  ...
  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++) sum += x[i] * G[i][j] * x[j];
  sum *= 0.5;

  for (int i = 0; i < n; i++) sum += g0[i] * x[i];
