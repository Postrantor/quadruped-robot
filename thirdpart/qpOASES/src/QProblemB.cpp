...
QProblemB::QProblemB() {
  ...
  if (options.printLevel != PL_NONE) printCopyrightNotice();
...
returnValue QProblemB::init(
  ...
  if ((xOpt == 0) && (yOpt != 0) && (guessedBounds != 0)) return THROWERROR(RET_INVALID_ARGUMENTS);

  if ((_R != 0) && ((xOpt != 0) || (yOpt != 0) || (guessedBounds != 0)))
    return THROWERROR(RET_NO_CHOLESKY_WITH_INITIAL_GUESS);

  /* 2) Setup QP data. */
  if (setupQPdata(_H, _g, _lb, _ub) != SUCCESSFUL_RETURN) return THROWERROR(RET_INVALID_ARGUMENTS);
...
returnValue QProblemB::hotstart(
  ...
  if (guessedBounds != 0) {
    ...
    if (setupAuxiliaryQP(guessedBounds) != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
  ...
  if (areBoundsConsistent(lb_new, ub_new) != SUCCESSFUL_RETURN) return setInfeasibilityFlag(returnvalue, BT_TRUE);
  ...
  if (options.enableFarBounds == BT_FALSE) {
    ...
    for (;;) {
      nWSR = nWSR_max;
      if (cputime != 0) cputime_remaining = *cputime - cputime_needed;

      /* Automatically call standard solveQP if regularisation is not active. */
      returnvalue =
          solveRegularisedQP(g_new, lb_new_far, ub_new_far, nWSR, &cputime_remaining, nWSR_performed, isFirstCall);
 ...
 */
returnValue QProblemB::getWorkingSet(real_t* workingSet) { return getWorkingSetBounds(workingSet); }
...
returnValue QProblemB::getWorkingSetBounds(real_t* workingSetB) {
  ...
  if (workingSetB == 0) return THROWERROR(RET_INVALID_ARGUMENTS);
...
real_t QProblemB::getObjVal(const real_t* const _x) const {
  ...
  for (i = 0; i < nV; ++i) objVal += _x[i] * g[i];
  ...
  switch (hessianType) {
    ...
    case HST_IDENTITY:
      for (i = 0; i < nV; ++i) objVal += 0.5 * _x[i] * _x[i];
      break;

    default:
      real_t* Hx = new real_t[nV];
      H->times(1, 1.0, _x, nV, 0.0, Hx, nV);
      for (i = 0; i < nV; ++i) objVal += 0.5 * _x[i] * Hx[i];
      delete[] Hx;
      break;
  }
  ...
  if (usingRegularisation() == BT_TRUE) {
    for (i = 0; i < nV; ++i) objVal += 0.5 * _x[i] * regVal * _x[i];
  }
...
#ifndef __MATLAB__
  if ((options.printLevel == PL_HIGH) && (options.printLevel != _printLevel)) THROWINFO(RET_PRINTLEVEL_CHANGED);
#endif /* __MATLAB__ */
...
returnValue QProblemB::determineHessianType() {
  ...
  switch (hessianType) {
    ...
    case HST_INDEF:
      ...
      if (H != 0) return SUCCESSFUL_RETURN;
  ...
  if (H->isDiag() == BT_FALSE) return SUCCESSFUL_RETURN;
  ...
  for (i = 0; i < nV; ++i) {
    curDiag = H->diag(i);
    if (curDiag >= INFTY) return RET_DIAGONAL_NOT_INITIALISED;
    ...
    if (getAbs(curDiag - 1.0) > EPS) isIdentity = BT_FALSE;

    if (getAbs(curDiag) > EPS) isZero = BT_FALSE;
  }

  if (isIdentity == BT_TRUE) hessianType = HST_IDENTITY;
 ...
 */
returnValue QProblemB::setupSubjectToType() { return setupSubjectToType(lb, ub); }
...
returnValue QProblemB::setupSubjectToType(const real_t* const lb_new, const real_t* const ub_new) {
  ...
  if ((lb_new != 0) && (ub_new != 0)) {
    ...
    if ((lb_new == 0) && (ub_new == 0)) {
      for (i = 0; i < nV; ++i) bounds.setType(i, ST_UNBOUNDED);
    } else {
      for (i = 0; i < nV; ++i) bounds.setType(i, ST_BOUNDED);
    }
  }
...
returnValue QProblemB::computeCholesky() {
  ...
  switch (hessianType) {
    case HST_ZERO:
      ...
      if (usingRegularisation() == BT_TRUE) {
        for (i = 0; i < nV; ++i) RR(i, i) = getSqrt(regVal);
      } else {
    ...
    default:
      ...
      if (nFR > 0) {
        ...
        for (i = 0; i < nFR - 1; ++i) RR(i + 1, i) = 0.0;
      }
      break;
  }
 ...
 *	b a c k s o l v e R
 */
returnValue QProblemB::backsolveR(const real_t* const b, BooleanType transposed, real_t* const a) const {
...
returnValue QProblemB::backsolveR(
  ...
  if (transposed == BT_FALSE) {
    ...
    for (i = (nR - 1); i >= 0; --i) {
      sum = b[i];
      for (j = (i + 1); j < nR; ++j) sum -= RR(i, j) * a[j];
...
returnValue QProblemB::setupQPdata(
    SymmetricMatrix* _H, const real_t* const _g, const real_t* const _lb, const real_t* const _ub) {
 ...
 */
returnValue QProblemB::setupQPdata(
    const real_t* const _H, const real_t* const _g, const real_t* const _lb, const real_t* const _ub) {
  /* 1) Setup Hessian matrix. */
  setH(_H);
 ...
 */
returnValue QProblemB::setupQPdataFromFile(
    const char* const H_file, const char* const g_file, const char* const lb_file, const char* const ub_file) {
  int_t i;
  int_t nV = getNV();
 ...
 */
returnValue QProblemB::loadQPvectorsFromFile(
    const char* const g_file,
    const char* const lb_file,
    const char* const ub_file,
    real_t* const g_new,
...
returnValue QProblemB::setInfeasibilityFlag(returnValue returnvalue, BooleanType doThrowError) {
  ...
  if ((doThrowError == BT_TRUE) || (options.enableFarBounds == BT_FALSE)) THROWERROR(returnvalue);
 ...
 */
BooleanType QProblemB::isCPUtimeLimitExceeded(const real_t* const cputime, real_t starttime, int_t nWSR) const {
  /* Always perform next QP iteration if no CPU time limit is given. */
  if (cputime == 0) return BT_FALSE;

  /* Always perform first QP iteration. */
  if (nWSR <= 0) return BT_FALSE;
...
returnValue QProblemB::regulariseHessian() {
  ...
  if (options.enableRegularisation == BT_FALSE) return SUCCESSFUL_RETURN;

  /* Regularisation of identity Hessian not possible. */
  if (hessianType == HST_IDENTITY) return THROWERROR(RET_CANNOT_REGULARISE_IDENTITY);
  ...
  if (usingRegularisation() == BT_TRUE)
    ...
    if (hessianType == HST_ZERO) {
      ...
      if (H->addToDiag(regVal) == RET_NO_DIAGONAL_AVAILABLE) return THROWERROR(RET_CANNOT_REGULARISE_SPARSE);
    }
 ...
 */
returnValue QProblemB::performRatioTest(
    int_t nIdx,
 ...
 */
real_t QProblemB::getRelativeHomotopyLength(
 ...
 */
returnValue QProblemB::updateFarBounds(
    real_t curFarBound,
    int_t nRamp,
    const real_t* const lb_new,
...
returnValue QProblemB::solveQP(
  ...
  if (setupSubjectToType(lb_new, ub_new) != SUCCESSFUL_RETURN) return THROWERROR(RET_HOTSTART_FAILED);
  ...
  for (iter = nWSRperformed; iter < nWSR; ++iter) {
    ...
    returnvalue = determineDataShift(g_new, lb_new, ub_new, delta_g, delta_lb, delta_ub, Delta_bB_isZero);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yFX;
      delete[] delta_xFX;
      delete[] delta_xFR;
    ...
    returnvalue = determineStepDirection(delta_g, delta_lb, delta_ub, Delta_bB_isZero, delta_xFX, delta_xFR, delta_yFX);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yFX;
      delete[] delta_xFX;
      delete[] delta_xFR;
    ...
    returnvalue = performStep(delta_g, delta_lb, delta_ub, delta_xFX, delta_xFR, delta_yFX, BC_idx, BC_status);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yFX;
      delete[] delta_xFX;
      delete[] delta_xFR;
 ...
 */
returnValue QProblemB::solveRegularisedQP(
    const real_t* const g_new,
    const real_t* const lb_new,
    const real_t* const ub_new,
    int_t& nWSR,
...
returnValue QProblemB::setupAuxiliaryQP(const Bounds* const guessedBounds) {
  ...
  /* nothing to do */
  if (guessedBounds == &bounds) return SUCCESSFUL_RETURN;
  ...
  if (shallRefactorise(guessedBounds) == BT_TRUE) {
    ...
    if (setupAuxiliaryWorkingSet(guessedBounds, BT_TRUE) != SUCCESSFUL_RETURN) THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);

    /* 3) Calculate Cholesky decomposition. */
    if (computeCholesky() != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
  } else {
  ...
  if (setupAuxiliaryQPbounds(BT_FALSE) != SUCCESSFUL_RETURN) THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
      ...
      switch (hessianType) {
        ...
        case HST_IDENTITY:
          for (i = 0; i < nFR; ++i) {
            ...
            /* compute max norm */
            if (rnrm < getAbs(delta_xFR_TMP[i])) rnrm = getAbs(delta_xFR_TMP[i]);
          }
          break;
  ...
  if (nFX > 0) {
    if ((hessianType == HST_ZERO) || (hessianType == HST_IDENTITY)) {
      for (i = 0; i < nFX; ++i) {
        ...
        if (hessianType == HST_ZERO) {
          if (usingRegularisation() == BT_TRUE) delta_yFX[i] += regVal * delta_xFX[i];
        } else
 ...
 */
returnValue QProblemB::performStep(
    const real_t* const delta_g,
    const real_t* const delta_lb,
    const real_t* const delta_ub,
  ...
  if (BC_status == ST_UNDEFINED)
    ...
    snprintf(
        messageString, MAX_STRING_LENGTH, "Stepsize is %.15e! (idx = %d, status = %d)", tau, (int)BC_idx,
        (int)BC_status);
...
BooleanType QProblemB::shallRefactorise(const Bounds* const guessedBounds) const {
  ...
  for (i = 0; i < nV; ++i)
    if (guessedBounds->getStatus(i) != bounds.getStatus(i)) ++differenceNumber;
 ...
 *	a d d B o u n d
 */
returnValue QProblemB::addBound(int_t number, SubjectToStatus B_status, BooleanType updateCholesky) {
  int_t i, j;
  int_t nV = getNV();
  int_t nFR = getNFR();
  ...
  if ((updateCholesky == BT_TRUE) && (hessianType != HST_ZERO) && (hessianType != HST_IDENTITY)) {
    ...
    for (i = 0; i < nFR - 1; ++i)
      for (j = number_idx + 1; j < nFR; ++j) RR(i, j - 1) = RR(i, j);
    /* ... last column of R. */
    for (i = 0; i < nFR; ++i) RR(i, nFR - 1) = 0.0;
  }
...
returnValue QProblemB::removeBound(int_t number, BooleanType updateCholesky) {
  ...
  if (options.enableFlippingBounds == BT_TRUE) flipper.set(&bounds, R);
  ...
  /* Perform cholesky updates only if QProblemB has been initialised! */
  if (getStatus() == QPS_PREPARINGAUXILIARYQP) return SUCCESSFUL_RETURN;
 ...
 */
returnValue QProblemB::printIteration(
    int_t iter, int_t BC_idx, SubjectToStatus BC_status, real_t homotopyLength, BooleanType isFirstCall) {
#ifndef __SUPPRESSANYOUTPUT__
  ...
  switch (options.printLevel) {
    case PL_DEBUG_ITER:
      ...
      snprintf(
          myPrintfString, MAX_STRING_LENGTH, "%9.2e %9.2e %9.2e %9.2e %9.2e\n", homotopyLength, tau, stat, bfeas,
          bcmpl);
      myPrintf(myPrintfString);
    ...
    case PL_MEDIUM:
      ...
      if (BC_status == ST_UNDEFINED) {
        ...
        if (isFirstCall == BT_TRUE)
          snprintf(
              myPrintfString, MAX_STRING_LENGTH, "   %5.1d   |   %1.6e   |    %s SOLVED     |  %4.1d   \n", (int)iter,
              tau, info, (int)getNFX());
        else
          snprintf(
              myPrintfString, MAX_STRING_LENGTH, "   %5.1d*  |   %1.6e   |    %s SOLVED     |  %4.1d   \n", (int)iter,
        ...
        snprintf(
            myPrintfString, MAX_STRING_LENGTH, "   %5.1d   |   %1.6e   |   %s %4.1d   |  %4.1d   \n", (int)iter, tau,
            info, (int)BC_idx, (int)getNFX());
        myPrintf(myPrintfString);
      }
      break;
