 ...
 *	c o p y
 */
returnValue SparseSolver::copy(const SparseSolver& rhs) { return SUCCESSFUL_RETURN; }
 ...
 */
returnValue Ma27SparseSolver::setMatrixData(
    int_t dim_, int_t numNonzeros_, const int_t* const irn, const int_t* const jcn, const real_t* const avals) {
  reset();
  dim = dim_;
...
returnValue Ma27SparseSolver::factorize() {
  ...
  MA27AD(
      &dim, &numNonzeros, irn_ma27, jcn_ma27, iw_ma27, &liw_ma27, ikeep_ma27, iw1_ma27, &nsteps_ma27, &iflag_ma27,
      icntl_ma27, cntl_ma27, info_ma27, &ops_ma27);
  ...
  for (int_t i = 0; i < numNonzeros; ++i) a_new[i] = a_ma27[i];
  delete[] a_ma27;
  a_ma27 = a_new;
  ...
  MA27BD(
      &dim, &numNonzeros, irn_ma27, jcn_ma27, a_ma27, &la_ma27, iw_ma27, &liw_ma27, ikeep_ma27, &nsteps_ma27,
...
returnValue Ma27SparseSolver::solve(int_t dim_, const real_t* const rhs, real_t* const sol) {
  /* consistency check */
  if (dim_ != dim) return THROWERROR(RET_INVALID_ARGUMENTS);
  ...
  MA27CD(
      &dim, a_ma27, &la_ma27, iw_ma27, &liw_ma27, w_ma27, &maxfrt_ma27, sol, iw1_ma27, &nsteps_ma27, icntl_ma27,
      cntl_ma27);
...
returnValue Ma27SparseSolver::reset() {
  ...
  if (SparseSolver::reset() != SUCCESSFUL_RETURN) return THROWERROR(RET_RESET_FAILED);
...
returnValue Ma27SparseSolver::copy(const Ma27SparseSolver& rhs) {
  ...
  for (int_t i = 0; i < 30; ++i) icntl_ma27[i] = rhs.icntl_ma27[i];

  for (int_t i = 0; i < 5; ++i) cntl_ma27[i] = rhs.cntl_ma27[i];
...
Ma57SparseSolver::Ma57SparseSolver() : SparseSolver() {
  ...
  cntl_ma57[1] = 5.0e-16;		// Pivots smaller than this are treated as zero and are placed at the end of the
  factorization (default: 1e-20) cntl_ma57[0] = 0.5;			// Set pivot tolerance: Higher values = more
  stable but slower/less sparse (default: 0.01, max 0.5) */
}
 ...
 *	s e t M a t r i x D a t a
 */
returnValue Ma57SparseSolver::setMatrixData(
    int_t dim_, int_t numNonzeros_, const int_t* const irn, const int_t* const jcn, const real_t* const avals) {
  reset();
  dim = dim_;
  numNonzeros = numNonzeros_;
...
returnValue Ma57SparseSolver::factorize() {
  ...
  MA57AD(&dim, &numNonzeros, irn_ma57, jcn_ma57, &lkeep_ma57, keep_ma57, iwork_ma57, icntl_ma57, info_ma57, rinfo_ma57);
  ...
  MA57BD(
      &dim, &numNonzeros, a_ma57, fact_ma57, &lfact_ma57, ifact_ma57, &lifact_ma57, &lkeep_ma57, keep_ma57, iwork_ma57,
      icntl_ma57, cntl_ma57, info_ma57, rinfo_ma57);
  ...
  for (k = 0; k < nblk; k++) {
    ...
    for (kk = 0; kk < nrows; kk++) pivots[count++] = ifact_ma57[iwpos + 2 + kk] - 1; /* convert Fortran to C indices! */
...
returnValue Ma57SparseSolver::solve(int_t dim_, const real_t* const rhs, real_t* const sol) {
  ...
  for (int_t i = 0; i < dim; ++i) sol[i] = rhs[i];
  MA57CD(
      &job_ma57, &dim, fact_ma57, &lfact_ma57, ifact_ma57, &lifact_ma57, &nrhs_ma57, sol, &lrhs_ma57, work_ma57,
      &lwork_ma57, iwork_ma57, icntl_ma57, info_ma57);
...
returnValue Ma57SparseSolver::getZeroPivots(int_t*& zeroPivots) {
  for (int_t k = 0; k < dim - rank; k++) zeroPivots[k] = pivots[rank + k];
...
returnValue Ma57SparseSolver::copy(const Ma57SparseSolver& rhs) {
  ...
  for (int_t i = 0; i < 30; ++i) icntl_ma57[i] = rhs.icntl_ma57[i];

  for (int_t i = 0; i < 5; ++i) cntl_ma57[i] = rhs.cntl_ma57[i];
...
returnValue DummySparseSolver::factorize() { return THROWERROR(RET_NO_SPARSE_SOLVER); }
