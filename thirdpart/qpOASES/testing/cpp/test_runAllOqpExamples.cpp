...
int main() {
  ...
  for (int_t ii = 0; ii < 2 * numBenchmarks; ++ii) {
    ...
    if (runOqpBenchmark(
            benchmarkPath[ii / 2], isSparse, useHotstarts, options, maxAllowedNWSR, maxNWSR, avgNWSR, maxCPUtime,
            avgCPUtime, maxStationarity, maxFeasibility, maxComplementarity) != SUCCESSFUL_RETURN) {
