...
/* solve with sparse matrices (Schur complement) */
  ...
  fprintf(
      stdFile, "Solved problem with Schur complement approach in %d iterations, %.3f seconds.\n", (int)nWSR, toc - tic);
#endif /* SOLVER_NONE */
...
#ifndef SOLVER_NONE
  for (i = 0; i < n; i++)
    if (getAbs(x[i] - xref[i]) > errP) errP = getAbs(x[i] - xref[i]);
#endif /* SOLVER_NONE */
...
#ifndef SOLVER_NONE
  for (i = 0; i < n + m; i++)
    if (getAbs(y[i] - yref[i]) > errD) errD = getAbs(y[i] - yref[i]);
#endif /* SOLVER_NONE */
