...
int main(int argc, char** argv) {
  ...
  real_t A[6 * 7] = {-0.270792, -0.140175,   -0.249325,  -0.412277, -0.250952,  -0.298071,  1.49078e-19,
                     0.534738,  0.00704892,  0.483388,   -0.193107, 0.302895,   -0.2471,    -2.1684e-17,
                     0,         -0.451464,   0.117068,   -0.348254, 0.00703103, 0.00620952, -7.96211e-20,
                     0,         0.0502231,   0.393264,   -0.584145, 0.758854,   -0.637889,  -0.023751,
                     0,         0.998738,    -0.0197759, 0.766682,  0.632337,   0.769921,   0.00352803,
  ...
  SQProblem joints_velocities(
      7, 6);  // create qp problem of 7 variables and 6 constraints with the hessian matrix set with the identity
  joints_velocities.init(H, g, A, lb, ub, lbA, ubA, nWSR, &cpu_time);
  if (joints_velocities.isSolved()) printf("qp problem solved");
  if (joints_velocities.isInfeasible()) printf("qp problem not feasible");
  joints_velocities.getPrimalSolution(xOpt);
  printf(
      "\nxOpt = [ %e, %e, %e, %e, %e, %e, %e ];  objVal = %e\n\n", xOpt[0], xOpt[1], xOpt[2], xOpt[3], xOpt[4], xOpt[5],
      xOpt[6], joints_velocities.getObjVal());
