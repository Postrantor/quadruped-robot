...
public:
  ...
  virtual int_t operator()(int_t constrIndex, const real_t* const x, real_t* const constrValue) const {
    ...
    for (i = 0; i < maxI; ++i) constrValue[0] += A[constrIndex * nV + i] * x[i];
...
int main() {
  ...
  if (readOqpDimensions(path, nQP, nV, nC, nEC) != SUCCESSFUL_RETURN) return TEST_DATA_NOT_FOUND;
  ...
  for (int_t ii = 0; ii < nV; ++ii) QPOASES_TEST_FOR_NEAR(xOptCP[ii], xOpt[ii]);

  for (int_t ii = 0; ii < nV + nC; ++ii) QPOASES_TEST_FOR_NEAR(yOptCP[ii], yOpt[ii]);
