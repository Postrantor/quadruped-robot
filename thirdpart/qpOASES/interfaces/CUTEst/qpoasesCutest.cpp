...
int convertTripletToHbf(
    int m, int n, int nnz, double *vals, int *iRow, int *jCol, double *vals_hbf, int *iRow_hbf, int *jCol_hbf) {
  int i, ii, j, k;

  /* Initialize output arrays */
  for (k = 0; k < n + 1; k++) jCol_hbf[k] = 0;
  for (k = 0; k < nnz; k++) {
    iRow_hbf[k] = 0;
    vals_hbf[k] = 0.0;
  }
  ...
  jCol_hbf[n] = nnz;
  for (k = n - 1; k > -1; k--) jCol_hbf[k] = jCol_hbf[k + 1] - jCol_hbf[k];
  ...
  for (k = n - 1; k > 0; k--) jCol_hbf[k] = jCol_hbf[k - 1];
  jCol_hbf[0] = 0;
...
int main() {
  ...
  CUTEST_csetup(
      &status, &funit, &iout, &io_buffer, &nVar, &nCon, xInit, lb, ub, yInit, lbA, ubA, isEq, isLinear, &e_order,
      &l_order, &v_order);
  if (status) {
  ...
  elapsedTime = (endTime.tv_sec * 1000000 + (endTime.tv_usec)) - (startTime.tv_sec * 1000000 + (startTime.tv_usec));
