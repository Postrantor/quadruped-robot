...
QPInstance::QPInstance(uint_t _nV, uint_t _nC, HessianType _hessianType, BooleanType _isSimplyBounded) {
  handle = s_nexthandle++;
...
int_t QPInstance::getNV() const {
  if (sqp != 0) return sqp->getNV();

  if (qpb != 0) return qpb->getNV();

  return 0;
}

int_t QPInstance::getNC() const {
  if (sqp != 0) return sqp->getNC();
 ...
 */
int_t allocateQPInstance(
    int_t nV, int_t nC, HessianType hessianType, BooleanType isSimplyBounded, const Options* options) {
  QPInstance* inst = new QPInstance(nV, nC, hessianType, isSimplyBounded);

  if ((inst->sqp != 0) && (options != 0)) inst->sqp->setOptions(*options);

  if ((inst->qpb != 0) && (options != 0)) inst->qpb->setOptions(*options);
...
QPInstance* getQPInstance(int_t handle) {
  ...
  for (ii = 0; ii < g_instances.size(); ++ii)
    if (g_instances[ii]->handle == handle) return g_instances[ii];
  return 0;
}
...
void deleteQPInstance(int_t handle) {
  ...
  if (instance != 0) {
    for (std::vector<QPInstance*>::iterator itor = g_instances.begin(); itor != g_instances.end(); ++itor)
 ...
 */
returnValue smartDimensionCheck(
    real_t** input, uint_t m, uint_t n, BooleanType emptyAllowed, const mxArray* prhs[], int_t idx) {
  ...
  if (mxIsEmpty(prhs[idx])) {
    ...
    if ((emptyAllowed == BT_TRUE) || (idx == 0)) /* idx==0 used for auxInput */
    {
      *input = 0;
      ...
      if (idx > 0) snprintf(msg, MAX_STRING_LENGTH, "ERROR (qpOASES): Empty argument %d not allowed!", idx + 1);
      myMexErrMsgTxt(msg);
        ...
        *input = (real_t*)mxGetPr(prhs[idx]);
        ...
        if (idx > 0)
          snprintf(
              ...
              "ERROR (qpOASES): Input dimension mismatch for argument %d ([%ld,%ld] ~= [%d,%d]).", idx + 1,
...
BooleanType containsNaN(const real_t* const data, uint_t dim) {
  ...
  if (data == 0) return BT_FALSE;

  for (i = 0; i < dim; ++i)
    if (mxIsNaN(data[i]) == 1) return BT_TRUE;
...
BooleanType containsInf(const real_t* const data, uint_t dim) {
  ...
  for (i = 0; i < dim; ++i)
    if (mxIsInf(data[i]) == 1) return BT_TRUE;
...
BooleanType containsNaNorInf(const mxArray* prhs[], int_t rhs_index, bool mayContainInf) {
  ...
  if (rhs_index < 0) return BT_FALSE;
  ...
  if (containsNaN((real_t*)mxGetPr(prhs[rhs_index]), dim) == BT_TRUE) {
    snprintf(msg, MAX_STRING_LENGTH, "ERROR (qpOASES): Argument %d contains 'NaN' !", rhs_index + 1);
    myMexErrMsgTxt(msg);
    return BT_TRUE;
  }
  ...
  if (mayContainInf == 0) {
    if (containsInf((real_t*)mxGetPr(prhs[rhs_index]), dim) == BT_TRUE) {
      snprintf(msg, MAX_STRING_LENGTH, "ERROR (qpOASES): Argument %d contains 'Inf' !", rhs_index + 1);
      myMexErrMsgTxt(msg);
      return BT_TRUE;
    }
  }
...
returnValue convertFortranToC(const real_t* const M_for, int_t nV, int_t nC, real_t* const M) {
  ...
  if ((M_for == 0) || (M == 0)) return RET_INVALID_ARGUMENTS;

  if ((nV < 0) || (nC < 0)) return RET_INVALID_ARGUMENTS;

  for (i = 0; i < nC; ++i)
    for (j = 0; j < nV; ++j) M[i * nV + j] = M_for[j * nC + i];
...
BooleanType hasOptionsValue(const mxArray* optionsPtr, const char* const optionString, double** optionValue) {
  ...
  if (optionName == 0) {
    char msg[MAX_STRING_LENGTH];
    snprintf(
        msg, MAX_STRING_LENGTH, "Option struct does not contain entry '%s', using default value instead!",
        optionString);
    mexWarnMsgTxt(msg);
    return BT_FALSE;
  }
...
returnValue setupOptions(Options* options, const mxArray* optionsPtr, int_t& nWSRin, real_t& maxCpuTime) {
  ...
  if (hasOptionsValue(optionsPtr, "printLevel", &optionValue) == BT_TRUE) {
    ...
    if (options->printLevel < PL_DEBUG_ITER) options->printLevel = PL_DEBUG_ITER;
    if (options->printLevel > PL_HIGH) options->printLevel = PL_HIGH;
#endif
  }
  ...
  if (hasOptionsValue(optionsPtr, "boundTolerance", &optionValue) == BT_TRUE) options->boundTolerance = *optionValue;

  if (hasOptionsValue(optionsPtr, "boundRelaxation", &optionValue) == BT_TRUE) options->boundRelaxation = *optionValue;

  if (hasOptionsValue(optionsPtr, "epsNum", &optionValue) == BT_TRUE) options->epsNum = *optionValue;

  if (hasOptionsValue(optionsPtr, "epsDen", &optionValue) == BT_TRUE) options->epsDen = *optionValue;

  if (hasOptionsValue(optionsPtr, "maxPrimalJump", &optionValue) == BT_TRUE) options->maxPrimalJump = *optionValue;

  if (hasOptionsValue(optionsPtr, "maxDualJump", &optionValue) == BT_TRUE) options->maxDualJump = *optionValue;

  if (hasOptionsValue(optionsPtr, "initialRamping", &optionValue) == BT_TRUE) options->initialRamping = *optionValue;

  if (hasOptionsValue(optionsPtr, "finalRamping", &optionValue) == BT_TRUE) options->finalRamping = *optionValue;

  if (hasOptionsValue(optionsPtr, "initialFarBounds", &optionValue) == BT_TRUE)
    options->initialFarBounds = *optionValue;

  if (hasOptionsValue(optionsPtr, "growFarBounds", &optionValue) == BT_TRUE) options->growFarBounds = *optionValue;

  if (hasOptionsValue(optionsPtr, "initialStatusBounds", &optionValue) == BT_TRUE) {
    optionValueInt = (int_t)*optionValue;
    if (optionValueInt < -1) optionValueInt = -1;
    if (optionValueInt > 1) optionValueInt = 1;
    options->initialStatusBounds = (REFER_NAMESPACE_QPOASES SubjectToStatus)optionValueInt;
  }

  if (hasOptionsValue(optionsPtr, "epsFlipping", &optionValue) == BT_TRUE) options->epsFlipping = *optionValue;
  ...
  if (hasOptionsValue(optionsPtr, "epsIterRef", &optionValue) == BT_TRUE) options->epsIterRef = *optionValue;

  if (hasOptionsValue(optionsPtr, "epsLITests", &optionValue) == BT_TRUE) options->epsLITests = *optionValue;

  if (hasOptionsValue(optionsPtr, "epsNZCTests", &optionValue) == BT_TRUE) options->epsNZCTests = *optionValue;
 ...
 */
returnValue setupAuxiliaryInputs(
    const mxArray* auxInput,
    uint_t nV,
    uint_t nC,
  ...
  if (curField == NULL)
    mexWarnMsgTxt(
        "auxInput struct does not contain entry 'hessianType'!\n         Type 'help qpOASES_auxInput' for further "
        "information.");
  else {
    if (mxIsEmpty(curField) == true) {
      *hessianType = HST_UNKNOWN;
    } else {
      if (mxIsScalar(curField) == false) return RET_INVALID_ARGUMENTS;

      double* hessianTypeTmp = mxGetPr(curField);
      int_t hessianTypeInt = (int_t)*hessianTypeTmp;
      if (hessianTypeInt < 0) hessianTypeInt = 6; /* == HST_UNKNOWN */
      if (hessianTypeInt > 5) hessianTypeInt = 6; /* == HST_UNKNOWN */
...
returnValue allocateOutputs(int nlhs, mxArray* plhs[], int_t nV, int_t nC = 0, int_t nP = 1, int_t handle = -1) {
  ...
  /* handle */
  if (handle >= 0) plhs[curIdx++] = mxCreateDoubleMatrix(1, 1, mxREAL);
      ...
      if (nlhs > curIdx) {
        ...
        if (nlhs > curIdx) {
          ...
          if (nlhs > curIdx) {
            ...
            if (curFieldNum >= 0) mxSetFieldByNumber(auxOutput, 0, curFieldNum, mxCreateDoubleMatrix(nV, nP, mxREAL));

            curFieldNum = mxAddField(auxOutput, "workingSetC");
            if (curFieldNum >= 0) mxSetFieldByNumber(auxOutput, 0, curFieldNum, mxCreateDoubleMatrix(nC, nP, mxREAL));

            curFieldNum = mxAddField(auxOutput, "cpuTime");
            if (curFieldNum >= 0) mxSetFieldByNumber(auxOutput, 0, curFieldNum, mxCreateDoubleMatrix(1, nP, mxREAL));
...
returnValue obtainOutputs(
  ...
  /* handle */
  if (handle >= 0) plhs[curIdx++] = mxCreateDoubleScalar(handle);
 ...
 */
returnValue setupHessianMatrix(
    const mxArray* prhsH, int_t nV, SymmetricMatrix** H, sparse_int_t** Hir, sparse_int_t** Hjc, real_t** Hv) {
  if (prhsH == 0) return SUCCESSFUL_RETURN;
    ...
    *Hv = new real_t[mat_jc[nV] + nV];
    for (j = 0; j < nV; j++) {
      ...
      for (i = mat_jc[j]; i < mat_jc[j + 1]; i++) {
        if (mat_ir[i] == j) needInsertDiag = BT_FALSE;
 ...
 */
returnValue setupConstraintMatrix(
    const mxArray* prhsA, int_t nV, int_t nC, Matrix** A, sparse_int_t** Air, sparse_int_t** Ajc, real_t** Av) {
  if (prhsA == 0) return SUCCESSFUL_RETURN;
