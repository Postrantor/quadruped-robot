...
GazeboRosHarness::GazeboRosHarness() : impl_(std::make_unique<GazeboRosHarnessPrivate>()) {}

GazeboRosHarness::~GazeboRosHarness() { impl_->ros_node_.reset(); }
...
void GazeboRosHarness::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  impl_->vel_sub_ = impl_->ros_node_->create_subscription<std_msgs::msg::Float32>(
      ...
      [this](const std_msgs::msg::Float32::ConstSharedPtr msg) { SetWinchVelocity(msg->data); });

  RCLCPP_INFO(impl_->ros_node_->get_logger(), "Subscribed to [%s]", impl_->vel_sub_->get_topic_name());
  ...
  RCLCPP_INFO(impl_->ros_node_->get_logger(), "Subscribed to [%s]", impl_->detach_sub_->get_topic_name());

  if (_sdf->HasElement("init_vel")) {
    auto init_vel = _sdf->Get<float>("init_vel");
    SetWinchVelocity(init_vel);
    RCLCPP_INFO(impl_->ros_node_->get_logger(), "Setting initial harness velocity to [%.2f]m/s", init_vel);
  }
}

void GazeboRosHarness::OnDetach(const std_msgs::msg::Empty::ConstSharedPtr /*msg*/) {
  if (impl_->detached_) {
    RCLCPP_WARN(impl_->ros_node_->get_logger(), "[%s] is already detached from harness", impl_->model_.c_str());
    return;
  }
  Detach();
