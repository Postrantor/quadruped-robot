...
// type `ALL`，`INDIVIDUAL` 和 `NONE`
// return `<joint_name>/<interface_type>`
// 将方法全部注册到基类对应的成员变量中
InterfaceConfiguration UnitreePositionController::command_interface_configuration() const {
  /*
...
InterfaceConfiguration UnitreePositionController::state_interface_configuration() const {
  std::vector<std::string> conf_names;
  for (const auto &joint_name : params_.left_wheel_names) {
...
controller_interface::return_type UnitreePositionController::update(
  ...
  // 由commond分解计算两侧轮子的速度信息
  // 应该pub这里的数据给电机？还是这里直接write()->motor?
  // 所以这里是进程间通信？hardware_interface会从这里交换数据？
...
// 1. 此方法会从 yaml 和 launch 中获取各类需要的参数，然后放到之前声明的各类成员变量中：
// 创建 publisher_, subscriber_
...
CLASS_LOADER_REGISTER_CLASS(unitree_position_controller::UnitreePositionController, controller_interface::ControllerInterface)
