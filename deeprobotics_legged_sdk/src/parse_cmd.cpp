  ...
  udpServer.onRawMessageReceived = [&](const char* message, int length, string ipv4, uint16_t port) {
    ...
    if (cm.command.type == command_type::CommandType::kMessValues) {
      switch (cm.command.code) {
        ...
        case 600:
          memcpy(
              (void*)&(user_command_set_.data_send_pose), nc.get_command_parameters(),
              sizeof(user_command_set_.data_send_pose));
      ...
      switch (cm.command.code) {
        case 0x21040001:
          ...
          printf(
              "index  %6d , dt_time  %04.3f, max %+04.3f, min %+04.3f, ave %+04.3f, timeout count  %6d drop count %2d",
              value, dt_time, max_time, min_time, ave_time, time_out_count, drop_count);
          cout << endl;
          // cout << "index " << count <<"  dt_time " << dt_time << " max  " << max_time  << "  min  " << min_time  << "
          // ave " << ave_time  << " drop count " << drop_count << endl;
          break;
          break;
      }
...
RobotState& ParseCMD::get_recv() { return state_rec; }
