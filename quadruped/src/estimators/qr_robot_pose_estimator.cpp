...
float qrRobotPoseEstimator::EstimateRobotHeight() {
  ...
  for (int legId = 0; legId < 4; legId++) {
    ...
    if (robot->controlParams["mode"] == LocomotionMode::WALK_LOCOMOTION) {
      flag =
          (desLegState != SubLegState::TRUE_SWING || desLegState == LegState::STANCE ||
  ...
  if (contacts.sum() == 0) {
    ...
    Vec4<float> heightsInControlFrame =
        -footPositionsControlFrame.block<1, 4>(2, 0).cwiseProduct(contacts.cast<float>());
