...
Vec3<float> qrRobotEstimator::ComputeZMP() {
  ...
  robot->stateDataFlow.zmp[1] =
      basePos[1] - basePos[2] * baseAcc[1] / (baseAcc[2]);  // baseAcc[2] receive from IMU already contains g value.
...
void qrRobotEstimator::CMUUpdate(double dt) {
  ...
  for (int i = 0; i < NumLeg; ++i) {
    ...
    R.block<3, 3>(i * 3, i * 3) =
        (1 + (1 - estimated_contacts[i]) * 1e3) * SENSOR_NOISE_PIMU_REL_FOOT * eye3;  // fk estimation
  ...
  for (int i = 0; i < NumLeg; ++i) {
    ...
    y.block<3, 1>(NumLeg * 3 + i * 3, 0) = (1.0 - estimated_contacts[i]) * x.segment<3>(3) +
