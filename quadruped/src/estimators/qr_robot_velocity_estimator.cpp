...
qrRobotVelocityEstimator::qrRobotVelocityEstimator(
  ...
  filter = new TinyEKF<3, 3>(
      0.f, initialVariance, userParametersIn->accelerometerVariance, userParametersIn->sensorVariance);
...
void qrRobotVelocityEstimator::Update(float currentTime) {
  ...
  for (int legId = 0; legId < 4; ++legId) {
    ...
    if (footContact[legId]) {
      ...
      Vec3<float> vB =
          footVInBaseFrame.col(legId) + robotics::math::vectorToSkewMat(rpyRate) * footPInBaseFrame.col(legId);
