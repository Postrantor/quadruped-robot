...
StatisticAnalysis::StatisticAnalysis(float meanIn) : defaultMean(meanIn), var_(0.f), sigma_(0.f), num(0) {
  mean_ = defaultMean;
  sum_ = defaultMean;
  data.clear();
}
...
void StatisticAnalysis::PrintStatistics() {
  GetStandradVar();
  std::cout << "Mean = " << mean_ << ",  Var = " << var_ << ", Sigma = " << sigma_ << std::endl;
}
...
void Visualization2D::Show() {
  int n = datax.size();
  std::cout << n << ", " << datay1.size() << ", " << datay2.size() << ", " << datay3.size() << ", " << datay4.size()
            << ", " << datay5.size() << ", " << datay6.size() << std::endl;
  if (n > 0) {
    plt::figure();
