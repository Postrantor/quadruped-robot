...
TorqueStanceLegController::TorqueStanceLegController(
  ...
  this->desiredSpeed << userParameters.desiredSpeed[0], userParameters.desiredSpeed[1], userParameters.desiredSpeed[2];
  ...
  if (robot->stop) {
    ...
    for (int legId = 0; legId < NumLeg; ++legId) {
      ...
      if (robot->controlParams["mode"] == LocomotionMode::VELOCITY_LOCOMOTION) {
        ...
        flag =
            ((gaitGenerator->desiredLegState[legId] == LegState::STANCE && gaitGenerator->allowSwitchLegState[legId]) ||
    ...
    for (int legId = 0; legId < NumLeg; ++legId) {
      ...
      if (detectedLegState == LegState::STANCE || detectedLegState == LegState::LOSE_CONTACT) {
        ...
        /* Plan to Swing State. The state from stance to swing will need to unload force and vice versa in walk
         * locomotion. */
  ...
  switch (robot->controlParams["mode"]) {
    case LocomotionMode::VELOCITY_LOCOMOTION: {
      ...
      if (groundEstimator->terrain.terrainType >= 2) { /* The terrain is not a horizontal plane. */
        ...
        robotComPosition =
            robotics::math::TransformVecByQuat(robotics::math::quatInverse(controlFrameOrientation), robotComPosition);
    ...
    case LocomotionMode::WALK_LOCOMOTION: {
      ...
      robotComVelocity =
          robotics::math::invertRigidTransform({0, 0, 0}, robotComOrientation, robotComVelocity);  // in world frame
      robotComRpyRate =
          robotics::math::invertRigidTransform({0, 0, 0}, robotComOrientation, robotComRpyRate);  // in world frame
      break;
    }
    ...
    switch (robot->controlParams["mode"]) {
      case LocomotionMode::WALK_LOCOMOTION:
        ...
        for (int motorId = 0; motorId < 3; motorId++) {
          if (contacts[legId]) {
            temp = {
                0.0 * desiredStateCommand->legJointq(motorId, legId), 0.0 * kps(3 * legId + motorId), 0.0,
                0.5 * kds(3 * legId + motorId), motorTorques[3 * legId + motorId]};
