...
void qrRaibertSwingLegController::Reset(float current_time) {
  ...
  if (robot->controlParams["mode"] != LocomotionMode::WALK_LOCOMOTION) {
    ...
    for (int i = 0; i < NumLeg; ++i) {
      swingFootTrajectories[i] =
          SwingFootTrajectory(splineInfo, phaseSwitchFootLocalPos.col(i), phaseSwitchFootLocalPos.col(i), 1.f, 0.15);
    }
  }
      ...
      for (int legId = 0; legId < NumLeg; ++legId) {
        if (newLegState(legId) == LegState::SWING && newLegState(legId) != gaitGenerator->curLegState(legId)) {
          ...
          phaseSwitchFootGlobalPos.col(legId) =
              Rb * phaseSwitchFootLocalPos.col(legId);  // robot->GetFootPositionsInWorldFrame().col(legId);
          ...
          swingFootTrajectories[legId] = SwingFootTrajectory(
              splineInfo, phaseSwitchFootLocalPos.col(legId), phaseSwitchFootLocalPos.col(legId), 1.f, 0.15);
        }
      }
    } break;
  }
...
map<int, Matrix<float, 5, 1>> qrRaibertSwingLegController::GetAction() {
  ...
  for (u8& legId : swingFootIds) {
    ...
    switch (robot->controlParams["mode"]) {
      case LocomotionMode::VELOCITY_LOCOMOTION: {
        ...
        targetHipHorizontalVelocity = desiredStateCommand->stateDes.segment(6, 3) +
                                      desiredStateCommand->stateDes(11) * twistingVector;  // in control frame

        footTargetPosition =
            dR.transpose() * (hipHorizontalVelocity * gaitGenerator->stanceDuration[legId] / 2.0 -
                              swingKp.cwiseProduct(targetHipHorizontalVelocity - hipHorizontalVelocity)) +
            Matrix<float, 3, 1>(hipOffset[0], hipOffset[1], 0) - robotBaseR.transpose() * desiredHeight;
        ...
        bool flag = swingFootTrajectories[legId].GenerateTrajectoryPoint(
            footPositionInBaseFrame, footVelocityInBaseFrame, footAccInBaseFrame, gaitGenerator->normalizedPhase(legId),
            true);
      ...
      case LocomotionMode::POSITION_LOCOMOTION: {
        ...
        //                                                   phaseSwitchFootGlobalPos.col(legId),
        //                                                   footHoldInWorldFrame.col(legId)); // interpolation in world
        //                                                   frame
      ...
      case LocomotionMode::WALK_LOCOMOTION: {
        ...
        //                                                             footPositionInControlFrame); // transfer to world
        //                                                             frame
      ...
      case LocomotionMode::ADVANCED_TROT: {
        ...
        Vec3<float> footTargetPositionInWorldFrame =
            robotics::math::invertRigidTransform(robot->basePosition, robotComOrientation, footTargetPosition);
        ...
        Vec3<float> foot_vel_cur = footVelocitysInBaseFrame.col(
            legId);  //(footPositionsInBaseFrame.col(legId) - foot_pos_rel_last_time.col(legId)) / 0.001;
        foot_pos_rel_last_time.col(legId) = footPositionsInBaseFrame.col(legId);

        Vec3<float> foot_vel_target =
            footVelocityInBaseFrame;  //(footPositionInBaseFrame - foot_pos_target_last_time.col(legId)) / 0.002;
        if (!(robot->isSim) && phase > 0.2 && phase < 0.9) {
          footPositionInBaseFrame[2] =
              foot_pos_target_last_time(2, legId) -
              robotics::math::clip(foot_pos_target_last_time(2, legId) - footPositionInBaseFrame[2], -0.002f, 0.002f);
        }
    ...
    Vec3<float> motorVelocity =
        robot->ComputeMotorVelocityFromFootLocalVelocity(legId, jointAngles, footVelocityInBaseFrame);
  ...
  if (!(robot->isSim)) {
    ...
    for (int singleLegId(0); singleLegId < NumLeg; ++singleLegId) {
      ...
      if (flag) {
        // std::cout << t << ", id = " << singleLegId << ", ep = " << foot_pos_error.col(singleLegId).transpose() << ",
        // ev = " <<  foot_vel_error.col(singleLegId).transpose() << std::endl;
        foot_forces_kin.col(singleLegId) =
            foot_pos_error.block<3, 1>(0, singleLegId).cwiseProduct(kps.segment(3 * singleLegId, 3)) / 2.0 +  // 40, 4
  ...
  for (auto it = swingJointAnglesVelocities.begin(); it != swingJointAnglesVelocities.end(); ++it) {
    ...
    switch (robot->controlParams["mode"]) {
      ...
      default: {
        ...
        // flag = (gaitGenerator->legState[singleLegId] == LegState::SWING
        //         || gaitGenerator->legState[singleLegId] == LegState::USERDEFINED_SWING) // for trot up to slope,  //
        //         in velocity mode
        //         || (gaitGenerator->curLegState[singleLegId] == LegState::SWING &&
        //         !gaitGenerator->allowSwitchLegState[singleLegId]); // for gait schedule mode
      } break;
