...
qrFootBSplinePatternGenerator::qrFootBSplinePatternGenerator(qrSplineInfo &splineInfo) {
  ...
  crv.knots = {0., 0., 0., 0., 0.3 / 6, 1.3 / 6, 2.5 / 6, 3.0 / 6, 4.0 / 6, 1, 1, 1, 1};
  crv.degree = 3;
...
void qrFootParabolaPatternGenerator::SetParameters(
  ...
  /* Setting the spline boundaries. */
  footSplinerZ.setBoundary(initial_time, params.duration, initial_pos(2), target_pos(2));
}
...
void qrFootCubicPatternGenerator::SetParameters(
  ...
  footSplinerDownZ.setBoundary(
      initial_time + params.duration / 2.0f, params.duration / 2.0f, target_appex, target_pos(2) - params.penetration);
}
...
SwingFootTrajectory::SwingFootTrajectory(
    qrSplineInfo splineInfoIn, Vec3<float> startPosIn, Vec3<float> endPosIn, float duration, float maxClearance)
    :
