...
qrWholeBodyImpulseCtrl<T>::qrWholeBodyImpulseCtrl(
    size_t dim_qdot, const std::vector<qrSingleContact<T> *> *contact_list, const std::vector<qrTask<T> *> *task_list)
    :
...
template <typename T>
void qrWholeBodyImpulseCtrl<T>::WeightedInverse(
    const DMat<T> &J, const DMat<T> &Winv, DMat<T> &Jinv, double threshold) {
