...
double qrFootStepper::CheckSolution(
  ...
  for (int i = 1; i < 5; ++i) {
    if (i <= 2) {
      CI[0][i] = front;
      b[i] = -CI[0][i] *
             (defaultFootholdDelta - (frontGap.distance + frontGap.width / 2.0 * CI[0][i]) + currentFootholdsX[i - 1]);
    } else {
      CI[0][i] = back;
      b[i] = -CI[0][i] *
             (defaultFootholdDelta - (backGap.distance + backGap.width / 2.0 * CI[0][i]) + currentFootholdsX[i - 1]);
    }
  }
  try {
...
int qrFootStepper::StepGenerator(
    Eigen::Matrix<float, 1, 4>& currentFootholdsX, Eigen::Matrix<float, 1, 4>& desiredFootholdsOffset) {
  ...
  if (gaitFlag) {
    ...
    desiredFootholdsOffset << defaultFootholdDelta / 2.0, defaultFootholdDelta, defaultFootholdDelta,
        defaultFootholdDelta / 2.0;
    gaitFlag = false;
  }
  return 0;
}
...
std::tuple<Eigen::Matrix<float, 3, 4>, Eigen::Matrix<float, 3, 4>> qrFootStepper::GetFootholdsInWorldFrame(
  ...
  if (currentFootholds(0, 3) < 2.0) {  // up
    ...
    printf(
        "[up] fourFootOnWhichStairK = %d, %d, %d, %d\n", fourFootOnWhichStairK[0], fourFootOnWhichStairK[1],
        fourFootOnWhichStairK[2], fourFootOnWhichStairK[3]);
    ...
    for (int legId : legIds) {
      ...
      if (nextFootPos[0] > stairUp.startPoint[0] - 0.1 + tmp && nextFootPos[0] < stairUp.startPoint[0] - 0.05 + tmp) {
        nextFootPos[0] = stairUp.startPoint[0] - 0.08 + tmp;
          ...
          nextFootPos[0] >= stairUp.startPoint[0] - 0.05 + tmp && nextFootPos[0] < stairUp.startPoint[0] + 0.02 + tmp) {
        nextFootPos[0] = stairUp.startPoint[0] - 0.05 + tmp;
      } else if (
          nextFootPos[0] >= stairUp.startPoint[0] + 0.02 + tmp && nextFootPos[0] < stairUp.startPoint[0] + 0.07 + tmp) {
        if (legId <= 1) {
          int legId1 = (legId + 1) % 2;  // robot->GetSameFrontBackLegId(legId);
          int legId2 = legId + 2;        // robot->GetSideLegId(legId);
          if (footOnWhichStairK <= fourFootOnWhichStairK[legId1] && footOnWhichStairK <= maxBackFootK + 1) {
            nextFootPos[0] = stairUp.startPoint[0] + 0.05 + tmp;
          ...
          int legId2 = legId - 2;                // robot->GetSideLegId(legId);
          if (footOnWhichStairK <= fourFootOnWhichStairK[legId1] && footOnWhichStairK < minFrontFootK) {
            nextFootPos[0] = stairUp.startPoint[0] + 0.05 + tmp;
            nextFootPos[2] += stairUp.height;
            dZ[legId] = stairUp.height;
          } else {
    ...
    for (int i = 0; i < stairDown.k; i++) {
      Vec2<float> stairFramePoint = {
          stairDown.startPoint[0] + i * stairUp.width, stairDown.startPoint[0] - i * stairUp.height};
      stairFrames.push_back(stairFramePoint);
      for (int legId = 0; legId < NumLeg; ++legId) {
        if (currentFootholds(0, legId) >= stairFramePoint[0]) {
          fourFootOnWhichStairK[legId]++;
    ...
    for (int legId : legIds) {
      ...
      if (nextFootPos[0] > stairDown.startPoint[0] - 0.1 + tmp &&
        ...
        if (legId <= 1) {
          int legId1 = (legId + 1) % 2;  // robot->GetSameFrontBackLegId(legId);
          int legId2 = legId + 2;        // robot->GetSideLegId(legId);
          if (footOnWhichStairK <= fourFootOnWhichStairK[legId1] && footOnWhichStairK <= maxBackFootK) {
            nextFootPos[0] = stairDown.startPoint[0] + 0.09 + tmp;
