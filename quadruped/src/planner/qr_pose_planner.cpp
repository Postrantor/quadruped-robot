...
qrPosePlanner::qrPosePlanner(
  ...
  contactLegNumber = 0;
  rBH << 0.18, 0.18, -0.18, -0.18, -0.047, 0.047, -0.047, 0.047, 0, 0, 0, 0;  // robot->GetHipPositionsInBaseFrame();
...
Vec6<float> qrPosePlanner::Update(float currentTime) {
  ...
  rSP_ = rSP_ * 2.f / 3.f +
         centerOfQuadrangle / 3.f;  // todo, reduce the value of the center of sp to avoid large base-moving amplitude.
  // rSP_[2] = std::max(-0.03f, (std::min(0.03f, bodyHight - rIBSource[2] - rIBSource[2] - Rcb(0,2)*rSP_[0] -
  // Rcb(1,2)*rSP_[1]) / Rcb(2,2) + rIBSource[2])); // Rcb^T * X = Y, where Y is in control frame, and Y[2] = bodyHeight
  rSP_[2] = bodyHight;
  ...
  // }
  // tarjectorySo3 = robotics::math::CubicSplineInSO3(sourceR, destR, currentTime, gaitGenerator->moveBaseTime,
  // robot->bodyInertia);
  return poseDest;
}
...
Eigen::MatrixXf qrPosePlanner::ComputeG() {
  ...
  if (N == 3) {
    ...
    Asp << B[1] - A[1], A[0] - B[0], 0, C[1] - B[1], B[0] - C[0], 0, D[1] - C[1], C[0] - D[0], 0, A[1] - D[1],
        D[0] - A[0], 0;

    bsp << A[0] * B[1] - B[0] * A[1], B[0] * C[1] - C[0] * B[1], C[0] * D[1] - D[0] * C[1], D[0] * A[1] - A[0] * D[1];
