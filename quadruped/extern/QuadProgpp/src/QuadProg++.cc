...
double solve_quadprog_test(
  ...
  Vector<double> s(m + p), z(n), r(m + p), d(n), np(n), x_old(n), u_old(m + p);
  u.resize(m + p);
    ...
    for (j = i; j < n; j++) {
      ...
      if (i == j) {
        if (sum <= 0.0) {
          ...
          // raise error
          print_matrix("A", A);
          os << "Error in cholesky decomposition, sum: " << sum << std::endl;
