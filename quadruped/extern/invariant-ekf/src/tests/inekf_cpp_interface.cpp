...
Vec3<double> INEKFInterface::getPosition() { return filter.getState().getPosition(); }
...
void INEKFInterface::ReceiveImuDataInSim(double T, const IMU& imu) {
  ...
  imu_measurement << imu.gyroscope[0], imu.gyroscope[1], imu.gyroscope[2], imu.accelerometer[0], imu.accelerometer[1],
      imu.accelerometer[2];
...
void INEKFInterface::ReceiveKinematicsMeasurement(const KinematicsMeasurement& kinematicsMeasurement) {
  ...
  const Eigen::Matrix<double, 3, 3> prior_kinematics_cov =
      0.05 * 0.05 * Eigen::Matrix<double, 3, 3>::Identity();  // 5 cm std Adds to FK covariance
  for (int i = 0; i < 4; ++i) {
    q[i].normalize();  // error
