...
private:
      ...
      if (messageLength < 0) {
        ...
        if (udpSocket->onMessageReceived)
          udpSocket->onMessageReceived(
              std::string(tempBuffer, messageLength), ipToString(udpSocket->address),
              ntohs(udpSocket->address.sin_port));
  ...
  static void ReceiveFrom(UDPSocket *udpSocket) {
    ...
    while (true) {
      int messageLength;
      messageLength =
          recvfrom(udpSocket->sock, tempBuffer, udpSocket->BUFFER_SIZE, 0, (sockaddr *)&hostAddr, &hostAddrSize);
