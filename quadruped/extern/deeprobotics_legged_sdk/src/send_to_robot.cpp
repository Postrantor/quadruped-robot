...
void SendToRobot::cmd_done(Command &command_temp) {
  ...
  // 复制数据并释放Command中的内容
  memcpy(command_message.data_buffer, command_temp.get_command_parameters(), command_message.command.paramters_size);
  delete[](char *) command_temp.get_command_parameters();

  message_size = sizeof(command_message.command) + command_message.command.paramters_size;
  char buffer[4096];
