...
returnValue Bounds::init(int_t _n) {
  if (_n < 0) return THROWERROR(RET_INVALID_ARGUMENTS);
...
returnValue Bounds::setupBound(int_t number, SubjectToStatus _status) {
  /* consistency check */
  if ((number < 0) || (number >= n)) return THROWERROR(RET_INDEX_OUT_OF_BOUNDS);
...
returnValue Bounds::moveFixedToFree(int_t number) {
  ...
  if (this->removeIndex(this->getFixed(), number) != SUCCESSFUL_RETURN) return THROWERROR(RET_MOVING_BOUND_FAILED);
...
returnValue Bounds::moveFreeToFixed(int_t number, SubjectToStatus _status) {
  ...
  if (this->removeIndex(this->getFree(), number) != SUCCESSFUL_RETURN) return THROWERROR(RET_MOVING_BOUND_FAILED);
...
returnValue Bounds::flipFixed(int_t number) {
  ...
  if (status != 0) switch (status[number]) {
      case ST_LOWER:
...
returnValue Bounds::swapFree(int_t number1, int_t number2) {
  /* consistency check */
  if ((number1 < 0) || (number1 >= n) || (number2 < 0) || (number2 >= n)) return THROWERROR(RET_INDEX_OUT_OF_BOUNDS);
...
returnValue Bounds::shift(int_t offset) {
  ...
  /* consistency check */
  if ((offset == 0) || (n <= 1)) return SUCCESSFUL_RETURN;

  if ((offset < 0) || (offset > n / 2)) return THROWERROR(RET_INDEX_OUT_OF_BOUNDS);

  if ((n % offset) != 0) return THROWERROR(RET_INVALID_ARGUMENTS);
  ...
  for (i = 0; i < n; ++i) {
    switch (getStatus(i)) {
      case ST_INACTIVE:
        if (shiftedFreee.addNumber(i) != SUCCESSFUL_RETURN) return THROWERROR(RET_SHIFTING_FAILED);
        break;

      case ST_LOWER:
        if (shiftedFixed.addNumber(i) != SUCCESSFUL_RETURN) return THROWERROR(RET_SHIFTING_FAILED);
        break;
...
returnValue Bounds::rotate(int_t offset) {
  ...
  /* consistency check */
  if ((offset == 0) || (offset == n) || (n <= 1)) return SUCCESSFUL_RETURN;

  if ((offset < 0) || (offset > n)) return THROWERROR(RET_INDEX_OUT_OF_BOUNDS);
  ...
  for (i = 0; i < n; ++i) {
    switch (getStatus(i)) {
      case ST_INACTIVE:
        if (rotatedFreee.addNumber(i) != SUCCESSFUL_RETURN) return THROWERROR(RET_ROTATING_FAILED);
        break;

      case ST_LOWER:
        if (rotatedFixed.addNumber(i) != SUCCESSFUL_RETURN) return THROWERROR(RET_ROTATING_FAILED);
        break;
...
returnValue Bounds::print() {
  if (n == 0) return SUCCESSFUL_RETURN;
  ...
  snprintf(
      myPrintfString, MAX_STRING_LENGTH, "Bounds object comprising %d variables (%d free, %d fixed):\n", (int)n,
      (int)nFR, (int)nFX);
...
returnValue Bounds::setupAll(SubjectToStatus _status) {
  ...
  for (i = 0; i < n; ++i) {
    if (getType(i) == ST_UNBOUNDED) {
      if (setupBound(i, _status) != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_BOUND_FAILED);
    }
  }
