...
returnValue Constraints::moveActiveToInactive(int_t number) {
  ...
  if (this->removeIndex(this->getActive(), number) != SUCCESSFUL_RETURN) return THROWERROR(RET_MOVING_BOUND_FAILED);
...
returnValue Constraints::moveInactiveToActive(int_t number, SubjectToStatus _status) {
  ...
  if (this->removeIndex(this->getInactive(), number) != SUCCESSFUL_RETURN) return THROWERROR(RET_MOVING_BOUND_FAILED);
...
returnValue Constraints::shift(int_t offset) {
  ...
  for (i = 0; i < n; ++i) {
    switch (getStatus(i)) {
      case ST_INACTIVE:
        if (shiftedInactive.addNumber(i) != SUCCESSFUL_RETURN) return THROWERROR(RET_SHIFTING_FAILED);
        break;

      case ST_LOWER:
        if (shiftedActive.addNumber(i) != SUCCESSFUL_RETURN) return THROWERROR(RET_SHIFTING_FAILED);
        break;
...
returnValue Constraints::rotate(int_t offset) {
  ...
  for (i = 0; i < n; ++i) {
    switch (getStatus(i)) {
      case ST_INACTIVE:
        if (rotatedInactive.addNumber(i) != SUCCESSFUL_RETURN) return THROWERROR(RET_ROTATING_FAILED);
        break;

      case ST_LOWER:
        if (rotatedActive.addNumber(i) != SUCCESSFUL_RETURN) return THROWERROR(RET_ROTATING_FAILED);
        break;
...
returnValue Constraints::print() {
  ...
  snprintf(
      myPrintfString, MAX_STRING_LENGTH, "Constraints object comprising %d constraints (%d inactive, %d active):\n",
...
returnValue Constraints::setupAll(SubjectToStatus _status) {
  ...
  for (i = 0; i < n; ++i) {
    if (getType(i) == ST_UNBOUNDED) {
      if (setupConstraint(i, _status) != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_CONSTRAINT_FAILED);
    }
  }
