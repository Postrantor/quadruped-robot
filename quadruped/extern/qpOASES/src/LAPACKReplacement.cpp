...
extern "C" void DPOTRF(const char* uplo, const la_uint_t* _n, double* a, const la_uint_t* _lda, la_int_t* info) {
  double sum;
  la_int_t i, j, k;
  ...
  for (i = 0; i < n; ++i) {
    ...
    for (j = (i + 1); j < n; ++j) {
      ...
      for (k = (i - 1); k >= 0; --k) sum -= a[k + lda * i] * a[k + lda * j];
...
extern "C" void SPOTRF(const char* uplo, const la_uint_t* _n, float* a, const la_uint_t* _lda, la_int_t* info) {
  float sum;
  la_int_t i, j, k;
  la_int_t n = (la_int_t)(*_n);
  la_int_t lda = (la_int_t)(*_lda);
