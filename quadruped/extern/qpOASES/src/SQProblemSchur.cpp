 ...
 *	\author Andreas Waechter and Dennis Janka, based on QProblem.cpp by Hans Joachim Ferreau, Andreas Potschka,
 *Christian Kirches \version 3.2 \date 2012-2017
 *
...
returnValue SQProblemSchur::reset() {
  ...
  if (SQProblem::reset() != SUCCESSFUL_RETURN) return THROWERROR(RET_RESET_FAILED);
 ...
 */
returnValue SQProblemSchur::setupAuxiliaryQP(
    SymmetricMatrix* H_new,
    Matrix* A_new,
    const real_t* lb_new,
    const real_t* ub_new,
  ...
  if (nC > 0) {
    if (A_new == 0) return THROWERROR(RET_INVALID_ARGUMENTS);
  ...
  if (H_new != 0) {
    ...
    if (determineHessianType() != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
    ...
    if ((hessianType == HST_ZERO) || (hessianType == HST_SEMIDEF) || (usingRegularisation() == BT_TRUE)) {
      ...
      if (regulariseHessian() != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
    }
  } else {
    if (H != 0) return THROWERROR(RET_NO_HESSIAN_SPECIFIED);
  }

  /* 3) Setup QP gradient. */
  if (setupAuxiliaryQPgradient() != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
  ...
  if (returnvalue == SUCCESSFUL_RETURN && neig == getNAC()) {
    ...
    /* b) KKT matrix has too many negative eigenvalues. Try to correct the inertia by adding bounds (reduce nullspace
     * dimension). */
    if (options.printLevel == PL_HIGH)
      MyPrintf(
          "WARNING: In hotstart for new matrices, reduced Hessian for initial working set has %i negative eigenvalues, "
          "should be %i.\n",
          neig, getNAC());
  ...
  for (i = 0; i < nV; i++) {
    ...
    if (bounds.getType(i) == ST_EQUALITY) {
      if (oldBounds.setStatus(i, ST_LOWER) != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
    } else
#endif
    {
...
/* Set all equalities active */
  ...
  for (i = 0; i < nC; ++i) {
    if (constraints.getType(i) == ST_EQUALITY)
      if (oldConstraints.setStatus(i, ST_LOWER) != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
  }
#endif
  ...
  /* adjust lb/ub */
  for (int_t ii = 0; ii < nC; ++ii) Ax_l[ii] = Ax_u[ii] = Ax[ii];
...
returnValue SQProblemSchur::setupAuxiliaryWorkingSet(
  ...
  for (i = 0; i < nV; ++i) {
    if ((bounds.getStatus(i) == ST_LOWER) && (auxiliaryBounds->getStatus(i) != ST_LOWER))
      if (bounds.moveFixedToFree(i) != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_WORKINGSET_FAILED);
  ...
  for (i = 0; i < nC; ++i) {
    if ((constraints.getStatus(i) == ST_LOWER) && (auxiliaryConstraints->getStatus(i) != ST_LOWER))
      if (constraints.moveActiveToInactive(i) != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_WORKINGSET_FAILED);
  ...
  if (resetSchurComplement(BT_FALSE) != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_WORKINGSET_FAILED);

  /* III.2.) Check if inertia is correct. If so, we now have a linearly independent working set with a pos def reduced
   * Hessian */
  int_t neig = sparseSolver->getNegativeEigenvalues();
  if (neig == getNAC()) {
  ...
  if (options.printLevel == PL_HIGH)
    MyPrintf(
        "WARNING: In setupAuxiliaryWorkingSet: Initial working set reduced Hessian has %i negative eigenvalues, should "
        "be %i.\n",
        neig, getNAC());
 ...
 */
returnValue SQProblemSchur::computeProjectedCholesky() { return SUCCESSFUL_RETURN; }

/*
 *	c o m p u t e I n i t i a l C h o l e s k y
 */
returnValue SQProblemSchur::computeInitialCholesky() { return SUCCESSFUL_RETURN; }
...
returnValue SQProblemSchur::addConstraint(
  ...
  for (int_t i = 0; i < nS; i++) {
    if (schurUpdate[i] == SUT_ConRemoved && number == schurUpdateIndex[i]) {
      if (deleteFromSchurComplement(i) != SUCCESSFUL_RETURN) return THROWERROR(RET_ADDCONSTRAINT_FAILED);
      found = true;
      idxDeleted = i;
      break;
    }
  ...
  if (!found) {
    if (nS < 0 || nS == nSmax) {
      ...
      for (int_t i = 0; i < nFRStart; i++) icolsNumber[i] = FR_idxStart[i];
      ...
      returnValue returnvalue = addToSchurComplement(
          number, SUT_ConAdded, numNonzerosM, MNpos, MNvals, numNonzerosN, MNpos + nFRStart, MNvals + nFRStart, 0.0);
      ...
      if (returnvalue != SUCCESSFUL_RETURN) return THROWERROR(RET_ADDCONSTRAINT_FAILED);

      found = true;
    }
  }

  if (!found) return THROWERROR(RET_ADDCONSTRAINT_FAILED);
...
returnValue SQProblemSchur::addConstraint_ensureLI(int_t number, SubjectToStatus C_status) {
  ...
  if (C_status != ST_LOWER) {
    for (i = 0; i < nAC; ++i) xiC[i] = -xiC[i];
    for (i = 0; i < nFX; ++i) xiB[i] = -xiB[i];
  }
...
returnValue SQProblemSchur::addBound(
  ...
  for (int_t i = 0; i < nS; i++) {
    if (schurUpdate[i] == SUT_VarFreed && number == schurUpdateIndex[i]) {
      if (deleteFromSchurComplement(i) != SUCCESSFUL_RETURN) return THROWERROR(RET_ADDBOUND_FAILED);
      found = true;
      idxDeleted = i;
      break;
    }
  ...
  if (!found) return THROWERROR(RET_ADDBOUND_FAILED);
...
returnValue SQProblemSchur::removeConstraint(
  ...
  for (int_t i = 0; i < nS; i++) {
    if (schurUpdate[i] == SUT_ConAdded && number == schurUpdateIndex[i]) {
      if (deleteFromSchurComplement(i, BT_TRUE) != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVECONSTRAINT_FAILED);
      found = true;
      idxDeleted = i;
      sModType = 2;
      break;
  ...
  if (!found) return THROWERROR(RET_REMOVECONSTRAINT_FAILED);
  ...
  if ((options.enableFlippingBounds == BT_TRUE) && (allowFlipping == BT_TRUE) && (exchangeHappened == BT_FALSE)) {
    if (sModType == 1) { /* Case 1: We added a row and column to S. */
      ...
      if (correctInertia()) return THROWERROR(RET_REMOVECONSTRAINT_FAILED);
...
returnValue SQProblemSchur::removeBound(
  ...
  for (int_t i = 0; i < nS; i++) {
    if (schurUpdate[i] == SUT_VarFixed && number == schurUpdateIndex[i]) {
      if (deleteFromSchurComplement(i, BT_TRUE) != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVEBOUND_FAILED);
      found = true;
      idxDeleted = i;
      sModType = 2;
      break;
  ...
  if (!found) {
    if (nS < 0 || nS == nSmax) {
      ...
      switch (hessianType) {
        ...
        default:
          N_diag = regularisation;
          for (int_t i = 0; i < nFRStart; i++) iNumber[i] = FR_idxStart[i];
          iLength = nFRStart;
          for (int_t i = 0; i < nS; i++)
            if (schurUpdate[i] == SUT_VarFreed) {
      ...
      for (int_t i = 0; i < nACStart; i++) iNumber[i] = AC_idxStart[i];
      iLength = nACStart;
      for (int_t i = 0; i < nS; i++)
      ...
      returnvalue =
          addToSchurComplement(number, SUT_VarFreed, numNonzerosM, Mpos, Mvals, numNonzerosN, Npos, Nvals, N_diag);
      ...
      if (returnvalue != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVEBOUND_FAILED);
  ...
  if (!found) return THROWERROR(RET_REMOVEBOUND_FAILED);
  ...
  if ((options.enableFlippingBounds == BT_TRUE) && (allowFlipping == BT_TRUE) && (exchangeHappened == BT_FALSE)) {
    if (sModType == 1) { /* Case 1: We added a row and column to S. */
      ...
      /* Check if flipping deleted the negative eigenvalue */
      if (correctInertia()) return THROWERROR(RET_REMOVEBOUND_FAILED);
    } else { /* None of the three cases happened */
      return THROWERROR(RET_REMOVEBOUND_FAILED);
    }
  } else { /* No flipping strategy, update QR factorization of S */
...
real_t SQProblemSchur::calcDetSchur(int_t idxDel) {
  ...
  if (idxDel < 0) {
    ...
    for (i = 0; i < nS - 1; i++) temp1[i] = S[i + (nS - 1) * nSmax];
    backsolveSchurQR(nS - 1, temp1, 1, temp2);

    newDet = S[(nS - 1) + (nS - 1) * nSmax];
    for (i = 0; i < nS - 1; i++) newDet -= temp1[i] * temp2[i];
    newDet *= detS;
    delete[] temp1;
    delete[] temp2;
  }
  /* Case 2: row and column idxDel have been deleted from S */
    ...
    for (j = 0; j < idxDel; j++)
      for (i = 0; i < dim; i++) tempR[i + j * dim] = R_[i + j * nSmax];
    for (j = idxDel; j < dim - 1; j++)
      for (i = 0; i < dim; i++) tempR[i + j * dim] = R_[i + (j + 1) * nSmax];
    /* Copy row idxDel of Q */
    for (j = 0; j < dim; j++) tempColQ[j] = Q_[idxDel + j * nSmax];
    ...
    for (i = idxDel; i < nS; i++) {
      ...
      for (j = i + 1; j < nS; j++)
        applyGivens(
            c, s, nu, tempR[i + j * dim], tempR[(i + 1) + j * dim], tempR[i + j * dim], tempR[(i + 1) + j * dim]);

      /* Simultaneously transform relevant column of Q**T */
      applyGivens(c, s, nu, tempColQ[i], tempColQ[i + 1], tempColQ[i], tempColQ[i + 1]);
    }

    /* Delete row: nS Givens rotations to transform last column (and row!) of (old) Q**T to getAbs((nS+1)-th unity
     * vector) */
    for (i = nS; i > 0; i--) {
      ...
      applyGivens(
          c, s, nu, tempR[nS + (i - 1) * dim], tempR[(i - 1) + (i - 1) * dim], tempR[nS + (i - 1) * dim],
          tempR[(i - 1) + (i - 1) * dim]);
    }
    ...
    if ((((nS - idxDel) % 2 == 1) && (tempColQ[nS] > 0.0)) || (((nS - idxDel) % 2 == 0) && (tempColQ[nS] < 0.0))) {
      tempR[0] = -tempR[0];
    }
...
returnValue SQProblemSchur::updateSchurQR(int_t idxDel) {
  ...
  if (idxDel < 0) {
    ...
    for (i = 0; i < nS; i++) R_[(nS - 1) + i * nSmax] = S[(nS - 1) + i * nSmax];

    /* IIb: Augment columns of R by Q**T * S[nS,:] */
    for (i = 0; i < nS; i++) {
      R_[i + (nS - 1) * nSmax] = 0.0;
      for (j = 0; j < nS; j++) R_[i + (nS - 1) * nSmax] += Q_[j + i * nSmax] * S[j + (nS - 1) * nSmax];
    }
  ...
  /* Case 2: row and column idxDel have been deleted from S */
    ...
    for (j = idxDel; j < nS; j++)
      for (i = 0; i < nS + 1; i++) R_[i + j * nSmax] = R_[i + (j + 1) * nSmax];
    ...
    for (j = 0; j < nS + 1; j++) {
      ...
      for (i = idxDel; i < nS; i++) Q_[i + j * nSmax] = Q_[(i + 1) + j * nSmax];
      Q_[nS + j * nSmax] = temp;
    }

    /* IV: Delete row: nS Givens rotations to transform last column (and row!) of (old) Q**T to getAbs((nS+1)-th unity
     * vector) */
    for (i = nS; i > 0; i--) {
    ...
    if ((((nS - idxDel) % 2 == 1) && (Q_[nS + nS * nSmax] > 0.0)) ||
      ...
      for (i = 0; i < nS + 1; i++) Q_[i] = -Q_[i];
      for (i = 0; i < nS; i++) R_[i * nSmax] = -R_[i * nSmax];
    }
  }
  ...
  if (options.printLevel == PL_HIGH) MyPrintf("1/cond(S) = %23.16e.\n", rcondS);
...
returnValue SQProblemSchur::backsolveSchurQR(int_t dimS, const real_t* const rhs, int_t dimRhs, real_t* const sol) {
  if (dimS < 1 || dimRhs < 1) return SUCCESSFUL_RETURN;
  ...
  for (i = 0; i < dimS; i++) sol[i] = 0.0;

  /* Compute sol = Q**T * rhs */
  for (i = 0; i < dimS; i++)
    for (j = 0; j < dimS; j++) sol[i] += Q_[j + i * nSmax] * rhs[j];
...
returnValue SQProblemSchur::stepCalcRhs(
  ...
  if (options.numRefinementSteps > 0) {
    for (i = 0; i < nFR; i++) rhs_max = getMax(rhs_max, getAbs(tempA[i]));
    for (i = 0; i < nAC; i++) rhs_max = getMax(rhs_max, getAbs(tempB[i]));
  }
  return SUCCESSFUL_RETURN;
}
...
returnValue SQProblemSchur::stepCalcBacksolveSchur(
    int_t nFR, int_t nFX, int_t nAC, int_t* FR_idx, int_t* FX_idx, int_t* AC_idx, int_t dim, real_t* rhs, real_t* sol) {
  returnValue retval;
  int_t i, ii;
...
returnValue SQProblemSchur::stepCalcResid(
  ...
  for (i = 0; i < nFR; ++i) tempA[i] += options.epsRegularisation * delta_xFR[i];
...
returnValue SQProblemSchur::stepCalcDeltayFx(
  ...
  if (hessianType == HST_ZERO) {
    ...
    for (i = 0; i < nFX; ++i) delta_yFX[i] += options.epsRegularisation * delta_xFX[i];
  } else if (hessianType == HST_IDENTITY) {
    for (i = 0; i < nFX; ++i) delta_yFX[i] += delta_xFX[i];
  } else {
...
returnValue SQProblemSchur::determineStepDirection2(
  ...
  /* The linear system to be solved here is this:
     ...
     | A_CF  A_CX    0     0 | | -delta_yAC | = |  delta_bA  |  <-- active entries of delta_lbA and delta_ubA with
     corresponding sign \  0     I      0     0 / \ -delta_yFX /   \  delta_bX  /  <-- fixed entries of delta_lb and
     delta_ub with corresponding sign
  ...
  */
  ...
  if (nFR + nAC > 0) {
    real_t rhs_max = 0.0;
    retval = stepCalcRhs(
        nFR, nFX, nAC, FR_idx, FX_idx, AC_idx, rhs_max, delta_g, delta_lbA, delta_ubA, delta_lb, delta_ub,
        Delta_bC_isZero, Delta_bB_isZero, delta_xFX, delta_xFR, delta_yAC, delta_yFX);

    if (retval != SUCCESSFUL_RETURN) return retval;
    int_t nFRStart = boundsFreeStart.getLength();
    int_t nACStart = constraintsActiveStart.getLength();
    ...
    for (r = 0; r <= options.numRefinementSteps; ++r) {
      retval = stepCalcReorder(
          nFR, nAC, FR_idx, AC_idx, nFRStart, nACStart, FR_idxStart, AC_idxStart, FR_iSort, FR_iSortStart, AC_iSort,
          AC_iSortStart, rhs);
      if (retval != SUCCESSFUL_RETURN) return retval;
      ...
      retval = stepCalcReorder2(
          nFR, nAC, FR_idx, AC_idx, nFRStart, nACStart, FR_idxStart, AC_idxStart, FR_iSort, FR_iSortStart, AC_iSort,
          AC_iSortStart, sol, delta_xFR, delta_yAC);
      if (retval != SUCCESSFUL_RETURN) return retval;

      if (r < options.numRefinementSteps)  // TODO: use "<" to avoid computation in last round
      {
        real_t rnrm;
        retval = stepCalcResid(
            nFR, nFX, nAC, FR_idx, FX_idx, AC_idx, Delta_bC_isZero, delta_xFX, delta_xFR, delta_yAC, delta_g, delta_lbA,
            delta_ubA, rnrm);
        if (retval != SUCCESSFUL_RETURN) return retval;
        ...
        if (rnrm <= options.epsIterRef * rhs_max) break;
      }
    }
...
returnValue SQProblemSchur::resetSchurComplement(BooleanType allowInertiaCorrection) {
  ...
  if (options.printLevel == PL_HIGH) MyPrintf("Resetting Schur complement.\n");
  ...
  if (options.epsRegularisation > 0.0) numNonzeros += nFR;
  ...
  A->getSparseSubmatrix(
      constraints.getActive(), bounds.getFree(), nFR + 1, 1, numNonzerosA, irn + numNonzeros, jcn + numNonzeros,
  ...
  if (retval != SUCCESSFUL_RETURN) return THROWERROR(RET_NO_SPARSE_SOLVER);
  ...
  if (retval != SUCCESSFUL_RETURN) return THROWERROR(RET_MATRIX_FACTORISATION_FAILED);
...
returnValue SQProblemSchur::computeMTimes(real_t alpha, const real_t* const x_, real_t beta, real_t* const y_) {
  if (isEqual(alpha, -1.0) == BT_FALSE || isEqual(beta, 1.0) == BT_FALSE) return THROWERROR(RET_NOT_YET_IMPLEMENTED);

  int_t i, j;

  for (j = 0; j < nS; j++) {
    const real_t xval = x_[j];
    for (i = M_jc[j]; i < M_jc[j + 1]; i++) y_[M_ir[i]] -= M_vals[i] * xval;
  }
...
returnValue SQProblemSchur::computeMTransTimes(real_t alpha, const real_t* const x_, real_t beta, real_t* const y_) {
  ...
  if (isZero(beta) == BT_TRUE) {
    for (j = 0; j < nS; j++) {
      y_[j] = 0.0;
      for (i = M_jc[j]; i < M_jc[j + 1]; i++) y_[j] += M_vals[i] * x_[M_ir[i]];
    }
  } else {
    for (j = 0; j < nS; j++) {
      y_[j] = -y_[j];
...
returnValue SQProblemSchur::addToSchurComplement(
  ...
  for (i = 0; i < dim; i++) rhs[i] = 0.0;

  for (i = 0; i < numNonzerosM; i++) rhs[Mpos[i]] = Mvals[i];
  ...
  for (i = 0; i < numNonzerosN; i++) new_Scol[Npos[i]] -= Nvals[i];

  real_t sdiag = -N_diag;
  for (i = 0; i < numNonzerosM; i++) sdiag += Mvals[i] * sol[Mpos[i]];

  /* Now augment S */
  for (i = 0; i < nS; i++) S[nS * nSmax + i] = new_Scol[i];
  for (i = 0; i < nS; i++) S[i * nSmax + nS] = new_Scol[i];
  S[nS * nSmax + nS] = sdiag;
...
returnValue SQProblemSchur::deleteFromSchurComplement(int_t idx, BooleanType allowUndo) {
  if (options.printLevel == PL_HIGH)
    MyPrintf(
        "deleting entry %d with idx = %d and type %d from Schur complement.", idx, schurUpdateIndex[idx],
        schurUpdate[idx]);
  ...
  if (idx != nS - 1) {
    ...
    if (allowUndo == BT_TRUE) {
      ...
      for (int_t i = 0; i < nS; i++) temp_vals[i] = S[idx * nSmax + i];
    ...
    for (int_t i = 0; i < idx; i++)
      for (int_t j = idx + 1; j < nS; j++) S[i * nSmax + j - 1] = S[i * nSmax + j];
    for (int_t i = idx + 1; i < nS; i++) {
      for (int_t j = 0; j < idx; j++) S[(i - 1) * nSmax + j] = S[i * nSmax + j];
      for (int_t j = idx + 1; j < nS; j++) S[(i - 1) * nSmax + j - 1] = S[i * nSmax + j];
    }
    for (int_t i = idx + 1; i < nS; i++) {
    ...
    for (int_t i = idx; i < nS; i++) M_jc[i] = M_jc[i + 1] - numEntries;
  ...
  if (options.printLevel == PL_HIGH) MyPrintf("  nS = %d\n", nS);
...
returnValue SQProblemSchur::undoDeleteFromSchurComplement(int_t idx) {
  if (options.printLevel == PL_HIGH)
    MyPrintf(
        "undo deletion of entry %d with idx = %d and type %d from Schur complement. nS = %i\n", idx,
        schurUpdateIndex[nS - 1], schurUpdate[nS - 1], nS + 1);
  ...
  if (idx != nS) {
    ...
    for (int_t i = 0; i < nS + 1; i++) temp_vals[i] = S[i + nS * nSmax];
    ...
    for (int_t i = idx - 1; i > -1; i--)
      for (int_t j = nS - 1; j > idx - 1; j--) S[(j + 1) + i * nSmax] = S[j + i * nSmax];
    for (int_t i = nS - 1; i > idx - 1; i--) {
      for (int_t j = idx - 1; j > -1; j--) S[j + (i + 1) * nSmax] = S[j + i * nSmax];
      for (int_t j = nS - 1; j > idx - 1; j--) S[(j + 1) + (i + 1) * nSmax] = S[j + i * nSmax];
    }
    for (int_t i = nS - 1; i > idx - 1; i--) {
      schurUpdateIndex[i + 1] = schurUpdateIndex[i];
      schurUpdate[i + 1] = schurUpdate[i];
    ...
    for (int_t i = nS; i > idx - 1; i--) M_jc[i + 1] = M_jc[i] + numEntries;
...
returnValue SQProblemSchur::correctInertia() {
  ...
  if (nS != 0 && nS != 1) return THROWERROR(RET_INERTIA_CORRECTION_FAILED);
  ...
  while (neig > getNAC() && k < nFR) {
    ...
    if (addBound_checkLI(number) == RET_LINEARLY_INDEPENDENT) {
      ...
      if (addBound(number, B_status, BT_TRUE, BT_FALSE) != SUCCESSFUL_RETURN) {
        if (options.printLevel == PL_HIGH) MyPrintf("In correctInertia: Adding bound[%i] = %i failed!\n", k, number);
      ...
      if (options.printLevel == PL_HIGH) MyPrintf("bound[%i] = %i is linearly dependent. Do not add.\n", k, number);
      k++;
      continue;
    }

    /* Case 1: Schur complement has been reset, check inertia of new factorization */
    if (nS == 0) neig = sparseSolver->getNegativeEigenvalues();
...
returnValue SQProblemSchur::repairSingularWorkingSet() {
  ...
  if (rank < 0) return RET_KKT_MATRIX_SINGULAR;

  /* Consistency check */
  if (defect <= 0) return RET_UNKNOWN_BUG;
  ...
  if (zeroPivots == 0) return RET_KKT_MATRIX_SINGULAR;
  ...
  for (k = defect - 1; k > -1; k--) {
    ...
    if (zeroPivots[k] < nFR) {
      ...
      if (bounds.moveFreeToFixed(number, B_status) != SUCCESSFUL_RETURN) return RET_ADDBOUND_FAILED;
    ...
    /* Linearly dependent row in the Jacobian: remove a constraint */
      ...
      if (constraints.moveActiveToInactive(number) != SUCCESSFUL_RETURN) return RET_REMOVECONSTRAINT_FAILED;
      ...
      if (constraints.getType(number) == ST_EQUALITY) {
        if (options.printLevel == PL_HIGH) MyPrintf("WARNING: Making this constraint no longer an equality.\n");
