...
returnValue Matrix::getSparseSubmatrix(
  ...
  return getSparseSubmatrix(
      irows->getLength(), rowsNumbers, icols->getLength(), colsNumbers, rowoffset, coloffset, numNonzeros, irn, jcn,
      avals, only_lower_triangular);
}
...
returnValue Matrix::getSparseSubmatrix(
  ...
  return getSparseSubmatrix(
      irows->getLength(), rowsNumbers, 1, &idx_icol, rowoffset, coloffset, numNonzeros, irn, jcn, avals,
...
returnValue Matrix::getSparseSubmatrix(
  ...
  return getSparseSubmatrix(
      1, &idx_row, icols->getLength(), colsNumbers, rowoffset, coloffset, numNonzeros, irn, jcn, avals,
      only_lower_triangular);
}

DenseMatrix::~DenseMatrix() {
  if (needToFreeMemory() == BT_TRUE) free();
}
...
BooleanType DenseMatrix::isDiag() const {
  ...
  if (nRows != nCols) return BT_FALSE;

  for (i = 0; i < nRows; ++i)
    for (j = 0; j < i; ++j)
      if ((getAbs(val[i * leaDim + j]) > EPS) || (getAbs(val[j * leaDim + i]) > EPS)) return BT_FALSE;

  return BT_TRUE;
}

real_t DenseMatrix::getNorm(int_t type) const { return REFER_NAMESPACE_QPOASES getNorm(val, nCols * nRows, type); }
...
returnValue DenseMatrix::getRow(int_t rNum, const Indexlist* const icols, real_t alpha, real_t* row) const {
  ...
  if (icols != 0) {
    if (isEqual(alpha, 1.0) == BT_TRUE)
      for (i = 0; i < icols->length; i++) row[i] = val[rNum * leaDim + icols->number[i]];
    else if (isEqual(alpha, -1.0) == BT_TRUE)
      for (i = 0; i < icols->length; i++) row[i] = -val[rNum * leaDim + icols->number[i]];
    else
      for (i = 0; i < icols->length; i++) row[i] = alpha * val[rNum * leaDim + icols->number[i]];
  } else {
    if (isEqual(alpha, 1.0) == BT_TRUE)
      for (i = 0; i < nCols; i++) row[i] = val[rNum * leaDim + i];
    else if (isEqual(alpha, -1.0) == BT_TRUE)
      for (i = 0; i < nCols; i++) row[i] = -val[rNum * leaDim + i];
    else
      for (i = 0; i < nCols; i++) row[i] = alpha * val[rNum * leaDim + i];
  }
  return SUCCESSFUL_RETURN;
}
...
returnValue DenseMatrix::getCol(int_t cNum, const Indexlist* const irows, real_t alpha, real_t* col) const {
  ...
  if (isEqual(alpha, 1.0) == BT_TRUE)
    for (i = 0; i < irows->length; i++) col[i] = val[irows->number[i] * leaDim + cNum];
  else if (isEqual(alpha, -1.0) == BT_TRUE)
    for (i = 0; i < irows->length; i++) col[i] = -val[irows->number[i] * leaDim + cNum];
  else
    for (i = 0; i < irows->length; i++) col[i] = alpha * val[irows->number[i] * leaDim + cNum];
...
returnValue DenseMatrix::getSparseSubmatrix(
  ...
  if (only_lower_triangular == BT_FALSE) {
    if (irn == 0) {
      if (jcn != 0 || avals != 0) return THROWERROR(RET_INVALID_ARGUMENTS);
      for (j = 0; j < irowsLength; j++) {
        irA = irowsNumber[j] * leaDim;
        for (i = 0; i < icolsLength; i++)
          if (isZero(val[irA + icolsNumber[i]]) == BT_FALSE) numNonzeros++;
      }
    } else {
      for (j = 0; j < irowsLength; j++) {
        irA = irowsNumber[j] * leaDim;
    ...
    if (irn == 0) {
      ...
      for (j = 0; j < irowsLength; j++) {
        irA = irowsNumber[j] * leaDim;
        for (i = 0; i <= j; i++)
          if (isZero(val[irA + irowsNumber[i]]) == BT_FALSE) numNonzeros++;
      }
    } else {
      for (j = 0; j < irowsLength; j++) {
        irA = irowsNumber[j] * leaDim;
...
returnValue DenseMatrix::transTimes(
    int_t xN, real_t alpha, const real_t* x, int_t xLD, real_t beta, real_t* y, int_t yLD) const {
  la_uint_t _xN = (la_uint_t)xN;
  la_uint_t _nRows = (la_uint_t)nRows;
  la_uint_t _nCols = (la_uint_t)nCols;
...
returnValue DenseMatrix::times(
  ...
  if (yCompr == BT_TRUE) {
    if (isZero(beta) == BT_TRUE)
      for (k = 0; k < xN; k++)
        for (j = 0; j < irows->length; j++) y[j + k * yLD] = 0.0;
    else if (isEqual(beta, -1.0) == BT_TRUE)
      for (k = 0; k < xN; k++)
        for (j = 0; j < irows->length; j++) y[j + k * yLD] = -y[j + k * yLD];
    else if (isEqual(beta, 1.0) == BT_FALSE)
      for (k = 0; k < xN; k++)
        for (j = 0; j < irows->length; j++) y[j + k * yLD] *= beta;
      ...
      if (isEqual(alpha, 1.0) == BT_TRUE)
        for (k = 0; k < xN; k++)
          for (j = 0; j < irows->length; j++) {
            ...
            for (i = 0; i < nCols; i++) y[iy] += val[irA + i] * x[k * xLD + i];
          }
      else if (isEqual(alpha, -1.0) == BT_TRUE)
        for (k = 0; k < xN; k++)
          for (j = 0; j < irows->length; j++) {
        ...
        for (k = 0; k < xN; k++)
          for (j = 0; j < irows->length; j++) {
            ...
            iy = row + k * yLD;
            irA = irows->number[row] * leaDim;
            for (i = 0; i < nCols; i++) y[iy] += alpha * val[irA + i] * x[k * xLD + i];
          }
    else /* icols != 0 */
      if (isEqual(alpha, 1.0) == BT_TRUE)
        for (k = 0; k < xN; k++)
  ...
  } else /* y not compressed */
  {
    if (isZero(beta) == BT_TRUE)
      for (k = 0; k < xN; k++)
        for (j = 0; j < irows->length; j++) y[irows->number[j] + k * yLD] = 0.0;
    else if (isEqual(beta, -1.0) == BT_TRUE)
      for (k = 0; k < xN; k++)
        for (j = 0; j < irows->length; j++) y[irows->number[j] + k * yLD] = -y[j + k * yLD];
    else if (isEqual(beta, 1.0) == BT_FALSE)
      for (k = 0; k < xN; k++)
        for (j = 0; j < irows->length; j++) y[irows->number[j] + k * yLD] *= beta;
...
returnValue DenseMatrix::transTimes(
  ...
  if (isZero(beta) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < icols->length; j++) y[j + k * yLD] = 0.0;
  else if (isEqual(beta, -1.0) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < icols->length; j++) y[j + k * yLD] = -y[j + k * yLD];
  else if (isEqual(beta, 1.0) == BT_FALSE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < icols->length; j++) y[j + k * yLD] *= beta;
...
returnValue DenseMatrix::addToDiag(real_t alpha) {
  int_t i;
  for (i = 0; i < nRows && i < nCols; i++) val[i * (leaDim + 1)] += alpha;
...
returnValue SymDenseMat::bilinear(
    const Indexlist* const icols, int_t xN, const real_t* x, int_t xLD, real_t* y, int_t yLD) const {
  int_t ii, jj, kk, col;
  int_t i, j, k, irA;

  for (ii = 0; ii < xN; ii++)
    for (jj = 0; jj < xN; jj++) y[ii * yLD + jj] = 0.0;

  real_t* Ax = new real_t[icols->length * xN];

  for (i = 0; i < icols->length * xN; ++i) Ax[i] = 0.0;
  ...
  for (j = 0; j < icols->length; j++) {
    ...
    for (i = 0; i < icols->length; i++) {
      ...
      for (k = 0; k < xN; k++) Ax[j + k * icols->length] += h * x[k * xLD + icols->number[i]];
    }
  }
...
SparseMatrix::SparseMatrix(int_t nr, int_t nc, int_t ld, const real_t* const v) : nRows(nr), nCols(nc), jd(0) {
  int_t i, j, nnz;
...
BooleanType SparseMatrix::isDiag() const {
  ...
  if (nCols != nRows) return BT_FALSE;

  for (j = 0; j < nCols; ++j) {
    if (jc[j + 1] > jc[j] + 1) return BT_FALSE;

    if ((jc[j + 1] == jc[j] + 1) && (ir[jc[j]] != j)) return BT_FALSE;
  }
...
returnValue SparseMatrix::getRowNorm(real_t* norm, int_t type) const {
  ...
  switch (type) {
    case 2:
      for (j = 0; j < nCols; ++j) {
        for (i = jc[j]; i < jc[j + 1]; i++) norm[ir[i]] += val[i] * val[i];
      }
...
returnValue SparseMatrix::getCol(int_t cNum, const Indexlist* const irows, real_t alpha, real_t* col) const {
  ...
  while (j < irows->length) col[irows->iSort[j++]] = 0.0;
...
returnValue SparseMatrix::getSparseSubmatrix(
  ...
  for (i = 0; i < nRows; i++) rowNumberInv[i] = -1;
  for (i = 0; i < irowsLength; i++) rowNumberInv[irowsNumber[i]] = i;
...
returnValue SparseMatrix::times(
    int_t xN, real_t alpha, const real_t* x, int_t xLD, real_t beta, real_t* y, int_t yLD) const {
  long i, j, k;

  if (isZero(beta) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < nRows; j++) y[j + k * yLD] = 0.0;
  else if (isEqual(beta, -1.0) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < nRows; j++) y[j + k * yLD] = -y[j + k * yLD];
  else if (isEqual(beta, 1.0) == BT_FALSE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < nRows; j++) y[j + k * yLD] *= beta;

  if (isEqual(alpha, 1.0) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < nCols; j++)
        for (i = jc[j]; i < jc[j + 1]; i++) y[ir[i] + k * yLD] += val[i] * x[j + k * xLD];
  else if (isEqual(alpha, -1.0) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < nCols; j++)
        for (i = jc[j]; i < jc[j + 1]; i++) y[ir[i] + k * yLD] -= val[i] * x[j + k * xLD];
    ...
    for (k = 0; k < xN; k++)
      for (j = 0; j < nCols; j++)
        for (i = jc[j]; i < jc[j + 1]; i++) y[ir[i] + k * yLD] += alpha * val[i] * x[j + k * xLD];

  return SUCCESSFUL_RETURN;
}

returnValue SparseMatrix::transTimes(
    int_t xN, real_t alpha, const real_t* x, int_t xLD, real_t beta, real_t* y, int_t yLD) const {
  long i, j, k;

  if (isZero(beta) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < nCols; j++) y[j + k * yLD] = 0.0;
  else if (isEqual(beta, -1.0) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < nCols; j++) y[j + k * yLD] = -y[j + k * yLD];
  else if (isEqual(beta, 1.0) == BT_FALSE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < nCols; j++) y[j + k * yLD] *= beta;

  if (isEqual(alpha, 1.0) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < nCols; j++)
        for (i = jc[j]; i < jc[j + 1]; i++) y[j + k * yLD] += val[i] * x[ir[i] + k * xLD];
  else if (isEqual(alpha, -1.0) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < nCols; j++)
        for (i = jc[j]; i < jc[j + 1]; i++) y[j + k * yLD] -= val[i] * x[ir[i] + k * xLD];
    ...
    for (k = 0; k < xN; k++)
      for (j = 0; j < nCols; j++)
        for (i = jc[j]; i < jc[j + 1]; i++) y[j + k * yLD] += alpha * val[i] * x[ir[i] + k * xLD];
...
returnValue SparseMatrix::times(
  ...
  if (isEqual(alpha, 0.0) == BT_TRUE) {
    if (yCompr == BT_TRUE) {
      ...
      if (isZero(beta) == BT_TRUE)
        ...
        for (k = 0; k < xN; k++)
          for (j = 0; j < irows->length; j++) y[irows->number[j] + k * yLD] = -y[irows->number[j] + k * yLD];
      else if (isEqual(beta, 1.0) == BT_FALSE)
        for (k = 0; k < xN; k++)
          for (j = 0; j < irows->length; j++) y[irows->number[j] + k * yLD] *= beta;
    }
  ...
  for (k = 0; k < xN * yfullLength; k++) ytmp[k] = 0.0;
    ...
    if (xN == 1) {
      for (l = 0; l < icols->length; l++) {
        ...
        if (isZero(xcol) == BT_FALSE) {
          j = icols->number[col];
          for (i = jc[j]; i < jc[j + 1]; i++) ytmp[ir[i]] += val[i] * xcol;
        }
      }
    } else {
      ...
      for (l = 0; l < icols->length; l++) {
        ...
        if (isZero(xmax) == BT_FALSE) {
          ...
          for (i = jc[j]; i < jc[j + 1]; i++)
            for (k = 0; k < xN; k++)
              // AW: Maybe it makes more sense to order ytmp by vectors, not vector entries, for better cache
              // peformance?
  ...
  } else /* icols == 0 */
  {
    if (xN == 1) {
      for (col = 0; col < nCols; col++) {
        xcol = x[col];
        if (isZero(xcol) == BT_FALSE)
          for (i = jc[col]; i < jc[col + 1]; i++) ytmp[ir[i]] += val[i] * xcol;
      }
    } else {
      // AW: I didn't test the case xN>1, but I hope it is working
      real_t* xcols = new real_t[xN];
      for (col = 0; col < nCols; col++) {
        ...
        if (isZero(xmax) == BT_FALSE)
          for (i = jc[col]; i < jc[col + 1]; i++)
            for (k = 0; k < xN; k++) ytmp[k * yfullLength + ir[i]] += val[i] * xcols[k];
        delete[] xcols;
      }
    }
  }
  ...
  if (yCompr == BT_TRUE) {
    if (isZero(beta) == BT_TRUE)
      for (k = 0; k < xN; k++)
        for (j = 0; j < irows->length; j++) y[j + k * yLD] = alpha * ytmp[irows->number[j] + k * yfullLength];
    else if (isEqual(beta, 1.0) == BT_TRUE)
      for (k = 0; k < xN; k++)
        for (j = 0; j < irows->length; j++) y[j + k * yLD] += alpha * ytmp[irows->number[j] + k * yfullLength];
    else if (isEqual(beta, -1.0) == BT_TRUE)
...
returnValue SparseMatrix::transTimes(
  ...
  if (isZero(beta) == BT_TRUE)
    ...
    for (k = 0; k < xN; k++)
      for (j = 0; j < icols->length; j++) y[j + k * yLD] *= beta;
  if (isEqual(alpha, 0.0) == BT_TRUE) return SUCCESSFUL_RETURN;

  // work with full, unordered copy of x
  const int_t xfullLength = nRows;
  real_t* xtmp = new real_t[xfullLength];
  for (k = 0; k < xN; k++) {
    for (i = 0; i < xfullLength; i++) xtmp[i] = 0.0;
    for (i = 0; i < irows->length; i++) xtmp[irows->number[i]] = x[k * xLD + i];
    for (l = 0; l < icols->length; l++) {
      col = icols->iSort[l];
      yadd = 0.0;
      j = icols->number[col];
      for (i = jc[j]; i < jc[j + 1]; i++) yadd += val[i] * xtmp[ir[i]];
      y[col] += alpha * yadd;
    }
...
returnValue SparseMatrix::addToDiag(real_t alpha) {
  ...
  if (jd == 0) return THROWERROR(RET_DIAGONAL_NOT_INITIALISED);
...
real_t* SparseMatrix::full() const {
  ...
  for (i = 0; i < nCols * nRows; i++) v[i] = 0.0;

  for (j = 0; j < nCols; j++)
    for (i = jc[j]; i < jc[j + 1]; i++) v[ir[i] * nCols + j] = val[i];
...
BooleanType SparseMatrixRow::isDiag() const {
  ...
  for (i = 0; i < nRows; ++i) {
    if (jr[i + 1] > jr[i] + 1) return BT_FALSE;

    if ((jr[i + 1] == jr[i] + 1) && (ic[jr[i]] != i)) return BT_FALSE;
  }
...
returnValue SparseMatrixRow::getRow(int_t rNum, const Indexlist* const icols, real_t alpha, real_t* row) const {
  ...
  if (icols != 0) {
    ...
    /* fill in remaining zeros */
    while (i < icols->length) row[icols->iSort[i++]] = 0.0;
  } else {
    for (i = 0; i < nCols; i++) row[i] = 0;

    if (isEqual(alpha, 1.0) == BT_TRUE)
      for (j = jr[rNum]; j < jr[rNum + 1]; j++) row[ic[j]] = val[j];
    else if (isEqual(alpha, -1.0) == BT_TRUE)
      for (j = jr[rNum]; j < jr[rNum + 1]; j++) row[ic[j]] = -val[j];
    else
      for (j = jr[rNum]; j < jr[rNum + 1]; j++) row[ic[j]] = alpha * val[j];
  }
...
returnValue SparseMatrixRow::times(
    int_t xN, real_t alpha, const real_t* x, int_t xLD, real_t beta, real_t* y, int_t yLD) const {
  long i, j, k;
  ...
  if (isEqual(alpha, 1.0) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < nRows; j++)
        for (i = jr[j]; i < jr[j + 1]; i++) y[j + k * yLD] += val[i] * x[ic[i] + k * xLD];
  else if (isEqual(alpha, -1.0) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (j = 0; j < nRows; j++)
        for (i = jr[j]; i < jr[j + 1]; i++) y[j + k * yLD] -= val[i] * x[ic[i] + k * xLD];
    ...
    for (k = 0; k < xN; k++)
      for (j = 0; j < nRows; j++)
        for (i = jr[j]; i < jr[j + 1]; i++) y[j + k * yLD] += alpha * val[i] * x[ic[i] + k * xLD];

  return SUCCESSFUL_RETURN;
}

returnValue SparseMatrixRow::transTimes(
    int_t xN, real_t alpha, const real_t* x, int_t xLD, real_t beta, real_t* y, int_t yLD) const {
  long i, j, k;
  ...
  if (isEqual(alpha, 1.0) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (i = 0; i < nRows; i++)
        for (j = jr[i]; j < jr[i + 1]; j++) y[ic[j] + k * yLD] += val[j] * x[i + k * xLD];
  else if (isEqual(alpha, -1.0) == BT_TRUE)
    for (k = 0; k < xN; k++)
      for (i = 0; i < nRows; i++)
        for (j = jr[i]; j < jr[i + 1]; j++) y[ic[j] + k * yLD] -= val[j] * x[i + k * xLD];
    ...
    for (k = 0; k < xN; k++)
      for (i = 0; i < nRows; i++)
        for (j = jr[i]; j < jr[i + 1]; j++) y[ic[j] + k * yLD] += alpha * val[j] * x[i + k * xLD];
      ...
      if (isEqual(alpha, 1.0) == BT_TRUE)
        for (l = 0; l < irows->length; l++) {
          ...
          for (j = jr[row]; j < jr[row + 1]; j++)
            for (k = 0; k < xN; k++) y[k * yLD + srt] += val[j] * x[k * xLD + ic[j]];
        }
      else if (isEqual(alpha, -1.0) == BT_TRUE)
        for (l = 0; l < irows->length; l++) {
          srt = irows->iSort[l];
        ...
        for (l = 0; l < irows->length; l++) {
          ...
          row = irows->number[srt];
          for (j = jr[row]; j < jr[row + 1]; j++)
            for (k = 0; k < xN; k++) y[k * yLD + srt] += alpha * val[j] * x[k * xLD + ic[j]];
        }
    else /* icols != 0 */
      if (isEqual(alpha, 1.0) == BT_TRUE)
        for (l = 0; l < irows->length; l++) {
          ...
          while (j < jr[row + 1] && i < icols->length)
            if (ic[j] == icols->number[icols->iSort[i]]) {
              for (k = 0; k < xN; k++) y[k * yLD + srt] += val[j] * x[k * xLD + icols->iSort[i]];
              j++, i++;
            } else if (ic[j] > icols->number[icols->iSort[i]])
              i++;
            else
        ...
        for (l = 0; l < irows->length; l++) {
          ...
          while (j < jr[row + 1] && i < icols->length)
            if (ic[j] == icols->number[icols->iSort[i]]) {
              for (k = 0; k < xN; k++) y[k * yLD + srt] += alpha * val[j] * x[k * xLD + icols->iSort[i]];
              j++, i++;
            } else if (ic[j] > icols->number[icols->iSort[i]])
              i++;
            else
    ...
    if (icols == 0)
      if (isEqual(alpha, 1.0) == BT_TRUE)
        for (l = 0; l < irows->length; l++) {
          ...
          for (j = jr[row]; j < jr[row + 1]; j++)
            for (k = 0; k < xN; k++) y[k * yLD + row] += val[j] * x[k * xLD + ic[j]];
        }
      else if (isEqual(alpha, -1.0) == BT_TRUE)
        for (l = 0; l < irows->length; l++) {
          row = irows->number[irows->iSort[l]];
        ...
        for (l = 0; l < irows->length; l++) {
          row = irows->number[irows->iSort[l]];
          for (j = jr[row]; j < jr[row + 1]; j++)
            for (k = 0; k < xN; k++) y[k * yLD + row] += alpha * val[j] * x[k * xLD + ic[j]];
        }
    else /* icols != 0 */
      if (isEqual(alpha, 1.0) == BT_TRUE)
        for (l = 0; l < irows->length; l++) {
          ...
          while (j < jr[irows->number[row] + 1] && i < icols->length)
            if (ic[j] == icols->number[icols->iSort[i]]) {
              for (k = 0; k < xN; k++) y[k * yLD + row] += val[j] * x[k * xLD + icols->iSort[i]];
              j++, i++;
            } else if (ic[j] > icols->number[icols->iSort[i]])
              i++;
            else
        ...
        for (l = 0; l < irows->length; l++) {
          ...
          while (j < jr[irows->number[row] + 1] && i < icols->length)
            if (ic[j] == icols->number[icols->iSort[i]]) {
              for (k = 0; k < xN; k++) y[k * yLD + row] += alpha * val[j] * x[k * xLD + icols->iSort[i]];
              j++, i++;
            } else if (ic[j] > icols->number[icols->iSort[i]])
              i++;
            else
...
returnValue SparseMatrixRow::transTimes(
  ...
  if (isEqual(alpha, 1.0) == BT_TRUE)
    for (l = 0; l < irows->length; l++) {
      ...
      while (j < jr[row + 1] && i < icols->length)
        if (ic[j] == icols->number[icols->iSort[i]]) {
          for (k = 0; k < xN; k++) y[k * yLD + icols->iSort[i]] += val[j] * x[k * xLD + srt];
          j++, i++;
        } else if (ic[j] > icols->number[icols->iSort[i]])
          i++;
        else
    ...
    for (l = 0; l < irows->length; l++) {
      ...
      while (j < jr[row + 1] && i < icols->length)
        if (ic[j] == icols->number[icols->iSort[i]]) {
          for (k = 0; k < xN; k++) y[k * yLD + icols->iSort[i]] += alpha * val[j] * x[k * xLD + srt];
          j++, i++;
        } else if (ic[j] > icols->number[icols->iSort[i]])
          i++;
        else
...
real_t* SparseMatrixRow::full() const {
  ...
  for (i = 0; i < nRows; i++)
    for (j = jr[i]; j < jr[i + 1]; j++) v[ic[j] + i * nCols] = val[j];
...
returnValue SymSparseMat::bilinear(
    const Indexlist* const icols, int_t xN, const real_t* x, int_t xLD, real_t* y, int_t yLD) const {
    ...
    while (idx < jc[col + 1] && k < icols->length) {
      ...
      if (ir[idx] == row) {
        ...
        if (row == col) /* diagonal element */
          for (i = 0; i < xN; i++)
            for (j = i; j < xN; j++) y[i * yLD + j] += val[idx] * x[i * xLD + col] * x[j * xLD + col];
  ...
  /* fill upper triangle */
  for (i = 0; i < xN; i++)
    for (j = i; j < xN; j++) y[j * yLD + i] = y[i * yLD + j];
