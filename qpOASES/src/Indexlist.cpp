...
returnValue Indexlist::init(int_t n) {
  if (n < 0) return THROWERROR(RET_INVALID_ARGUMENTS);
...
returnValue Indexlist::getNumberArray(int_t** const numberarray) const {
  if (numberarray == 0) return THROWERROR(RET_INVALID_ARGUMENTS);
...
returnValue Indexlist::addNumber(int_t addnumber) {
  if (length >= physicallength) return THROWERROR(RET_INDEXLIST_EXCEEDS_MAX_LENGTH);

  int_t i, j;
  number[length] = addnumber;
  j = findInsert(addnumber);
  for (i = length; i > j + 1; i--) iSort[i] = iSort[i - 1];
  iSort[j + 1] = length;
  ++length;
...
returnValue Indexlist::removeNumber(int_t removenumber) {
  ...
  if (number[iSidx] != removenumber) return SUCCESSFUL_RETURN;

  /* update sorted indices iSort first */
  for (i = 0; i < length; i++)
    if (iSort[i] > iSidx) iSort[i]--;
  for (i = idx + 1; i < length; i++) iSort[i - 1] = iSort[i];

  /* remove from numbers list */
  for (i = iSidx; i < length - 1; ++i) number[i] = number[i + 1];
  number[length - 1] = -1;
...
returnValue Indexlist::copy(const Indexlist& rhs) {
  ...
  if (rhs.number != 0) {
    ...
    for (i = 0; i < physicallength; ++i) number[i] = rhs.number[i];
    iSort = new int_t[physicallength];
    for (i = 0; i < physicallength; ++i) iSort[i] = rhs.iSort[i];
  } else {
    number = 0;
    iSort = 0;
  }
