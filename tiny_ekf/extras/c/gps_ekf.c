...
static void init(ekf_t* ekf) {
  ...
  const double Qxyz[4] = {
      sigma * sigma * T * T * T / 3, sigma * sigma * T * T / 2, sigma * sigma * T * T / 2, sigma * sigma * T};
  ...
  for (i = 0; i < 8; ++i) ekf->P[i][i] = P0;

  for (i = 0; i < 4; ++i) ekf->R[i][i] = R0;
...
static void model(ekf_t* ekf, double SV[4][3]) {
  ...
  for (j = 0; j < 8; ++j) ekf->F[j][j] = 1;

  for (j = 0; j < 4; ++j) ekf->F[2 * j][2 * j + 1] = T;
  ...
  for (i = 0; i < 4; ++i) {
    for (j = 0; j < 3; ++j) ekf->H[i][j * 2] = dx[i][j] / ekf->hx[i];
    ekf->H[i][6] = 1;
  }
}

static void readline(char* line, FILE* fp) { fgets(line, 1000, fp); }
...
int main(int argc, char** argv) {
  ...
  for (j = 0; j < 25; ++j) {
    ...
    for (k = 0; k < 3; ++k) Pos_KF[j][k] = ekf.x[2 * k];
  }

  // Compute means of filtered positions
  double mean_Pos_KF[3] = {0, 0, 0};
  for (j = 0; j < 25; ++j)
    for (k = 0; k < 3; ++k) mean_Pos_KF[k] += Pos_KF[j][k];
  for (k = 0; k < 3; ++k) mean_Pos_KF[k] /= 25;
