...
bool startControllers(
    ros::NodeHandle &n, std::string serviceName, std::vector<std::string> &controllersToStart) {
  ros::ServiceClient switchController =
      n.serviceClient<controller_manager_msgs::SwitchController>(serviceName);
...
bool stopControllers(
    ros::NodeHandle &n, std::string serviceName, std::vector<std::string> &controllers_to_stop) {
  ros::ServiceClient switchController =
      n.serviceClient<controller_manager_msgs::SwitchController>(serviceName);
  controller_manager_msgs::SwitchController switchControllerMsg;
...
bool ResetRobot() {
  ...
  int statu0 = system(
      "rosrun gazebo_ros spawn_model -urdf -z 0.15 -model aliengo_gazebo -param robot_description "
      "-unpause");
  ROS_INFO("spawn model statu: %d", statu0);
  int statu1 = system(
      "rosrun controller_manager spawner __ns:=/aliengo_gazebo joint_state_controller "
...
int main(int argc, char **argv) {
  ...
  std::vector<std::string> controllerList = {
      "joint_state_controller", "FL_hip_controller",   "FL_thigh_controller", "FL_calf_controller",
      "FR_hip_controller",      "FR_thigh_controller", "FR_calf_controller",  "RL_hip_controller",
      "RL_thigh_controller",    "RL_calf_controller",  "RR_hip_controller",   "RR_thigh_controller",
      "RR_calf_controller"};
  ...
  while (ros::ok() && currentTime - startTime < MAX_TIME_SECONDS) {
    ...
    if (switchMode != 2 && quadruped->controlParams["mode"] != switchMode) {
      ROS_INFO_STREAM(
          "switch mode from " << quadruped->controlParams["mode"] << " to " << switchMode);
      SwitchMode<A1Sim>(
          quadruped, locomotionController, desiredSpeed, desiredTwistingSpeed, switchMode,
          startTimeWall);
    }
  ...
  int value = system(
      "ps -ef | grep controller_manager| awk '{print $2}'| xargs -i kill -9 {} > /dev/null 2>&1");
  ros::shutdown();
  return 0;
}
