...
bool ResetRobot(ros::ServiceClient &modelStateClient, ros::ServiceClient &jointStateClient) {
  ...
  setjointstate.request.joint_names = {"FR_HipX", "FR_HipY", "FR_Knee", "FL_HipX",
                                       "FL_HipY", "FL_Knee", "HR_HipX", "HR_HipY",
                                       "HR_Knee", "HL_HipX", "HL_HipY", "HL_Knee"};
  double hip_angle = 0.167136;
  double thigh_angle = -0.934969;
  double calf_angle = 2.54468;

  setjointstate.request.joint_positions = {-hip_angle,  thigh_angle, calf_angle,  hip_angle,
                                           thigh_angle, calf_angle,  -hip_angle,  thigh_angle,
                                           calf_angle,  hip_angle,   thigh_angle, calf_angle};
...
void GetComPositionInWorldFrame(qrRobot *quadruped, ros::ServiceClient &baseStateClient) {
  ...
  quadruped->gazeboFootPositionInWorldFrame = quadruped->GetFootPositionsInWorldFrame(
      true, posIn.cast<float>(), OrientationIn.cast<float>());
...
int main(int argc, char **argv) {
  ...
  // qrRobot *quadruped = new qrRobotLite3Sim(nh, privateNh, homeDir +
  // "config/lite3_sim/robot.yaml");
  ...
  qrController2GazeboMsg *controller2gazeboMsg =
      new qrController2GazeboMsg(quadruped, locomotionController, nh);
  ...
  while (ros::ok() && currentTime - startTime < MAX_TIME_SECONDS) {
    ...
    //     stateEstimators->GetRobotEstimator()->GetEstimatedVelocity();  // base frame
    // }
