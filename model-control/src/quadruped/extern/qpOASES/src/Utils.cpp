...
returnValue myPrintf(const char* s) {
  ...
  if (s == 0) return RET_INVALID_ARGUMENTS;
...
returnValue printCopyrightNotice() {
  ...
  myPrintf(
      "\nqpOASES -- An Implementation of the Online Active Set Strategy.\nCopyright (C) 2007-2017 by Hans Joachim "
      "Ferreau, Andreas Potschka,\nChristian Kirches et al. All rights reserved.\n\nqpOASES is distributed under the "
...
returnValue readFromFile(real_t* data, int_t nrow, int_t ncol, const char* datafilename) {
  ...
  if ((datafile = fopen(datafilename, "r")) == 0) {
    ...
    return getGlobalMessageHandler()->throwError(
        RET_UNABLE_TO_OPEN_FILE, errstr, __FUNC__, __FILE__, __LINE__, VS_VISIBLE);
  }
  ...
  for (i = 0; i < nrow; ++i) {
    for (j = 0; j < ncol; ++j) {
...
#endif /* __USE_SINGLE_PRECISION__ */
      {
        ...
        snprintf(errstr, MAX_STRING_LENGTH, "(%s)", datafilename);
        return getGlobalMessageHandler()->throwError(
            RET_UNABLE_TO_READ_FILE, errstr, __FUNC__, __FILE__, __LINE__, VS_VISIBLE);
      }
...
returnValue writeIntoFile(
    const real_t* const data, int_t nrow, int_t ncol, const char* datafilename, BooleanType append) {
#ifndef __SUPPRESSANYOUTPUT__
...
returnValue writeIntoMatFile(
    FILE* const matFile, const real_t* const data, int_t nRows, int_t nCols, const char* name) {
  /*  Note, this code snippet has been inspired from the document
   *  "Matlab(R) MAT-file Format, R2013b" by MathWorks */

#ifndef __SUPPRESSANYOUTPUT__

  if ((matFile == 0) || (data == 0) || (nRows < 0) || (nCols < 0) || (name == 0)) return RET_INVALID_ARGUMENTS;
  ...
  if (fwrite(&var, sizeof(MatMatrixHeader), 1, matFile) < 1) return RET_UNABLE_TO_WRITE_FILE;

  if (fwrite(name, sizeof(char), (unsigned long)(var.nCharName), matFile) < 1) return RET_UNABLE_TO_WRITE_FILE;
  ...
  for (ii = 0; ii < nCols; ++ii)
    for (jj = 0; jj < nRows; ++jj) {
      ...
      if (fwrite(&curData, sizeof(double), 1, matFile) < 1) return RET_UNABLE_TO_WRITE_FILE;
    }
...
returnValue writeIntoMatFile(FILE* const matFile, const int_t* const data, int_t nRows, int_t nCols, const char* name) {
  ...
  for (ii = 0; ii < nRows; ++ii)
    for (jj = 0; jj < nCols; ++jj) realData[ii * nCols + jj] = (real_t)data[ii * nCols + jj];
...
returnValue getKktViolation(
  ...
  for (i = 0; i < nV; i++) {
    ...
    if (H != 0)
      ...
      if (hasIdentityHessian == BT_TRUE) sum += x[i];
    }
  ...
  for (i = 0; i < nV; i++) {
    if (lb != 0)
      if (lb[i] - x[i] > feas) feas = lb[i] - x[i];

    if (ub != 0)
      if (x[i] - ub[i] > feas) feas = x[i] - ub[i];
  }
  ...
  if (workingSetB == 0) {
    for (i = 0; i < nV; i++) {
      ...
      /* lower bound */
      if (lb != 0)
        if (y[i] > dualActiveTolerance) prod = (x[i] - lb[i]) * y[i];

      /* upper bound */
      if (ub != 0)
        if (y[i] < -dualActiveTolerance) prod = (x[i] - ub[i]) * y[i];
    ...
    for (i = 0; i < nV; i++) {
      ...
      /* lower bound */
      if (lb != 0) {
        if (isEqual(workingSetB[i], -1.0) == BT_TRUE) prod = (x[i] - lb[i]) * y[i];
      }

      /* upper bound */
      if (ub != 0) {
        if (isEqual(workingSetB[i], 1.0) == BT_TRUE) prod = (x[i] - ub[i]) * y[i];
      }
  ...
  for (i = 0; i < nC; i++) {
    ...
    sum = 0.0;
    if (A != 0)
      for (j = 0; j < nV; j++) sum += A[i * nV + j] * x[j];

    /* feasibility */
    if (lbA != 0)
      if (lbA[i] - sum > feas) feas = lbA[i] - sum;

    if (ubA != 0)
      if (sum - ubA[i] > feas) feas = sum - ubA[i];
    ...
    if (lbA != 0) {
      if (workingSetC == 0) {
        if (y[nV + i] > dualActiveTolerance) prod = (sum - lbA[i]) * y[nV + i];
      } else {
        if (isEqual(workingSetC[i], -1.0) == BT_TRUE) prod = (sum - lbA[i]) * y[nV + i];
      }
    }
    ...
    if (ubA != 0) {
      if (workingSetC == 0) {
        if (y[nV + i] < -dualActiveTolerance) prod = (sum - ubA[i]) * y[nV + i];
      } else {
        if (isEqual(workingSetC[i], 1.0) == BT_TRUE) prod = (sum - ubA[i]) * y[nV + i];
      }
    }
...
returnValue getKktViolation(
    ...
    const real_t* const workingSetB,
    BooleanType hasIdentityHessian) {
  return getKktViolation(nV, 0, H, g, 0, lb, ub, 0, 0, x, y, stat, feas, cmpl, workingSetB, 0, hasIdentityHessian);
}
...
returnValue normaliseConstraints(int_t nV, int_t nC, real_t* A, real_t* lbA, real_t* ubA, int_t type) {
  ...
  if ((nV <= 0) || (nC <= 0) || (A == 0)) return THROWERROR(RET_INVALID_ARGUMENTS);
  ...
  for (ii = 0; ii < nC; ++ii) {
    ...
    if (curNorm > EPS) {
      ...
      for (jj = 0; jj < nV; ++jj) A[ii * nV + jj] /= curNorm;
      ...
      if (type == 1) {
        for (jj = 0; jj < nV; ++jj) A[ii * nV + jj] = 1.0 / ((real_t)nV);
      } else {
        /* assume type == 2 */
        for (jj = 0; jj < nV; ++jj) A[ii * nV + jj] = 1.0 / getSqrt((real_t)nV);
      }
...
extern "C" void gdb_printmat(const char* fname, real_t* M, int_t n, int_t m, int_t ldim) {
  ...
  for (i = 0; i < n; i++) {
    for (j = 0; j < m; j++) fprintf(fid, " %23.16e", M[j * ldim + i]);
    fprintf(fid, "\n");
  }
  fclose(fid);
