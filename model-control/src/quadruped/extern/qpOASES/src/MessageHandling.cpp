 ...
 */
MessageHandling::MessageHandling(
    VisibilityStatus _errorVisibility, VisibilityStatus _warningVisibility, VisibilityStatus _infoVisibility) {
...
MessageHandling::~MessageHandling() {
#ifndef __SUPPRESSANYOUTPUT__
  if ((outputFile != 0) && (outputFile != stdout) && (outputFile != stderr)) fclose(outputFile);
#endif /* __SUPPRESSANYOUTPUT__ */
}
...
returnValue MessageHandling::throwError(
  ...
  /* consistency check */
  if (Enumber <= SUCCESSFUL_RETURN) return throwError(RET_ERROR_UNDEFINED, 0, __FUNC__, __FILE__, __LINE__, VS_VISIBLE);
...
returnValue MessageHandling::throwInfo(
    ...
    VisibilityStatus localVisibilityStatus) {
  /* consistency check */
  if (Inumber < SUCCESSFUL_RETURN) return throwError(RET_INFO_UNDEFINED, 0, __FUNC__, __FILE__, __LINE__, VS_VISIBLE);
...
returnValue MessageHandling::throwMessage(
  ...
  if (numberOfWhitespaces >= (int_t)MAX_STRING_LENGTH) numberOfWhitespaces = (int_t)MAX_STRING_LENGTH - 1;
...
#ifdef __DEBUG__
      snprintf(
          myPrintfString, MAX_STRING_LENGTH, "%s (%s, %s:%d): \t%s\n", RETstring, functionname, filename,
