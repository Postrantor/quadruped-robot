...
returnValue QProblem::reset() {
  ...
  if (nV == 0) return THROWERROR(RET_QPOBJECT_NOT_SETUP);
  ...
  if (T != 0)
    for (i = 0; i < sizeT * sizeT; ++i) T[i] = 0.0;

  if (Q != 0)
    for (i = 0; i < nV * nV; ++i) Q[i] = 0.0;
...
returnValue QProblem::init(
  ...
  if (guessedBounds != 0) {
    for (i = 0; i < nV; ++i) {
      if (guessedBounds->getStatus(i) == ST_UNDEFINED) return THROWERROR(RET_INVALID_ARGUMENTS);
    }
  }

  if (guessedConstraints != 0) {
    for (i = 0; i < nC; ++i)
      if (guessedConstraints->getStatus(i) == ST_UNDEFINED) return THROWERROR(RET_INVALID_ARGUMENTS);
  }
  ...
  /* 2) Setup QP data. */
  if (setupQPdata(_H, _g, _A, _lb, _ub, _lbA, _ubA) != SUCCESSFUL_RETURN) return THROWERROR(RET_INVALID_ARGUMENTS);
...
returnValue QProblem::init(
  ...
  if (R_file == 0) {
    ...
    if (returnvalue != SUCCESSFUL_RETURN) return THROWWARNING(returnvalue);
...
returnValue QProblem::hotstart(
  ...
  if (haveCholesky == BT_FALSE) {
    ...
    if (returnvalue != SUCCESSFUL_RETURN) return THROWERROR(returnvalue);
  }
  ...
  if (options.enableFarBounds == BT_FALSE) {
    ...
    returnvalue = solveRegularisedQP(g_new, lb_new, ub_new, lbA_new, ubA_new, nWSR, cputime, 0, isFirstCall);
  } else {
    ...
    updateFarBounds(
        farbound, nV + nC, lb_new, lb_new_far, ub_new, ub_new_far, lbA_new, lbA_new_far, ubA_new, ubA_new_far);
    ...
    for (;;) {
      ...
      returnvalue = solveRegularisedQP(
          g_new, lb_new_far, ub_new_far, lbA_new_far, ubA_new_far, nWSR, pcputime_rem, nWSR_performed, isFirstCall);
      ...
      if (infeasible == BT_TRUE) {
        ...
        for (i = 0; i < nV; ++i) {
          if (((lb_new == 0) || (lb_new_far[i] > lb_new[i])) && (getAbs(lb_new_far[i] - x[i]) < tol)) ++nActiveFar;
          if (((ub_new == 0) || (ub_new_far[i] < ub_new[i])) && (getAbs(ub_new_far[i] - x[i]) < tol)) ++nActiveFar;
        }
        for (i = 0; i < nC; ++i) {
          if (((lbA_new == 0) || (lbA_new_far[i] > lbA_new[i])) && (getAbs(lbA_new_far[i] - Ax[i]) < tol)) ++nActiveFar;
          if (((ubA_new == 0) || (ubA_new_far[i] < ubA_new[i])) && (getAbs(ubA_new_far[i] - Ax[i]) < tol)) ++nActiveFar;
        }

        if (nActiveFar == 0) break;
  ...
  farewell:
    ...
    if (cputime != 0) *cputime = cputime_needed + auxTime;
    delete[] lbA_new_far;
    delete[] ubA_new_far;
    delete[] lb_new_far;
    delete[] ub_new_far;
...
returnValue QProblem::hotstart(
  ...
  /* consistency check */
  if (g_file == 0) return THROWERROR(RET_INVALID_ARGUMENTS);
  ...
  returnvalue =
      loadQPvectorsFromFile(g_file, lb_file, ub_file, lbA_file, ubA_file, g_new, lb_new, ub_new, lbA_new, ubA_new);
  if (returnvalue != SUCCESSFUL_RETURN) {
    if (ubA_file != 0) delete[] ubA_new;
    if (lbA_file != 0) delete[] lbA_new;
    if (ub_file != 0) delete[] ub_new;
    if (lb_file != 0) delete[] lb_new;
    delete[] g_new;
  ...
  returnvalue = hotstart(g_new, lb_new, ub_new, lbA_new, ubA_new, nWSR, cputime, guessedBounds, guessedConstraints);
 ...
 */
returnValue QProblem::solveCurrentEQP(
    const int_t n_rhs,
    const real_t* g_in,
    const real_t* lb_in,
    const real_t* ub_in,
    ...
    real_t* x_out,
    real_t* y_out) {
  if ((x_out == 0) || (y_out == 0)) return THROWERROR(RET_INVALID_ARGUMENTS);
  ...
  for (ii = 0; ii < (nV + nC) * n_rhs; ++ii) y_out[ii] = 0.0;

  for (ii = 0; ii < n_rhs; ++ii) {
    returnvalue = determineStepDirection(
        g_in, lbA_in, ubA_in, lb_in, ub_in, BT_FALSE, BT_FALSE, delta_xFX, delta_xFR, delta_yAC, delta_yFX);

    for (jj = 0; jj < nFX; ++jj) x_out[FX_idx[jj]] = delta_xFX[jj];
    for (jj = 0; jj < nFR; ++jj) x_out[FR_idx[jj]] = delta_xFR[jj];
    for (jj = 0; jj < nFX; ++jj) y_out[FX_idx[jj]] = delta_yFX[jj];
    for (jj = 0; jj < nAC; ++jj) y_out[nV + AC_idx[jj]] = delta_yAC[jj];
...
returnValue QProblem::getWorkingSet(real_t* workingSet) {
  ...
  if (workingSet == 0) return THROWERROR(RET_INVALID_ARGUMENTS);
 ...
 */
returnValue QProblem::getWorkingSetBounds(real_t* workingSetB) { return QProblemB::getWorkingSetBounds(workingSetB); }
...
returnValue QProblem::getWorkingSetConstraints(real_t* workingSetC) {
  ...
  if (workingSetC == 0) return THROWERROR(RET_INVALID_ARGUMENTS);
...
returnValue QProblem::printProperties() {
  ...
  if (options.printLevel == PL_NONE) return SUCCESSFUL_RETURN;
...
returnValue QProblem::getFreeVariablesFlags(BooleanType* varIsFree) {
  int_t nV = getNV();
  for (int_t i = 0; i < nV; i++) varIsFree[i] = BT_FALSE;

  int_t nFR = getNFR();
  int_t* FR_idx;
  bounds.getFree()->getNumberArray(&FR_idx);

  for (int_t i = 0; i < nFR; i++) varIsFree[FR_idx[i]] = BT_TRUE;
...
returnValue QProblem::solveInitialQP(
  ...
  if (determineHessianType() != SUCCESSFUL_RETURN) return THROWERROR(RET_INIT_FAILED);
  ...
  if (obtainAuxiliaryWorkingSet(
          xOpt, yOpt, guessedBounds, guessedConstraints, &auxiliaryBounds, &auxiliaryConstraints) != SUCCESSFUL_RETURN)
  ...
  if ((hessianType == HST_ZERO) || (hessianType == HST_SEMIDEF)) {
    if (regulariseHessian() != SUCCESSFUL_RETURN) return THROWERROR(RET_INIT_FAILED_REGULARISATION);
  }
  ...
  if (_R != 0) {
    if (options.initialStatusBounds != ST_INACTIVE) {
      ...
      if (_R == R) {
        ...
        for (i = 0; i < nV; ++i)
          for (j = i; j < nV; ++j) RR(i, j) = _R[i * nV + j];
        haveCholesky = BT_TRUE;
      } else {
        THROWWARNING(RET_NO_CHOLESKY_WITH_INITIAL_GUESS);
      }
  ...
  if (options.enableRamping == BT_TRUE) performRamping();
  ...
  */
  ...
  for (iter = nWSRperformed; iter < nWSR; ++iter) {
    ...
    getGlobalMessageHandler()->throwInfo(
        RET_ITERATION_STARTED, messageString, __FUNC__, __FILE__, __LINE__, VS_VISIBLE);
#endif

    /* 2) Determination of shift direction of the gradient and the (constraints') bounds. */
    returnvalue = determineDataShift(
        g_new, lbA_new, ubA_new, lb_new, ub_new, delta_g, delta_lbA, delta_ubA, delta_lb, delta_ub, Delta_bC_isZero,
        Delta_bB_isZero);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yAC;
      delete[] delta_yFX;
    ...
    returnvalue = determineStepDirection(
        delta_g, delta_lbA, delta_ubA, delta_lb, delta_ub, Delta_bC_isZero, Delta_bB_isZero, delta_xFX, delta_xFR,
        delta_yAC, delta_yFX);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yAC;
      delete[] delta_yFX;
    ...
    returnvalue = performStep(
        delta_g, delta_lbA, delta_ubA, delta_lb, delta_ub, delta_xFX, delta_xFR, delta_yAC, delta_yFX, BC_idx,
  ...
  if (options.printLevel == PL_HIGH) {
    ...
    return getGlobalMessageHandler()->throwWarning(
        RET_MAX_NWSR_REACHED, messageString, __FUNC__, __FILE__, __LINE__, VS_VISIBLE);
#else
...
returnValue QProblem::solveRegularisedQP(
  ...
  if (returnvalue != SUCCESSFUL_RETURN) {
    if (cputime != 0) *cputime = cputime_total;

    if (returnvalue == RET_MAX_NWSR_REACHED) THROWWARNING(RET_NO_REGSTEP_NWSR);
  ...
  for (step = 0; step < options.numRegularisationSteps; ++step) {
    ...
    for (i = 0; i < nV; ++i) gMod[i] = g_new[i] - regVal * x[i];
    ...
    if (returnvalue != SUCCESSFUL_RETURN) {
      ...
      if (returnvalue == RET_MAX_NWSR_REACHED) THROWWARNING(RET_FEWER_REGSTEPS_NWSR);

      return returnvalue;
    }
  }

  for (i = 0; i < nV; ++i) g[i] = g_new[i];
 ...
 */
returnValue QProblem::updateActivitiesForHotstart(
    const real_t* const lb_new, const real_t* const ub_new, const real_t* const lbA_new, const real_t* const ubA_new) {
  int_t i;
  int_t nV = getNV();
 ...
 *	s e t u p S u b j e c t T o T y p e
 */
returnValue QProblem::setupSubjectToType() { return setupSubjectToType(lb, ub, lbA, ubA); }
...
returnValue QProblem::setupSubjectToType(
  ...
  if ((lbA_new != 0) && (ubA_new != 0)) {
    for (i = 0; i < nC; ++i) {
      if (constraints.getType(i) == ST_DISABLED) continue;
    ...
    if ((lbA_new == 0) && (ubA_new == 0)) {
      for (i = 0; i < nC; ++i) {
        if (constraints.getType(i) != ST_DISABLED) constraints.setType(i, ST_UNBOUNDED);
      }
    } else {
      for (i = 0; i < nC; ++i) {
        if (constraints.getType(i) != ST_DISABLED) constraints.setType(i, ST_BOUNDED);
      }
    }
  }
...
returnValue QProblem::computeProjectedCholesky() {
  ...
  if (getNFX() + getNAC() == 0) return QProblemB::computeCholesky();
  ...
  switch (hessianType) {
    case HST_ZERO:
      if (usingRegularisation() == BT_TRUE) {
        ...
        if (nZ > 0) return THROWERROR(RET_UNKNOWN_BUG);
      }
      break;
    ...
    default:
      if (getNAC() == 0) {
        /* make Z trivial */
        for (j = 0; j < nZ; ++j) {
          for (i = 0; i < nV; ++i) QQ(i, j) = 0.0;
          QQ(FR_idx[j], j) = 1.0;
        }
        /* now Z is trivial, and so is Z'HZ */
        int_t nFR = getNFR();
        for (j = 0; j < nFR; ++j) H->getCol(FR_idx[j], bounds.getFree(), 1.0, &R[j * nV]);
      } else {
...
returnValue QProblem::setupInitialCholesky() {
  ...
  if ((getNV() != getNFR() - getNFV()) && (options.enableRegularisation == BT_TRUE))
    if (regulariseHessian() != SUCCESSFUL_RETURN) return RET_INIT_FAILED_REGULARISATION;
  ...
  if (returnvalueCholesky != SUCCESSFUL_RETURN) return RET_INIT_FAILED_CHOLESKY;
...
returnValue QProblem::setupAuxiliaryWorkingSet(
    const Bounds* const auxiliaryBounds, const Constraints* const auxiliaryConstraints, BooleanType setupAfresh) {
  int_t i;
  int_t nV = getNV();
  int_t nC = getNC();
  ...
  if (WSisTrivial == BT_TRUE) {
    for (i = 0; i < nV; i++)
      if (bounds.getStatus(i) == ST_INACTIVE) bounds.moveFreeToFixed(i, auxiliaryBounds->getStatus(i));
...
returnValue QProblem::setupAuxiliaryQPgradient() {
  ...
  switch (hessianType) {
    case HST_ZERO:
      if (usingRegularisation() == BT_FALSE)
        for (i = 0; i < nV; ++i) g[i] = y[i];
      else
        for (i = 0; i < nV; ++i) g[i] = y[i] - regVal * x[i];
      break;

    case HST_IDENTITY:
      for (i = 0; i < nV; ++i) g[i] = y[i] - x[i];
      break;
 ...
 */
returnValue QProblem::areBoundsConsistent(
    const real_t* const lb_new,
    const real_t* const ub_new,
    const real_t* const lbA_new,
    const real_t* const ubA_new) const {
  if (QProblemB::areBoundsConsistent(lb_new, ub_new) == RET_QP_INFEASIBLE) return RET_QP_INFEASIBLE;
...
returnValue QProblem::setupAuxiliaryQPbounds(
    const Bounds* const auxiliaryBounds, const Constraints* const auxiliaryConstraints, BooleanType useRelaxation) {
  int_t i;
  int_t nV = getNV();
  int_t nC = getNC();
    ...
    switch (bounds.getStatus(i)) {
      ...
      case ST_LOWER:
        ...
        if (bounds.getType(i) == ST_EQUALITY) {
          ...
          if (useRelaxation == BT_TRUE) ub[i] = x[i] + options.boundRelaxation;
        }
        break;
      ...
      case ST_UPPER:
        ...
        if (bounds.getType(i) == ST_EQUALITY) {
          ...
          if (useRelaxation == BT_TRUE) lb[i] = x[i] - options.boundRelaxation;
        }
        break;
    ...
    switch (constraints.getStatus(i)) {
      ...
      case ST_LOWER:
        ...
        if (constraints.getType(i) == ST_EQUALITY) {
          ...
          if (useRelaxation == BT_TRUE) ubA[i] = Ax_l[i] + options.boundRelaxation;
        }
        break;
      ...
      case ST_UPPER:
        ...
        if (constraints.getType(i) == ST_EQUALITY) {
          ...
          if (useRelaxation == BT_TRUE) lbA[i] = Ax_u[i] - options.boundRelaxation;
        }
        break;
 ...
 */
returnValue QProblem::addConstraint(
  ...
  for (i = 0; i < nFR; ++i) {
    ii = FR_idx[i];
    for (j = 0; j < nZ; ++j) wZ[j] += aFR[i] * QQ(ii, j);
  }

  /* 2) Calculate wY and store it directly into T. */
  if (nAC > 0) {
    for (j = 0; j < nAC; ++j) TT(nAC, tcol + j) = 0.0;
    for (i = 0; i < nFR; ++i) {
      ii = FR_idx[i];
      for (j = 0; j < nAC; ++j) TT(nAC, tcol + j) += aFR[i] * QQ(ii, nZ + j);
    }
  }
  ...
  if (nZ > 0) {
    ...
    for (j = 0; j < nZ - 1; ++j) {
      ...
      if ((updateCholesky == BT_TRUE) && (hessianType != HST_ZERO) && (hessianType != HST_IDENTITY)) {
        for (i = 0; i <= j + 1; ++i) applyGivens(c, s, nu, RR(i, 1 + j), RR(i, j), RR(i, 1 + j), RR(i, j));
      }
    }
...
returnValue QProblem::addConstraint_checkLI(int_t number) {
  ...
  if (options.enableFullLITests) {
    ...
    if (dsdreturnvalue != SUCCESSFUL_RETURN) returnvalue = dsdreturnvalue;
    ...
    if (zero > options.epsLITests * weight) returnvalue = RET_LINEARLY_INDEPENDENT;
    ...
    l2 = 0.0;
    for (i = 0; i < nFR; i++) l2 += Arow[i] * Arow[i];
...
returnValue QProblem::addConstraint_ensureLI(int_t number, SubjectToStatus C_status) {
  ...
  if (y_min_number >= 0) {
    ...
    if (y_min_isBound == BT_TRUE) {
      ...
      getGlobalMessageHandler()->throwInfo(
          RET_REMOVE_FROM_ACTIVESET, messageString, __FUNC__, __FILE__, __LINE__, VS_VISIBLE);
#endif
...
farewell:
  ...
  return ((returnvalue != SUCCESSFUL_RETURN) && (returnvalue != RET_ENSURELI_FAILED_NOINDEX)) ? THROWERROR(returnvalue)
                                                                                              : returnvalue;
}
...
returnValue QProblem::addBound(
  ...
  for (i = 0; i < nFR; ++i) w[i] = QQ(FR_idx[nFR - 1], i);
...
returnValue QProblem::addBound_checkLI(int_t number) {
  ...
  if (options.enableFullLITests) {
    ...
    for (ii = 0; ii < nV; ++ii) delta_g[ii] = 0.0;
...
returnValue QProblem::addBound_ensureLI(int_t number, SubjectToStatus B_status) {
  ...
  if (nAC > 0) {
    if (B_status == ST_LOWER) {
      for (i = 0; i < nAC; ++i) xiC_TMP[i] = QQ(number, nZ + i);
    } else {
      for (i = 0; i < nAC; ++i) xiC_TMP[i] = -QQ(number, nZ + i);
    }
 ...
 */
returnValue QProblem::removeConstraint(
  ...
  if (number_idx < nAC - 1) {
    for (i = (number_idx + 1); i < nAC; ++i)
      for (j = (nAC - i - 1); j < nAC; ++j) TT(i - 1, tcol + j) = TT(i, tcol + j);
    /* gimmick: write zeros into the last row of T */
    for (j = 0; j < nAC; ++j) TT(nAC - 1, tcol + j) = 0.0;
    ...
    for (j = (nAC - 2 - number_idx); j >= 0; --j) {
      computeGivens(
          TT(nAC - 2 - j, tcol + 1 + j), TT(nAC - 2 - j, tcol + j), TT(nAC - 2 - j, tcol + 1 + j),
  ...
  if ((updateCholesky == BT_TRUE) && (hessianType != HST_ZERO) && (hessianType != HST_IDENTITY)) {
    ...
    for (j = 0; j < nFR; ++j) z[j] = QQ(FR_idx[j], nZ);
    ...
    if (nZ > 0) {
      ...
      for (j = 0; j < nFR; ++j) {
        ...
        for (i = 0; i < nZ; ++i) ZHz[i] += QQ(jj, i) * Hz[j];
      }
    ...
    /* 5) Store rho into R. */
    for (j = 0; j < nFR; ++j) rho2 += QQ(FR_idx[j], nZ) * Hz[j];
 ...
 */
returnValue QProblem::removeBound(
    int_t number, BooleanType updateCholesky, BooleanType allowFlipping, BooleanType ensureNZC) {
  int_t i, j, ii, jj;
  returnValue returnvalue = SUCCESSFUL_RETURN;
  int_t addIdx;
  ...
  if (nAC > 0) {
    ...
    for (j = (nAC - 1); j >= 0; --j) {
      ...
      for (i = (nAC - j); i < nAC; ++i) applyGivens(c, s, nu, tmp[i], TT(i, tcol + j), TT(i, tcol + j), tmp[i]);
...
returnValue QProblem::performPlainRatioTest(
    int_t nIdx,
...
returnValue QProblem::ensureNonzeroCurvature(
  ...
  if (removeBoundNotConstraint) {
    ...
    for (ii = 0; ii < dim; ++ii) nul[ii] = 0.0;
    for (ii = 0; ii < nV; ++ii) ek[ii] = 0.0;
    ek[remIdx] = bounds.getStatus(remIdx) == ST_LOWER ? 1.0 : -1.0;

    returnvalue =
        determineStepDirection(nul, nul, nul, ek, ek, BT_FALSE, BT_FALSE, delta_xFX, delta_xFR, delta_yAC, delta_yFX);
    delete[] ek;
    delete[] nul;
  } else {
    real_t* nul = new real_t[nV];
    real_t* ek = new real_t[nC]; /* minus e_k (constraint k is removed) */
    for (ii = 0; ii < nV; ++ii) nul[ii] = 0.0;
    for (ii = 0; ii < nC; ++ii) ek[ii] = 0.0;
    ek[remIdx] = constraints.getStatus(remIdx) == ST_LOWER ? 1.0 : -1.0;

    returnvalue =
        determineStepDirection(nul, ek, ek, nul, nul, BT_FALSE, BT_TRUE, delta_xFX, delta_xFR, delta_yAC, delta_yFX);
    delete[] ek;
    delete[] nul;
  }
  ...
  if (normXi < options.epsNZCTests * normS) {
    ...
    for (i = 0; i < nFR; i++) delta_xFR[i] = -delta_xFR[i];
    ...
    if (sigma >= options.maxPrimalJump) {
      ...
      for (i = 0; i < nFR; i++) x[FR_idx[i]] += sigma * delta_xFR[i];

      for (i = 0; i < nFX; i++) x[FX_idx[i]] += sigma * delta_xFX[i];
...
returnValue QProblem::backsolveT(const real_t* const b, BooleanType transposed, real_t* const a) const {
  ...
  if (transposed == BT_FALSE) {
    ...
    for (i = 0; i < nT; ++i) {
      sum = b[i];
      for (j = 0; j < i; ++j) sum -= TT(i, sizeT - 1 - j) * a[nT - 1 - j];
    ...
    for (i = 0; i < nT; ++i) {
      sum = b[i];
      for (j = 0; j < i; ++j) sum -= TT(nT - 1 - j, tcol + i) * a[nT - 1 - j];
...
returnValue QProblem::determineDataShift(
  ...
  QProblemB::determineDataShift(g_new, lb_new, ub_new, delta_g, delta_lb, delta_ub, Delta_bB_isZero);
...
returnValue QProblem::determineStepDirection(
  ...
  if (Delta_bB_isZero == BT_FALSE) {
    ...
    for (i = 0; i < nFX; ++i) delta_xFX[i] = 0.0;
  }
  ...
  for (i = 0; i < nAC; ++i) delta_yAC[i] = 0.0;
  ...
  for (r = 0; r <= options.numRefinementSteps; ++r) {
    ...
    if (nFR > 0) {
      for (i = 0; i < nFR; ++i) delta_xFR_TMP[i] = 0.0;
      ...
      if (nAC > 0) {
        if ((Delta_bC_isZero == BT_TRUE) && (Delta_bB_isZero == BT_TRUE)) {
          ...
          for (i = 0; i < nFR; ++i) {
            ii = FR_idx[i];
            for (j = 0; j < nAC; ++j) delta_xFR_TMP[i] += QQ(ii, nZ + j) * delta_xFRy[j];
          }
        }
      }
      ...
      if ((hessianType == HST_ZERO) || (hessianType == HST_IDENTITY)) {
        ...
        for (j = 0; j < nFR; ++j) {
          jj = FR_idx[j];
          for (i = 0; i < nZ; ++i) delta_xFRz[i] -= QQ(jj, i) * tempA[j];
        }

        if (hessianType == HST_ZERO) {
          if (usingRegularisation() == BT_TRUE) {
            for (i = 0; i < nZ; ++i) delta_xFRz[i] /= regVal;
          } else {
      ...
      if (nZ > 0) {
        for (i = 0; i < nFR; ++i) {
          ...
          ii = FR_idx[i];
          for (j = 0; j < nZ; ++j) ZFR_delta_xFRz[i] += QQ(ii, j) * delta_xFRz[j];
    ...
    if (nAC > 0) /* => ( nFR = nZ + nAC > 0 ) */
    {
      if ((hessianType == HST_ZERO) || (hessianType == HST_IDENTITY)) {
        ...
        if (hessianType == HST_IDENTITY) {
          for (j = 0; j < nAC; ++j) tempB[j] = delta_xFRy[j];
        } else /* hessianType == HST_ZERO */
        {
          if (usingRegularisation() == BT_TRUE) {
            for (j = 0; j < nAC; ++j) tempB[j] = regVal * delta_xFRy[j];
          } else {
            for (j = 0; j < nAC; ++j) tempB[j] = 0.0;
          }
        }
        ...
        if (nZ > 0) H->times(bounds.getFree(), bounds.getFree(), 1, 1.0, ZFR_delta_xFRz, nFR, 1.0, tempA, nFR);
    ...
    for (i = 0; i < nFR; ++i) delta_xFR[i] += delta_xFR_TMP[i];
    for (i = 0; i < nAC; ++i) delta_yAC[i] += delta_yAC_TMP[i];
    ...
    if (options.numRefinementSteps > 0) {
      ...
      switch (hessianType) {
        case HST_ZERO:
          if (usingRegularisation() == BT_TRUE)
            for (i = 0; i < nFR; ++i) tempA[i] += regVal * delta_xFR[i];
          break;

        case HST_IDENTITY:
          for (i = 0; i < nFR; ++i) tempA[i] += delta_xFR[i];
          break;
      ...
      real_t rnrm = 0.0;
      for (i = 0; i < nFR; ++i)
        if (rnrm < getAbs(tempA[i])) rnrm = getAbs(tempA[i]);
  ...
  if (nFX > 0) {
    ...
    switch (hessianType) {
      case HST_ZERO:
        if (usingRegularisation() == BT_TRUE)
          for (i = 0; i < nFX; ++i) delta_yFX[i] += regVal * delta_xFX[i];
        break;

      case HST_IDENTITY:
        for (i = 0; i < nFX; ++i) delta_yFX[i] += 1.0 * delta_xFX[i];
        break;
...
returnValue QProblem::performStep(
  ...
  if (BC_status == ST_UNDEFINED)
    ...
    snprintf(
        messageString, MAX_STRING_LENGTH, "Stepsize is %.15e! (idx = %d, isBound = %d, status = %d)", tau, (int)BC_idx,
        (int)BC_isBound, (int)BC_status);

  getGlobalMessageHandler()->throwInfo(
      RET_STEPSIZE_NONPOSITIVE, messageString, __FUNC__, __FILE__, __LINE__, VS_VISIBLE);
#endif
  ...
  switch (BC_status) {
    ...
    default:
      ...
      if (BC_isBound == BT_TRUE) {
        ...
        getGlobalMessageHandler()->throwInfo(
            RET_ADD_TO_ACTIVESET, messageString, __FUNC__, __FILE__, __LINE__, VS_VISIBLE);
#endif

        returnvalue = addBound(BC_idx, BC_status, BT_TRUE);
        if (returnvalue == RET_ADDBOUND_FAILED_INFEASIBILITY) return returnvalue;
        if (returnvalue != SUCCESSFUL_RETURN) return THROWERROR(RET_ADD_TO_ACTIVESET_FAILED);
      } else {
#ifndef __SUPPRESSANYOUTPUT__
        if (BC_status == ST_LOWER)
        ...
        if (returnvalue == RET_ADDCONSTRAINT_FAILED_INFEASIBILITY) return returnvalue;
        if (returnvalue != SUCCESSFUL_RETURN) return THROWERROR(RET_ADD_TO_ACTIVESET_FAILED);
      }
  }
 ...
 */
real_t QProblem::getRelativeHomotopyLength(
    const real_t* const g_new,
    const real_t* const lb_new,
    const real_t* const ub_new,
    const real_t* const lbA_new,
...
returnValue QProblem::updateFarBounds(
  ...
  returnValue returnvalue = QProblemB::updateFarBounds(curFarBound, nRamp, lb_new, lb_new_far, ub_new, ub_new_far);
  if (returnvalue != SUCCESSFUL_RETURN) {
    return returnvalue;
  }
...
returnValue QProblem::setupAuxiliaryQP(const Bounds* const guessedBounds, const Constraints* const guessedConstraints) {
  ...
  /* consistency check */
  if ((guessedBounds == 0) || (guessedConstraints == 0)) return THROWERROR(RET_INVALID_ARGUMENTS);

  /* nothing to do */
  if ((guessedBounds == &bounds) && (guessedConstraints == &constraints)) return SUCCESSFUL_RETURN;
  ...
  if (shallRefactorise(guessedBounds, guessedConstraints) == BT_TRUE) {
    ...
    if (setupSubjectToType() != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);

    if (bounds.setupAllFree() != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);

    if (constraints.setupAllInactive() != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);

    /* 2) Setup TQ factorisation. */
    if (setupTQfactorisation() != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
    ...
    if (computeProjectedCholesky() != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
  } else {
  ...
  for (i = 0; i < nV; ++i)
    if (bounds.getStatus(i) == ST_INACTIVE) y[i] = 0.0;

  for (i = 0; i < nC; ++i)
    if (constraints.getStatus(i) == ST_INACTIVE) y[nV + i] = 0.0;

  /* 2) Setup gradient and (constraints') bound vectors. */
  if (setupAuxiliaryQPgradient() != SUCCESSFUL_RETURN) THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
  ...
  if (setupAuxiliaryQPbounds(0, 0, BT_FALSE) != SUCCESSFUL_RETURN) THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
...
BooleanType QProblem::shallRefactorise(
    const Bounds* const guessedBounds, const Constraints* const guessedConstraints) const {
  int_t i;
  int_t nV = getNV();
  int_t nC = getNC();

  /* always refactorise if Hessian is not known to be positive definite */
  if ((hessianType == HST_SEMIDEF) || (hessianType == HST_INDEF)) return BT_TRUE;
  ...
  for (i = 0; i < nV; ++i)
    if (guessedBounds->getStatus(i) != bounds.getStatus(i)) ++differenceNumberBounds;
  ...
  for (i = 0; i < nC; ++i)
    if (guessedConstraints->getStatus(i) != constraints.getStatus(i)) ++differenceNumberConstraints;
...
returnValue QProblem::setupQPdata(
  ...
  if (QProblemB::setupQPdata(_H, _g, _lb, _ub) != SUCCESSFUL_RETURN) return THROWERROR(RET_INVALID_ARGUMENTS);
 ...
 */
returnValue QProblem::setupQPdataFromFile(
    const char* const H_file,
    const char* const g_file,
    const char* const A_file,
    const char* const lb_file,
  ...
  if ((nC > 0) && (A_file == 0)) return THROWERROR(RET_INVALID_ARGUMENTS);
...
returnValue QProblem::printIteration(
  ...
  /* consistency check */
  if (iter < 0) return THROWERROR(RET_INVALID_ARGUMENTS);
  ...
  switch (options.printLevel) {
    case PL_DEBUG_ITER:
      ...
      switch (hessianType) {
        case HST_ZERO:
          for (i = 0; i < nV; ++i) grad[i] += regVal * x[i];
          break;

        case HST_IDENTITY:
          for (i = 0; i < nV; ++i) grad[i] += 1.0 * x[i];
          break;
      ...
      for (i = 0; i < nV; i++)
        if (getAbs(grad[i]) > stat) stat = getAbs(grad[i]);

      /* feasibility */
      for (i = 0; i < nV; i++)
        if (lb[i] - x[i] > bfeas) bfeas = lb[i] - x[i];
      for (i = 0; i < nV; i++)
        if (x[i] - ub[i] > bfeas) bfeas = x[i] - ub[i];
      A->times(1, 1.0, x, nV, 0.0, AX, nC);
      for (i = 0; i < nC; i++)
        if (lbA[i] - AX[i] > cfeas) cfeas = lbA[i] - AX[i];
      for (i = 0; i < nC; i++)
        if (AX[i] - ubA[i] > cfeas) cfeas = AX[i] - ubA[i];

      /* complementarity */
      for (i = 0; i < nV; i++)
        if (y[i] > +EPS && getAbs((lb[i] - x[i]) * y[i]) > bcmpl) bcmpl = getAbs((lb[i] - x[i]) * y[i]);
      for (i = 0; i < nV; i++)
        if (y[i] < -EPS && getAbs((ub[i] - x[i]) * y[i]) > bcmpl) bcmpl = getAbs((ub[i] - x[i]) * y[i]);
      for (i = 0; i < nC; i++)
      ...
      if ((iter % 10 == 0) && (isFirstCall == BT_TRUE)) {
        snprintf(
            ...
            "addB", "remB", "addC", "remC", "hom len", "tau", "stat", "bfeas", "cfeas", "bcmpl", "ccmpl", "Tmin");
        myPrintf(myPrintfString);
      }
      ...
      snprintf(
          myPrintfString, MAX_STRING_LENGTH, "%9.2e %9.2e %9.2e %9.2e %9.2e %9.2e %9.2e %9.2e\n", homotopyLength, tau,
          stat, bfeas, cfeas, bcmpl, ccmpl, Tmin);
      myPrintf(myPrintfString);
    ...
    case PL_TABULAR:
      ...
      if (tabularOutput.idxAddB >= 0) {
        snprintf(
            myPrintfString, MAX_STRING_LENGTH, "%5d%c ", (int)(tabularOutput.idxAddB), excStr[tabularOutput.excAddB]);
        myPrintf(myPrintfString);
      } else {
        myPrintf("       ");
      }
      ...
      if (tabularOutput.idxRemB >= 0) {
        snprintf(
            myPrintfString, MAX_STRING_LENGTH, "%5d%c ", (int)(tabularOutput.idxRemB), excStr[tabularOutput.excRemB]);
        myPrintf(myPrintfString);
      } else {
        myPrintf("       ");
      }
      ...
      if (tabularOutput.idxAddC >= 0) {
        snprintf(
            myPrintfString, MAX_STRING_LENGTH, "%5d%c ", (int)(tabularOutput.idxAddC), excStr[tabularOutput.excAddC]);
        myPrintf(myPrintfString);
      } else {
        myPrintf("       ");
      }
      ...
      if (tabularOutput.idxRemC >= 0) {
        snprintf(
            myPrintfString, MAX_STRING_LENGTH, "%5d%c ", (int)(tabularOutput.idxRemC), excStr[tabularOutput.excRemC]);
        myPrintf(myPrintfString);
      } else {
        myPrintf("       ");
      }
    ...
    case PL_MEDIUM:
      ...
      if (BC_status == ST_UNDEFINED) {
        ...
        if (isFirstCall == BT_TRUE)
          snprintf(
              myPrintfString, MAX_STRING_LENGTH, "   %5.1d   |   %1.6e   |    %s SOLVED     |  %4.1d   |  %4.1d   \n",
        ...
        snprintf(
            myPrintfString, MAX_STRING_LENGTH, "   %5.1d   |   %1.6e   |   %s %4.1d   |  %4.1d   |  %4.1d   \n",
 ...
 */
returnValue QProblem::dropInfeasibles(
    int_t BC_number, SubjectToStatus BC_status, BooleanType BC_isBound, real_t* xiB, real_t* xiC) {
  int_t i;
...
returnValue QProblem::writeQpDataIntoMatFile(const char* const filename) const {
  ...
  if (matFile == 0) return RET_UNABLE_TO_OPEN_FILE;
