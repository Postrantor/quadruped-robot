 ...
 * h o t s t a r t
 */
returnValue SQProblem::hotstart(
  ...
  returnValue returnvalue =
      QProblem::hotstart(g_new, lb_new, ub_new, lbA_new, ubA_new, nWSR, cputime, guessedBounds, guessedConstraints);
...
returnValue SQProblem::hotstart(
  ...
  /* consistency checks */
  if ((H_file == 0) || (g_file == 0)) return THROWERROR(RET_INVALID_ARGUMENTS);
  ...
  returnvalue =
      hotstart(H_new, g_new, A_new, lb_new, ub_new, lbA_new, ubA_new, nWSR, cputime, guessedBounds, guessedConstraints);
...
returnValue SQProblem::hotstart(
    ...
    const Constraints* const guessedConstraints) {
  /* Call to hotstart function for fixed QP matrices. */
  return QProblem::hotstart(
      g_file, lb_file, ub_file, lbA_file, ubA_file, nWSR, cputime, guessedBounds, guessedConstraints);
}
  ...
  for (n_try = 0; n_try < 2; ++n_try) {
    if (n_try > 0) {
      ...
      for (int_t ii = 0; ii < nV; ++ii)
        if (oldBounds.getStatus(ii) == ST_INACTIVE) oldBounds.setStatus(ii, options.initialStatusBounds);
    }
    ...
    if (returnvalue == SUCCESSFUL_RETURN) break;
  }
...
returnValue SQProblem::setupNewAuxiliaryQP(
  ...
  if (A_new != 0) {
    ...
    if (nC > 0) return THROWERROR(RET_INVALID_ARGUMENTS);
  }

  if (H_new != 0) sH = new SymDenseMat(nV, nV, nV, (real_t*)H_new);

  returnValue returnvalue = setupNewAuxiliaryQP(sH, dA, lb_new, ub_new, lbA_new, ubA_new);

  if (H_new != 0) freeHessian = BT_TRUE;
