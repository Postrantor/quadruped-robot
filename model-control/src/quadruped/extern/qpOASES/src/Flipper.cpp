 ...
 *	g e t
 */
returnValue Flipper::get(
    Bounds* const _bounds,
    real_t* const _R,
    Constraints* const _constraints,
    real_t* const _Q,
    real_t* const _T) const {
  if (_bounds != 0) *_bounds = bounds;

  if (_constraints != 0) *_constraints = constraints;

  if ((_R != 0) && (R != 0)) memcpy(_R, R, nV * nV * sizeof(real_t));

  if ((_Q != 0) && (Q != 0)) memcpy(_Q, Q, nV * nV * sizeof(real_t));

  if ((_T != 0) && (T != 0)) memcpy(_T, T, getDimT() * sizeof(real_t));
...
returnValue Flipper::set(
  ...
  if (_bounds != 0) bounds = *_bounds;

  if (_constraints != 0) constraints = *_constraints;

  if (_R != 0) {
    if (R == 0) R = new real_t[nV * nV];

    memcpy(R, _R, nV * nV * sizeof(real_t));
  }

  if (_Q != 0) {
    if (Q == 0) Q = new real_t[nV * nV];

    memcpy(Q, _Q, nV * nV * sizeof(real_t));
  }

  if (_T != 0) {
    if (T == 0) T = new real_t[getDimT()];
 ...
 *	c o p y
 */
returnValue Flipper::copy(const Flipper& rhs) { return set(&(rhs.bounds), rhs.R, &(rhs.constraints), rhs.Q, rhs.T); }
