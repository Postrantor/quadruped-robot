...
returnValue readOqpDimensions(const char* path, int_t& nQP, int_t& nV, int_t& nC, int_t& nEC) {
  ...
  int_t dims[4];
  if (readFromFile(dims, 4, filename) != SUCCESSFUL_RETURN) return THROWERROR(RET_UNABLE_TO_READ_FILE);
  ...
  /* consistency check */
  if ((nQP <= 0) || (nV <= 0) || (nC < 0) || (nEC < 0)) return THROWERROR(RET_FILEDATA_INCONSISTENT);
...
returnValue readOqpData(
  ...
  /* consistency check */
  if ((H == 0) || (g == 0) || (lb == 0) || (ub == 0)) return THROWERROR(RET_INVALID_ARGUMENTS);

  /* 1) Obtain OQP dimensions. */
  if (readOqpDimensions(path, nQP, nV, nC, nEC) != SUCCESSFUL_RETURN) return THROWERROR(RET_UNABLE_TO_READ_FILE);

  /* another consistency check */
  if ((nC > 0) && ((A == 0) || (lbA == 0) || (ubA == 0))) return THROWERROR(RET_FILEDATA_INCONSISTENT);
...
returnValue solveOqpBenchmark(
  ...
  returnValue returnvalue = solveOqpBenchmark(
      nQP, nV, nC, nEC, _H, g, _A, lb, ub, lbA, ubA, isSparse, BT_TRUE, options, nWSR, maxNWSR, avgNWSR, maxCPUtime,
...
returnValue solveOqpBenchmark(
  ...
  for (k = 0; k < nQP; ++k) {
    ...
    if (((double)nWSRcur) > maxNWSR) maxNWSR = ((double)nWSRcur);
    ...
    if (CPUtimeCur > maxCPUtime) maxCPUtime = CPUtimeCur;
...
returnValue solveOqpBenchmark(
  ...
  returnValue returnvalue = solveOqpBenchmark(
      nQP, nV, _H, g, lb, ub, isSparse, BT_TRUE, options, nWSR, maxNWSR, avgNWSR, maxCPUtime, avgCPUtime,
...
returnValue solveOqpBenchmark(
  ...
  for (k = 0; k < nQP; ++k) {
    ...
    if (((real_t)nWSRcur) > maxNWSR) maxNWSR = ((real_t)nWSRcur);
    if (stat > maxStationarity) maxStationarity = stat;
    if (feas > maxFeasibility) maxFeasibility = feas;
    if (cmpl > maxComplementarity) maxComplementarity = cmpl;
...
returnValue runOqpBenchmark(
  ...
  returnValue returnvalue = runOqpBenchmark(
      path, isSparse, BT_TRUE, options, nWSR, maxNWSR, avgNWSR, maxCPUtime, avgCPUtime, maxStationarity, maxFeasibility,
      maxComplementarity);
  nWSR = (int_t)maxNWSR;
...
returnValue runOqpBenchmark(
  ...
  if (readOqpDimensions(path, nQP, nV, nC, nEC) != SUCCESSFUL_RETURN) return THROWERROR(RET_UNABLE_TO_READ_BENCHMARK);

  /* 2) Read OQP benchmark data. */
  if (readOqpData(path, nQP, nV, nC, nEC, &H, &g, &A, &lb, &ub, &lbA, &ubA, 0, 0, 0) != SUCCESSFUL_RETURN) {
  ...
  if (nC > 0) {
    returnvalue = solveOqpBenchmark(
        nQP, nV, nC, nEC, H, g, A, lb, ub, lbA, ubA, isSparse, useHotstarts, options, maxAllowedNWSR, maxNWSR, avgNWSR,
        maxCPUtime, avgCPUtime, maxStationarity, maxFeasibility, maxComplementarity);
