...
real_t SolutionAnalysis::getKktViolation(
  ...
  switch (qp->getHessianType()) {
    ...
    default:
      ...
      if (qp->usingRegularisation() == BT_TRUE)
        for (i = 0; i < nV; ++i) H_ptr[i * nV + i] -= qp->regVal;
  }
  ...
  if (workingSetB != 0) delete[] workingSetB;

  if (H_ptr != 0) delete[] H_ptr;

  if (returnvalue != SUCCESSFUL_RETURN) THROWERROR(returnvalue);

  /* assign return values */
  if (maxStat != 0) *maxStat = stat;

  if (maxFeas != 0) *maxFeas = feas;

  if (maxCmpl != 0) *maxCmpl = cmpl;
...
real_t SolutionAnalysis::getKktViolation(
  ...
  returnValue returnvalue = REFER_NAMESPACE_QPOASES getKktViolation(
      nV, nC, H_ptr, qp->g, A_ptr, qp->lb, qp->ub, qp->lbA, qp->ubA, qp->x, qp->y, stat, feas, cmpl, workingSetB,
      workingSetC, hasIdentityHessian);

  if (workingSetC != 0) delete[] workingSetC;

  if (workingSetB != 0) delete[] workingSetB;

  if (A_ptr != 0) delete[] A_ptr;
...
returnValue SolutionAnalysis::getVarianceCovariance(
  ...
  for (run3 = 0; run3 < dim; run3++) {
    ...
    returnvalue = qp->determineStepDirection(
        delta_g_cov, delta_lbA_cov, delta_ubA_cov, delta_lb_cov, delta_ub_cov, Delta_bC_isZero, Delta_bB_isZero,
...
returnValue SolutionAnalysis::checkCurvatureOnStronglyActiveConstraints(SQProblemSchur* qp) {
  ...
  if (nFX == 0) return SUCCESSFUL_RETURN;
  ...
  for (k = 0; k < nFX; k++)
    if (getAbs(qp->x[FX_idx[k]]) > eps)
      if (qp->bounds.moveFixedToFree(FX_idx[k]) != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVEBOUND_FAILED);
  ...
  if (ret == SUCCESSFUL_RETURN && neig != nAC) ret = RET_INERTIA_CORRECTION_FAILED;
