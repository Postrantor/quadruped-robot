...
int_t QProblem_qpOASES(
  ...
  for (int_t k = 0; k < nP; ++k) {
    if (k > 0) {
      ...
      g_current = &(g[k * nV]);
      if (lb != 0) lb_current = &(lb[k * nV]);
      if (ub != 0) ub_current = &(ub[k * nV]);
      if (lbA != 0) lbA_current = &(lbA[k * nC]);
      if (ubA != 0) ubA_current = &(ubA[k * nC]);
...
int_t QProblemB_qpOASES(
  ...
  returnvalue = QP.init(H, g, lb, ub, nWSRout, &maxCpuTimeOut, x0, 0, (guessedBounds != 0) ? &bounds : 0, _R);
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  ...
  nP = (int_t)mxGetN(
      prhs[g_idx]); /* number of columns of the gradient matrix (vectors series have to be stored columnwise!) */
  ...
  if (isSimplyBoundedQp == BT_TRUE) {
    if ((nrhs >= 5) && (!mxIsEmpty(prhs[4])) && (mxIsStruct(prhs[4]))) options_idx = 4;
  } else {
    /* Consistency check */
    ...
    if ((nrhs >= 8) && (!mxIsEmpty(prhs[7])) && (mxIsStruct(prhs[7]))) options_idx = 7;
  }
  ...
  if ((H_idx >= 0) && ((mxGetN(prhs[H_idx]) != nV) || (mxGetM(prhs[H_idx]) != nV))) {
    char msg[MAX_STRING_LENGTH];
    snprintf(
        msg, MAX_STRING_LENGTH, "ERROR (qpOASES): Hessian matrix dimension mismatch (%ld != %d)!",
  ...
  if (nC > 0) {
    ...
    if (mxGetN(prhs[A_idx]) != nV) {
      char msg[MAX_STRING_LENGTH];
      snprintf(
          msg, MAX_STRING_LENGTH, "ERROR (qpOASES): Constraint matrix input dimension mismatch (%ld != %d)!",
  ...
  if (smartDimensionCheck(&g, nV, nP, BT_FALSE, prhs, g_idx) != SUCCESSFUL_RETURN) return;

  if (smartDimensionCheck(&lb, nV, nP, BT_TRUE, prhs, lb_idx) != SUCCESSFUL_RETURN) return;

  if (smartDimensionCheck(&ub, nV, nP, BT_TRUE, prhs, ub_idx) != SUCCESSFUL_RETURN) return;

  if (smartDimensionCheck(&x0, nV, 1, BT_TRUE, prhs, x0_idx) != SUCCESSFUL_RETURN) return;

  if (nC > 0) {
    if (smartDimensionCheck(&lbA, nC, nP, BT_TRUE, prhs, lbA_idx) != SUCCESSFUL_RETURN) return;

    if (smartDimensionCheck(&ubA, nC, nP, BT_TRUE, prhs, ubA_idx) != SUCCESSFUL_RETURN) return;
  }
  ...
  if (options_idx > 0) setupOptions(&options, prhs[options_idx], nWSRin, maxCpuTimeIn);

  /* make a deep-copy of the user-specified Hessian matrix (possibly sparse) */
  if (H_idx >= 0) setupHessianMatrix(prhs[H_idx], nV, &H, &Hir, &Hjc, &Hv);

  /* make a deep-copy of the user-specified constraint matrix (possibly sparse) */
  if ((nC > 0) && (A_idx >= 0)) setupConstraintMatrix(prhs[A_idx], nV, nC, &A, &Air, &Ajc, &Av);
  ...
  if (nC == 0) {
    ...
    QProblemB_qpOASES(
        nV, hessianType, nP, H, g, lb, ub, nWSRin, maxCpuTimeIn, x0, &options, nlhs, plhs, guessedBounds, R);
    ...
    QProblem_qpOASES(
        nV, nC, hessianType, nP, H, g, A, lb, ub, lbA, ubA, nWSRin, maxCpuTimeIn, x0, &options, nlhs, plhs,
        guessedBounds, guessedConstraints, R, isSparse);
