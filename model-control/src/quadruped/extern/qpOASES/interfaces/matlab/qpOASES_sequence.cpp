...
int_t QProblemB_init(
  ...
  if (guessedBounds != 0) {
    for (int_t i = 0; i < nV; i++) {
      if (isEqual(guessedBounds[i], -1.0) == BT_TRUE) {
        ...
        snprintf(msg, MAX_STRING_LENGTH, "ERROR (qpOASES): Only {-1, 0, 1} allowed for status of bounds!");
        myMexErrMsgTxt(msg);
        return -1;
      }
    }
  ...
  returnvalue = globalQPB->init(H, g, lb, ub, nWSRout, &maxCpuTimeOut, x0, 0, (guessedBounds != 0) ? &bounds : 0, _R);
...
int_t SQProblem_init(
  ...
  if (guessedConstraints != 0) {
    for (int_t i = 0; i < nC; i++) {
      if (isEqual(guessedConstraints[i], -1.0) == BT_TRUE) {
        ...
        char msg[MAX_STRING_LENGTH];
        snprintf(msg, MAX_STRING_LENGTH, "ERROR (qpOASES): Only {-1, 0, 1} allowed for status of constraints!");
        myMexErrMsgTxt(msg);
        return -1;
      }
    }
  ...
  returnvalue = globalSQP->init(
      H, g, A, lb, ub, lbA, ubA, nWSRout, &maxCpuTimeOut, x0, 0, (guessedBounds != 0) ? &bounds : 0,
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  ...
  if ((strcmp(typeString, "i") != 0) && (strcmp(typeString, "I") != 0) && (strcmp(typeString, "h") != 0) &&
      (strcmp(typeString, "H") != 0) && (strcmp(typeString, "m") != 0) && (strcmp(typeString, "M") != 0) &&
      (strcmp(typeString, "e") != 0) && (strcmp(typeString, "E") != 0) && (strcmp(typeString, "c") != 0) &&
  ...
  if ((strcmp(typeString, "i") == 0) || (strcmp(typeString, "I") == 0)) {
    ...
    if (containsNaNorInf(prhs, H_idx, 0) == BT_TRUE) return;

    /* Check for 'Inf' and 'Nan' in gradient */
    if (containsNaNorInf(prhs, g_idx, 0) == BT_TRUE) return;
    ...
    if (isSimplyBoundedQp == BT_TRUE) {
      ...
      if (containsNaNorInf(prhs, lb_idx, 1) == BT_TRUE) return;

      if (containsNaNorInf(prhs, ub_idx, 1) == BT_TRUE) return;

      /* Check inputs dimensions and assign pointers to inputs. */
      nC = 0; /* row number of constraint matrix */

      if (smartDimensionCheck(&g, nV, 1, BT_FALSE, prhs, 2) != SUCCESSFUL_RETURN) return;

      if (smartDimensionCheck(&lb, nV, 1, BT_TRUE, prhs, 3) != SUCCESSFUL_RETURN) return;

      if (smartDimensionCheck(&ub, nV, 1, BT_TRUE, prhs, 4) != SUCCESSFUL_RETURN) return;
      ...
      if (nrhs >= 6) {
        if ((!mxIsEmpty(prhs[5])) && (mxIsStruct(prhs[5]))) setupOptions(&options, prhs[5], nWSRin, maxCpuTimeIn);
      ...
      if (containsNaNorInf(prhs, A_idx, 0) == BT_TRUE) return;

      if (containsNaNorInf(prhs, lb_idx, 1) == BT_TRUE) return;

      if (containsNaNorInf(prhs, ub_idx, 1) == BT_TRUE) return;

      if (containsNaNorInf(prhs, lbA_idx, 1) == BT_TRUE) return;

      if (containsNaNorInf(prhs, ubA_idx, 1) == BT_TRUE) return;
      ...
      if (smartDimensionCheck(&g, nV, 1, BT_FALSE, prhs, g_idx) != SUCCESSFUL_RETURN) return;

      if (smartDimensionCheck(&lb, nV, 1, BT_TRUE, prhs, lb_idx) != SUCCESSFUL_RETURN) return;

      if (smartDimensionCheck(&ub, nV, 1, BT_TRUE, prhs, ub_idx) != SUCCESSFUL_RETURN) return;

      if (smartDimensionCheck(&lbA, nC, 1, BT_TRUE, prhs, lbA_idx) != SUCCESSFUL_RETURN) return;

      if (smartDimensionCheck(&ubA, nC, 1, BT_TRUE, prhs, ubA_idx) != SUCCESSFUL_RETURN) return;
      ...
      /* Check whether x0 and options are specified .*/
      if (nrhs >= 9) {
        if ((!mxIsEmpty(prhs[8])) && (mxIsStruct(prhs[8]))) setupOptions(&options, prhs[8], nWSRin, maxCpuTimeIn);
    ...
    if (smartDimensionCheck(&x0, nV, 1, BT_TRUE, prhs, x0_idx) != SUCCESSFUL_RETURN) return;
    ...
    if (H_idx >= 0 && !mxIsSparse(prhs[H_idx])) isSparse = BT_FALSE;
    if (nC > 0 && A_idx >= 0 && !mxIsSparse(prhs[A_idx])) isSparse = BT_FALSE;
    ...
    /* Call qpOASES. */
    if (isSimplyBoundedQp == BT_TRUE) {
      QProblemB_init(handle, globalQP->H, g, lb, ub, nWSRin, maxCpuTimeIn, x0, &options, nlhs, plhs, guessedBounds, R);
    } else {
      SQProblem_init(
          handle, globalQP->H, g, globalQP->A, lb, ub, lbA, ubA, nWSRin, maxCpuTimeIn, x0, &options, nlhs, plhs,
  ...
  if ((strcmp(typeString, "h") == 0) || (strcmp(typeString, "H") == 0)) {
    ...
    if (isSimplyBoundedQp == BT_TRUE) {
      ...
      /* Check whether options are specified .*/
      if (nrhs == 8)
        if ((!mxIsEmpty(prhs[7])) && (mxIsStruct(prhs[7]))) setupOptions(&options, prhs[7], nWSRin, maxCpuTimeIn);
    }
  ...
  if ((strcmp(typeString, "m") == 0) || (strcmp(typeString, "M") == 0)) {
    ...
    /* Check whether options are specified .*/
    if (nrhs > 9)
      if ((!mxIsEmpty(prhs[9])) && (mxIsStruct(prhs[9]))) setupOptions(&options, prhs[9], nWSRin, maxCpuTimeIn);
    ...
    /* Call qpOASES */
    SQProblem_hotstart(
        handle, globalQP->H, g, globalQP->A, lb, ub, lbA, ubA, nWSRin, maxCpuTimeIn, &options, nlhs, plhs);
  ...
  if ((strcmp(typeString, "e") == 0) || (strcmp(typeString, "E") == 0)) {
    ...
    if (smartDimensionCheck(&g, nV, nRHS, BT_FALSE, prhs, g_idx) != SUCCESSFUL_RETURN) return;

    if (smartDimensionCheck(&lb, nV, nRHS, BT_TRUE, prhs, lb_idx) != SUCCESSFUL_RETURN) return;

    if (smartDimensionCheck(&ub, nV, nRHS, BT_TRUE, prhs, ub_idx) != SUCCESSFUL_RETURN) return;

    if (smartDimensionCheck(&lbA, nC, nRHS, BT_TRUE, prhs, lbA_idx) != SUCCESSFUL_RETURN) return;

    if (smartDimensionCheck(&ubA, nC, nRHS, BT_TRUE, prhs, ubA_idx) != SUCCESSFUL_RETURN) return;
