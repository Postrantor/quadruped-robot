...
static void mdlOutputs(SimStruct *S, int_T tid) {
  ...
  if (H != 0) {
    ...
    for (ii = 0; ii < nV * nV; ++ii) H[ii] = (mxGetPr(in_H))[ii];
  }

  convertFortranToC(mxGetPr(in_A), nV, nC, A);

  for (ii = 0; ii < nV; ++ii) g[ii] = (*in_g)[ii];

  if (lb != 0) {
    for (ii = 0; ii < nV; ++ii) lb[ii] = (*in_lb)[ii];
  }

  if (ub != 0) {
    for (ii = 0; ii < nV; ++ii) ub[ii] = (*in_ub)[ii];
  }

  if (lbA != 0) {
    for (ii = 0; ii < nC; ++ii) lbA[ii] = (*in_lbA)[ii];
  }

  if (ubA != 0) {
    for (ii = 0; ii < nC; ++ii) ubA[ii] = (*in_ubA)[ii];
  }
  ...
  for (ii = 0; ii < nU; ++ii) out_uOpt[ii] = (real_T)(xOpt[ii]);
...
static void mdlTerminate(SimStruct *S) {
  ...
  if (ssGetPWork(S)[0] != 0) delete ((QProblem *)(ssGetPWork(S)[0]));

  for (ii = 1; ii < 8; ++ii) {
    if (ssGetPWork(S)[ii] != 0) free(ssGetPWork(S)[ii]);
  }
}
