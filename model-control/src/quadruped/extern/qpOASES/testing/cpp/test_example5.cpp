...
int main() {
   ...
   *	    (         |           ...      )
   *	    (         |               1e-6 ) */
  for (i = 0; i < 7 * 7; ++i) H[i] = 0.0;
  for (i = 2; i < 7; ++i) H[i * 7 + i] = 1.0e-6;
  H[0] = 1.0;
  H[1] = 0.5;
  H[7] = 0.5;
  H[8] = 2.0;
   ...
   *	    ( x.x x.x |             ... )
   *	    ( x.x x.x |             1.0 ) */
  for (i = 0; i < 50 * 7; ++i) A[i] = 0.0;
  for (i = 0; i < 50; ++i) {
    for (j = 0; j < 2; ++j) A[i * 7 + j] = (real_t)rand() / (real_t)RAND_MAX;
   ...
   *	    (      )
   *	    (      ) */
  for (i = 0; i < 7; ++i) g[i] = 0.0;
  g[0] = -1.0;
  g[1] = -0.5;

  for (i = 0; i < 50; ++i) lbA[i] = 1.0;
  ...
  for (ii = 0; ii < 7; ++ii) g_in[ii * 7 + ii] = 1.0;
  for (ii = 0; ii < 7; ++ii) b_in[(ii + 7) * 7 + ii] = 1.0;
  for (ii = 0; ii < 50; ++ii) bA_in[(ii + 14) * 50 + ii] = 1.0;
  ...
  for (ii = 0; ii < n_rhs; ++ii) {
    for (jj = 0; jj < 7; ++jj) g_in[ii * 7 + jj] = g[jj] + g_in[ii * 7 + jj] * perturb;
    for (jj = 0; jj < 50; ++jj) bA_in[ii * 50 + jj] = lbA[jj] + bA_in[ii * 50 + jj] * perturb;
