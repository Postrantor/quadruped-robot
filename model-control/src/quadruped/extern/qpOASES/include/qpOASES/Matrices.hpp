...
public:
   ...
   */
  virtual real_t getRowNorm(
      int_t rNum,    /**< Row number. */
      int_t type = 2 /**< Norm type, 1: one-norm, 2: Euclidean norm. */
  ) const = 0;

  /** Get the N-norm of all rows
   *  \return SUCCESSFUL_RETURN */
  virtual returnValue getRowNorm(
      real_t* norm,  /**< Norm of each row. */
      int_t type = 2 /**< Norm type, 1: one-norm, 2: Euclidean norm. */
  ) const = 0;
  ...
  virtual returnValue getSparseSubmatrix(
      ...
      BooleanType only_lower_triangular = BT_FALSE /**< if true, only the lower triangular portion is returned.  This
