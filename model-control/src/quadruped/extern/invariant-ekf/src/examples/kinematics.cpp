...
int main() {
  ...
  while (getline(infile, line)) {
    ...
    if (measurement[0].compare("IMU") == 0) {
      ...
      // Read in IMU data
      imu_measurement << stod98(measurement[2]), stod98(measurement[3]), stod98(measurement[4]), stod98(measurement[5]),
      ...
      for (int i = 2; i < measurement.size(); i += 44) {
        ...
        q = Eigen::Quaternion<double>(
            stod98(measurement[i + 1]), stod98(measurement[i + 2]), stod98(measurement[i + 3]),
