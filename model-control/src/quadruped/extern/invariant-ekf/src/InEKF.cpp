...
Observation::Observation(
    Eigen::VectorXd& Y, Eigen::VectorXd& b, Eigen::MatrixXd& H, Eigen::MatrixXd& N, Eigen::MatrixXd& PI)
...
// ------------ InEKF -------------
// Default constructor
InEKF::InEKF() : g_((Eigen::VectorXd(3) << 0, 0, -9.81).finished()) { std::cout << "InEKF() success!!" << std::endl; }
...
void InEKF::Propagate(const Eigen::Matrix<double, 6, 1>& m, double dt) {
  ...
  Eigen::MatrixXd Qk_hat =
      PhiAdj * Qk * PhiAdj.transpose() * dt;  // Approximated discretized noise matrix (faster by 400 microseconds)
  ...
  if (new_landmarks.size() > 0) {
    ...
    for (vectorLandmarksIterator it = new_landmarks.begin(); it != new_landmarks.end(); ++it) {
      ...
      F.block(0, 0, state_.dimP() - state_.dimTheta(), state_.dimP() - state_.dimTheta()) =
          Eigen::MatrixXd::Identity(state_.dimP() - state_.dimTheta(), state_.dimP() - state_.dimTheta());  // for old X
      F.block(state_.dimP() - state_.dimTheta(), 6, 3, 3) = Eigen::Matrix3d::Identity();  // for new landmark
      F.block(
          state_.dimP() - state_.dimTheta() + 3, state_.dimP() - state_.dimTheta(), state_.dimTheta(),
...
void InEKF::CorrectKinematics(const vectorKinematics& measured_kinematics) {
  ...
  if (remove_contacts.size() > 0) {
    ...
    for (vector<pair<int, int> >::iterator it = remove_contacts.begin(); it != remove_contacts.end(); ++it) {
      ...
      // We also need to update the indices of remove_contacts in the case where multiple contacts are being removed at
      // once
