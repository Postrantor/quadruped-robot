...
private:
  ...
  std::vector<Eigen::Quaternion<double>> q;  // R_VectorNav_to_FRToeBottom, R_VectorNav_to_FLToeBottom,
                                             // R_VectorNav_to_BRToeBottom, R_VectorNav_to_BLToeBottom
...
const Eigen::MatrixXd INEKFInterface::getRobotStateX() { return filter.getState().getX(); }

const Eigen::VectorXd INEKFInterface::getRobotStateTheta() { return filter.getState().getTheta(); }
const Eigen::MatrixXd INEKFInterface::getRobotStateP() { return filter.getState().getP(); }

void INEKFInterface::Initialize(Eigen::Matrix3d _R0, Eigen::Vector3d _v0, Eigen::Vector3d _bg0, Eigen::Vector3d _ba0) {
...
void INEKFInterface::ReceiveMotorAngleData(double T, std::array<float, 12>& motor_angles) { t = T; }
...
void INEKFInterface::ReceiveImuDataInSim(double T, const pybind11::dict& dictionary) {
  ...
  imu_measurement << gyroscope[0], gyroscope[1], gyroscope[2], accelerometer[0], accelerometer[1], accelerometer[2];
  // cout<<"t="<<t<<endl;
}
...
void INEKFInterface::ReceiveImuData(double T, const IMU& imu) {
  ...
  imu_measurement << (double)imu.gyroscope[0], (double)imu.gyroscope[1], (double)imu.gyroscope[2],
      (double)imu.accelerometer[0], (double)imu.accelerometer[1], (double)imu.accelerometer[2];
...
RobotState INEKFInterface::Update_1() {
  ...
  if (cycle % 5 == 0) filter.setOrientation(oritationMatrix);
  cycle++;
...
PYBIND11_MODULE(inekf_interface, m) {
  ...
  py::class_<INEKFInterface>(m, "INEKFInterface")
      ...
      // .def("receive_imu_data_in_sim", py::overload_cast<double, const
      // py::dict&>(&INEKFInterface::ReceiveImuDataInSim)) .def("receive_imu_data", py::overload_cast<double, const
      // IMU&>(&INEKFInterface::ReceiveImuData))
