...
std::ostream& operator<<(std::ostream& os, const NestedStruct& v) { return os << "n:a=" << v.a << ";b=" << v.b; }
...
std::ostream& operator<<(std::ostream& os, const ComplexStruct& v) { return os << "c:" << v.cflt << "," << v.cdbl; }
...
TEST_SUBMODULE(numpy_dtypes, m) {
  ...
  py::class_<SimpleStruct>(m, "SimpleStruct")
      ...
      .def(
          "astuple",
          [](const SimpleStruct& self) { return py::make_tuple(self.bool_, self.uint_, self.float_, self.ldbl_); })
      .def_static("fromtuple", [](const py::tuple tup) {
        if (py::len(tup) != 4) {
          throw py::cast_error("Invalid size");
        }
        return SimpleStruct{
            tup[0].cast<bool>(), tup[1].cast<uint32_t>(), tup[2].cast<float>(), tup[3].cast<long double>()};
      });
  ...
  m.def("print_dtypes", []() {
    ...
    for (const py::handle& d :
         {py::dtype::of<SimpleStruct>(), py::dtype::of<PackedStruct>(), py::dtype::of<NestedStruct>(),
          py::dtype::of<PartialStruct>(), py::dtype::of<PartialNestedStruct>(), py::dtype::of<StringStruct>(),
          py::dtype::of<ArrayStruct>(), py::dtype::of<EnumStruct>(), py::dtype::of<StructWithUglyNames>(),
  ...
  m.def("create_string_array", [](bool non_empty) {
    ...
    if (non_empty) {
      ...
      for (ssize_t i = 0; i < req.size * req.itemsize; i++) static_cast<char*>(req.ptr)[i] = 0;
      ptr[1].a[0] = 'a';
      ptr[1].b[0] = 'a';
      ptr[2].a[0] = 'a';
      ptr[2].b[0] = 'a';
  ...
  m.def("create_array_array", [](size_t n) {
    ...
    for (size_t i = 0; i < n; i++) {
      ...
      for (size_t j = 0; j < 4; j++)
        for (size_t k = 0; k < 2; k++) ptr[i].d[j][k] = float(i) * 100.0f + float(j) * 10.0f + float(k);
    }
    return arr;
  });
