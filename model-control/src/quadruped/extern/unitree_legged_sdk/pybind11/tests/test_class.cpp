/*
    tests/test_class.cpp -- tests py::class_ definitions and basic functionality
...
TEST_SUBMODULE(class_, m) {
  ...
  m.def("check_instances", [](py::list l) {
    return py::make_tuple(
        py::isinstance<py::tuple>(l[0]), py::isinstance<py::dict>(l[1]), py::isinstance<Pet>(l[2]),
        py::isinstance<Pet>(l[3]), py::isinstance<Dog>(l[4]), py::isinstance<Rabbit>(l[5]),
  ...
  // This tests is actually part of test_local_bindings (test_duplicate_local), but we need a
...
#define CHECK_ALIAS(N)                                                                                       \
  static_assert(                                                                                             \
      DoesntBreak##N::has_alias && std::is_same<typename DoesntBreak##N::type_alias, BreaksTramp<N>>::value, \
      "DoesntBreak" #N " has wrong type_alias!")
#define CHECK_NOALIAS(N)                                                                      \
  static_assert(                                                                              \
      !DoesntBreak##N::has_alias && std::is_void<typename DoesntBreak##N::type_alias>::value, \
...
#define CHECK_HOLDER(N, TYPE)                                                                    \
  static_assert(                                                                                 \
      std::is_same<typename DoesntBreak##N::holder_type, std::TYPE##_ptr<BreaksBase<N>>>::value, \
...
// There's no nice way to tests that these fail because they fail to compile; leave them here,
