...
/* Custom type caster move/copy tests classes */
class MoveOnlyInt {
public:
...
TEST_SUBMODULE(copy_move_policies, m) {
  ...
  m.def(
      "copy_tuple", [](std::tuple<CopyOnlyInt, CopyOnlyInt> t) { return std::get<0>(t).value + std::get<1>(t).value; });
  m.def(
      "move_copy_nested",
      [](std::pair<
          MoveOnlyInt, std::pair<std::tuple<MoveOrCopyInt, CopyOnlyInt, std::tuple<MoveOnlyInt>>, MoveOrCopyInt>> x) {
