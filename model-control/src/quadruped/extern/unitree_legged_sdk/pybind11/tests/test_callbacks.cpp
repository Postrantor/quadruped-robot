...
TEST_SUBMODULE(callbacks, m) {
  ...
  m.def("test_unpacking_and_keywords2", [](py::function f) {
    ...
    return f(
        "positional", *py::make_tuple(1), 2, *py::make_tuple(3, 4), 5, "key"_a = "value", **kwargs1, "b"_a = 2,
        **kwargs2, "e"_a = 5);
  });
  ...
  m.def(
      ...
      [](std::function<int(int)> f, bool expect_none = false) {
        if (expect_none && f) throw std::runtime_error("Expected None to be converted to empty std::function");
        return f;
      },
