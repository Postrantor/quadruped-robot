...
template <>
struct hash<Vector2> {
  // Not a good hash function, but easy to tests
  size_t operator()(const Vector2 &) { return 4; }
};
}  // namespace std

// Not a good abs function, but easy to tests.
...
TEST_SUBMODULE(operators, m) {
  ...
  py::class_<Hashable>(m, "Hashable").def(py::init<int>()).def(py::self == py::self).def("__hash__", &Hashable::hash);

  // define __hash__ before __eq__
  py::class_<Hashable2>(m, "Hashable2").def("__hash__", &Hashable::hash).def(py::init<int>()).def(py::self == py::self);
}
