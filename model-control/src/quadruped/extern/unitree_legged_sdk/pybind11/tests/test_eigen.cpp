...
// Returns element 2,1 from a matrix (used to tests copy/nocopy)
double get_elem(Eigen::Ref<const Eigen::MatrixXd> m) { return m(2, 1); };

// Returns a matrix with 10*r + 100*c added to each matrix element (to help tests that the matrix
...
TEST_SUBMODULE(eigen, m) {
  ...
  m.def(
      "block", [](const Eigen::Ref<const Eigen::MatrixXd> &x, int start_row, int start_col, int block_rows,
  ...
  m.def(
      ...
      py::arg().noconvert());
  // Also tests a row-major-only no-copy const ref:
  m.def(
      "get_elem_rm_nocopy",
  ...
  m.def(
      "iss738_f2", &adjust_matrix<const Eigen::Ref<const Eigen::Matrix<double, -1, -1, Eigen::RowMajor>> &>,
      py::arg().noconvert());
