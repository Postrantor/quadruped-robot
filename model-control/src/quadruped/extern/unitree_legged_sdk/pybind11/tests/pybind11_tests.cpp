...
tests .cpp calls with its initialization lambda.  It's convenient here because we can just not
compile some tests files to disable/ignore some of the tests code.

It is NOT recommended as a way to use pybind11 in practice, however: the initialization order will
be essentially random, which is okay for our tests scripts (there are no dependencies between the
individual pybind11 tests .cpp files), but most likely not what you want when using pybind11
productively.
...
void bind_ConstructorStats(py::module_ &m) {
  py::class_<ConstructorStats>(m, "ConstructorStats")
      ...
      .def_static(
          "get", (ConstructorStats & (*)(py::object)) & ConstructorStats::get,
