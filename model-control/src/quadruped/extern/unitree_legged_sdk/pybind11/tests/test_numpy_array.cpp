/*
    tests/test_numpy_array.cpp -- tests core array functionality
...
arr& mutate_data(arr& a, Ix... index) {
  ...
  for (ssize_t i = 0; i < a.nbytes() - a.offset_at(index...); i++) ptr[i] = (uint8_t)(ptr[i] * 2);
  return a;
}
...
arr_t& mutate_data_t(arr_t& a, Ix... index) {
  ...
  for (ssize_t i = 0; i < a.size() - a.index_at(index...); i++) ptr[i]++;
  return a;
}
...
TEST_SUBMODULE(numpy_array, sm) {
  ...
  // tests numpy scalars (empty shape, ndim==0)
  ...
  sm.def(
      ...
      [](py::array_t<double> a, double v) {
        ...
        for (ssize_t i = 0; i < r.shape(0); i++)
          for (ssize_t j = 0; j < r.shape(1); j++) r(i, j) += v;
      },
  ...
  sm.def("proxy_init3", [](double start) {
    ...
    for (ssize_t i = 0; i < r.shape(0); i++)
      for (ssize_t j = 0; j < r.shape(1); j++)
        for (ssize_t k = 0; k < r.shape(2); k++) r(i, j, k) = start++;
    return a;
  });
  sm.def("proxy_init3F", [](double start) {
    ...
    for (ssize_t k = 0; k < r.shape(2); k++)
      for (ssize_t j = 0; j < r.shape(1); j++)
        for (ssize_t i = 0; i < r.shape(0); i++) r(i, j, k) = start++;
    return a;
  });
  sm.def("proxy_squared_L2_norm", [](py::array_t<double> a) {
    auto r = a.unchecked<1>();
    double sumsq = 0;
    for (ssize_t i = 0; i < r.shape(0); i++) sumsq += r[i] * r(i);  // Either notation works for a 1D array
    return sumsq;
  });
  ...
  sm.def(
      "array_initializer_list1", []() { return py::array_t<float>(1); });  // { 1 } also works, but clang warns about it
  ...
  sm.def(
      "accept_double_c_style_forcecast", [](py::array_t<double, py::array::forcecast | py::array::c_style>) {},
      py::arg("a"));
  sm.def(
      "accept_double_f_style", [](py::array_t<double, py::array::f_style>) {}, py::arg("a"));
  sm.def(
      "accept_double_f_style_forcecast", [](py::array_t<double, py::array::forcecast | py::array::f_style>) {},
      py::arg("a"));
  sm.def(
      "accept_double_noconvert", [](py::array_t<double, 0>) {}, py::arg("a").noconvert());
  sm.def(
      "accept_double_forcecast_noconvert", [](py::array_t<double, py::array::forcecast>) {}, py::arg("a").noconvert());
  sm.def(
      "accept_double_c_style_noconvert", [](py::array_t<double, py::array::c_style>) {}, py::arg("a").noconvert());
  sm.def(
      "accept_double_c_style_forcecast_noconvert",
      [](py::array_t<double, py::array::forcecast | py::array::c_style>) {}, py::arg("a").noconvert());
  sm.def(
      "accept_double_f_style_noconvert", [](py::array_t<double, py::array::f_style>) {}, py::arg("a").noconvert());
  sm.def(
