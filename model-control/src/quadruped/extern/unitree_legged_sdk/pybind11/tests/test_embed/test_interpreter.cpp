...
TEST_CASE("Import error handling") {
  ...
  REQUIRE_THROWS_WITH(py::module_::import("throw_error_already_set"), Catch::Contains("ImportError: KeyError"));
}
...
TEST_CASE("Restart the interpreter") {
  ...
  REQUIRE(
      reinterpret_cast<uintptr_t>(*py::detail::get_internals_pp()) ==
...
TEST_CASE("Subinterpreter") {
  // Add tags to the modules in the main interpreter and tests the basics.
...
TEST_CASE("Reload module from file") {
  // Disable generation of cached bytecode (.pyc files) for this tests, otherwise
  ...
  int result = module_.attr("tests")().cast<int>();
  REQUIRE(result == 1);
  ...
  // Reload the module
  module_.reload();
  result = module_.attr("tests")().cast<int>();
  REQUIRE(result == 2);
}
