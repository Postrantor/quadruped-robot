...
GLOBAL void AMD_postorder(
...
#ifndef NDEBUG
  {
    ...
    for (j = 0; j < nn; j++) {
      if (Nv[j] > 0) {
        AMD_DEBUG1(
            ("" ID " :  nels " ID " npiv " ID " size " ID " parent " ID " maxfr " ID "\n", j, nels, Nv[j], Fsize[j],
             Parent[j], Fsize[j]));
        /* this is an element */
  ...
  for (i = 0; i < nn; i++) {
    if (Nv[i] > 0 && Child[i] != EMPTY) {
      ...
      AMD_DEBUG1(
          ("bigf " ID " maxfrsize " ID " bigfprev " ID " fnext " ID " fprev " ID "\n", bigf, maxfrsize, bigfprev, fnext,
           fprev));
