...
T FloatingBaseModel<T>::applyTestForce(
    const int gc_index, const Vec3<T> &force_ics_at_contact, DVec<T> &dstate_out) {
  forwardKinematics();
...
int FloatingBaseModel<T>::addBody(
    ...
    const Mat6<T> &Xrot) {
  return addBody(
      SpatialInertia<T>(inertia), SpatialInertia<T>(rotorInertia), gearRatio, parent, jointType,
      jointAxis, Xtree, Xrot);
}

template <typename T>
void FloatingBaseModel<T>::check() {
  if (_nDof != _parents.size()) throw std::runtime_error("Invalid dof and parents length");
}
...
void FloatingBaseModel<T>::forwardKinematics() {
  ...
  for (size_t j = 0; j < _nGroundContact; j++) {
    ...
    // std::cout << "gc [" <<j<< "] " << "pGC=" << (R* (_pGC.at(j) -
    // _state.bodyPosition)).transpose() << "  vGC=" << _vGC.at(j).transpose() << std::endl;
  }
...
void FloatingBaseModel<T>::contactJacobians() {
  ...
  for (size_t k = 0; k < _nGroundContact; k++) {
    ...
    // Rotation to absolute coords
    Mat3<T> Rai = _Xa[i].template block<3, 3>(0, 0).transpose();
    // _gcLocation[k] is the local coordinate of point k in body-i frame, but body-i frame is not
    // the same as COM frame ?
    Mat6<T> Xc = createSXform(
        Rai,
        _gcLocation.at(k));  // Xc is binded to translated world frame WITH origin at body i COM ?
...
DVec<T> FloatingBaseModel<T>::generalizedGravityForce() {
  ...
  for (size_t i = 6; i < _nDof; i++) {
    ...
    // body and rotor
    _G[i] =
        -_S[i].dot(_IC[i].getMatrix() * _ag[i]) - _Srot[i].dot(_Irot[i].getMatrix() * _agrot[i]);
  }
  return _G;
}
...
void FloatingBaseModel<T>::runABA(const DVec<T> &tau, FBModelStateDerivative<T> &dstate) {
  ...
  for (size_t i = _nDof - 1; i >= 6; i--) {
    _u[i] = tau[i - 6] - _S[i].transpose() * _pA[i] - _Srot[i].transpose() * _pArot[i] -
            _U[i].transpose() * _c[i] - _Urot[i].transpose() * _crot[i];
