---
tip: translate by baidu@2023-09-19 06:47:39
...
...
> 此分支已在**Ubuntu 20.04**下的**ROS Noetic**中进行了测试。一些用户报告它可以在**Ubuntu 18.04**下在**ROS Melodic**中运行。为了确保最佳性能，我们建议在**Ubuntu 20.004**下**ROS Nootic**中测试此分支。
...
> - **示例**包含一些示例，以帮助用户了解用法和项目架构。
> - **四足动物**包含定义机器人、控制器、规划器、动力学和支持算法的主要模块。
...
> - 导航到**quadruped**文件夹，取消注释“CMakeLists.txt”文件中任何与“BLAS”相关的内容(如果有注释)，然后将 followihng 选项设置为“ON”，并确保已安装“libopenblas-dev”。
...
> “use_xacro”(可选)参数确定是通过“xacro”还是“urdf”加载模型。默认值为“true”。
...
> **请注意，在执行上述命令后，按“Enter”键启动机器人的控制器。低级控制器现在处于活动状态，准备支持四足机器人的运动**
...
> 这里是一个控制 Unitree 机器人 A1(或 DeepRobotics Lite3)四处移动的例子。请检查**quadruped/config**文件夹中的`user_parameters.yaml`文件是否已为 robot 正确配置。
...
> 利用 ROS JOY，您可以使用游戏操纵杆(例如 Logitech F710)发送速度命令来操作四足机器人。
...
> 启动欢乐节点后，按下“A”按钮激活欢乐控制模式，然后操纵操纵杆手柄控制机器人移动。
...
> 要在真实的机器人(A1)上运行示例代码，请打开一个新终端并执行
